...
const liquidationReserve = Number(YUSD_LIQUIDATION_RESERVE);
const maximumBorrowingRate = Number(MAXIMUM_BORROWING_RATE);

const maxDebt = 10 * Number(YUSD_MINIMUM_DEBT);
        ...
        fc.property(validNonEmptyTrove(), validNonEmptyTrove(), (a, b) => {
          ...
          return (
            ...
            (change.type === "adjustment" &&
              ...
              !change.params.borrowYUSD?.isZero &&
              !change.params.repayYUSD?.isZero)
          );
        })
      );
    });
      ...
      fc.assert(
        fc.property(fc.float({ max: maximumBorrowingRate }), borrowingRate => {
          const withMinimumDebt = Trove.recreate(
            new Trove(Decimal.ONE, YUSD_MINIMUM_DEBT),
            borrowingRate
          );

          const ret = Trove.create(withMinimumDebt, borrowingRate).debt.gte(YUSD_MINIMUM_DEBT);
