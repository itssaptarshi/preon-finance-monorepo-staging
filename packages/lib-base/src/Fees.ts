...
export class Fees {
   ...
   * the `when` parameter.
   *
   * To calculate the borrowing fee in YUSD, multiply the borrowed YUSD amount by the borrowing rate.
   *
   * @example
   * ```typescript
   * const fees = await liquity.getFees();
   *
   * const borrowedYUSDAmount = 100;
   * const borrowingRate = fees.borrowingRate();
   * const borrowingFeeYUSD = borrowingRate.mul(borrowedYUSDAmount);
   * ```
   */
   ...
   *
   * @param redeemedFractionOfSupply - The amount of YUSD being redeemed divided by the total supply.
   ...
   * precise, it depends on the fraction of the redeemed amount compared to the total YUSD supply,
   * which must be passed as a parameter.
   *
   * To calculate the redemption fee in YUSD, multiply the redeemed YUSD amount with the redemption
   * rate.
   *
   * @example
   * ```typescript
   ...
   *
   * const redeemedYUSDAmount = Decimal.from(100);
   * const redeemedFractionOfSupply = redeemedYUSDAmount.div(total.debt);
   * const redemptionRate = fees.redemptionRate(redeemedFractionOfSupply);
   * const redemptionFeeYUSD = redemptionRate.mul(redeemedYUSDAmount);
   * ```
   */
