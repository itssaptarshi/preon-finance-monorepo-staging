...
export interface TroveCreationDetails {
  ...
  /** Amount of YUSD added to the Trove's debt as borrowing fee. */
  fee: Decimal;
}
...
/**
 * Details of a {@link TransactableLiquity.redeemYUSD | redeemYUSD()} transaction.
 *
 * @public
 */
export interface RedemptionDetails {
   ...
   *
   * @remarks
   * This can end up being lower than `attemptedYUSDAmount` due to interference from another
...
/**
 * Details of a
 * {@link TransactableLiquity.depositYUSDInStabilityPool | depositYUSDInStabilityPool()} or
 * {@link TransactableLiquity.withdrawYUSDFromStabilityPool | withdrawYUSDFromStabilityPool()}
 * transaction.
 *
 * @public
 */
...
export interface TransactableLiquity {
   ...
   * {@link @liquity/lib-base#YUSD_MINIMUM_DEBT}.
   *
   * If `maxBorrowingRate` is omitted, the current borrowing rate plus 0.5% is used as maximum
   * acceptable rate.
   */
   ...
   *
   * @param amount - The amount of YUSD to borrow.
   * @param maxBorrowingRate - Maximum acceptable
   *                           {@link @liquity/lib-base#Fees.borrowingRate | borrowing rate}.
   *
   * @throws
   ...
   * ```
   */
  borrowYUSD(amount: Decimalish, maxBorrowingRate?: Decimalish): Promise<TroveAdjustmentDetails>;

  /**
   * Adjust existing Trove by repaying some of its debt.
   *
   * @param amount - The amount of YUSD to repay.
   *
   * @throws
   * Throws {@link TransactionFailedError} in case of transaction failure.
   *
   ...
   *
   * @param amount - Amount of YUSD to add to new or existing deposit.
   ...
   *
   * @param amount - Amount of YUSD to withdraw.
   *
   * @throws
   * Throws {@link TransactionFailedError} in case of transaction failure.
   *
   ...
   * {@link @liquity/lib-base#StabilityDeposit.lqtyReward | LQTY reward}.
   */
  withdrawYUSDFromStabilityPool(amount: Decimalish): Promise<StabilityDepositChangeDetails>;
   ...
   * @param amount - Amount of YUSD to send.
   *
   * @throws
   * Throws {@link TransactionFailedError} in case of transaction failure.
   */
   ...
   *
   * @param amount - Amount of YUSD to be redeemed.
   ...
   */
  redeemYUSD(amount: Decimalish, maxRedemptionRate?: Decimalish): Promise<RedemptionDetails>;
   ...
   * {@link @liquity/lib-base#veYETIStake.collateralGain | collateral gain} and
   * {@link @liquity/lib-base#veYETIStake.yusdGain | YUSD gain}.
   */
  ...
  /**
   * Withdraw {@link @liquity/lib-base#veYETIStake.collateralGain | collateral gain} and
   * {@link @liquity/lib-base#veYETIStake.yusdGain | YUSD gain} from LQTY stake.
   *
   * @throws
   * Throws {@link TransactionFailedError} in case of transaction failure.
   */
  ...
  /**
   * Allow the liquidity mining contract to use Uniswap ETH/YUSD LP tokens for
  ...
  /**
   * Stake Uniswap ETH/YUSD LP tokens to participate in liquidity mining and earn LQTY.
   *
  ...
  /**
   * Withdraw Uniswap ETH/YUSD LP tokens from liquidity mining.
   *
