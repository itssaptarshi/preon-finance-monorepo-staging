...
export class ObservableEthersLiquity implements ObservableLiquity {
  ...
  watchYUSDInStabilityPool(
    onYUSDInStabilityPoolChanged: (yusdInStabilityPool: Decimal) => void
  ): () => void {
    const { yusdToken, stabilityPool } = _getContracts(this._readable.connection);
    const { Transfer } = yusdToken.filters;

    const transferYUSDFromStabilityPool = Transfer(stabilityPool.address);
    const transferYUSDToStabilityPool = Transfer(null, stabilityPool.address);

    const stabilityPoolYUSDFilters = [transferYUSDFromStabilityPool, transferYUSDToStabilityPool];

    const stabilityPoolYUSDListener = debounce((blockTag: number) => {
      this._readable.getYUSDInStabilityPool({ blockTag }).then(onYUSDInStabilityPoolChanged);
    });

    stabilityPoolYUSDFilters.forEach(filter => yusdToken.on(filter, stabilityPoolYUSDListener));

    return () =>
      stabilityPoolYUSDFilters.forEach(filter =>
        yusdToken.removeListener(filter, stabilityPoolYUSDListener)
      );
  }
  ...
  watchYUSDBalance(onYUSDBalanceChanged: (balance: Decimal) => void, address?: string): () => void {
    ...
    const { Transfer } = yusdToken.filters;
    const transferYUSDFromUser = Transfer(address);
    const transferYUSDToUser = Transfer(null, address);

    const yusdTransferFilters = [transferYUSDFromUser, transferYUSDToUser];

    const yusdTransferListener = debounce((blockTag: number) => {
      this._readable.getYUSDBalance(address, { blockTag }).then(onYUSDBalanceChanged);
    });

    yusdTransferFilters.forEach(filter => yusdToken.on(filter, yusdTransferListener));

    return () =>
      yusdTransferFilters.forEach(filter => yusdToken.removeListener(filter, yusdTransferListener));
  }
}
