...
describe("EthersLiquity", () => {
  ...
  describe("findHintForCollateralRatio", () => {
    it("should pick the closest approx hint", async () => {
      ...
      const params = Trove.recreate(new Trove(Decimal.from(1), YUSD_MINIMUM_DEBT));
  ...
  describe("Trove", () => {
    ...
    it("should fail to create an undercollateralized Trove", async () => {
      const price = await liquity.getPrice();
      const undercollateralized = new Trove(YUSD_MINIMUM_DEBT.div(price), YUSD_MINIMUM_DEBT);
    ...
    it("should fail to create a Trove with too little debt", async () => {
      const withTooLittleDebt = new Trove(Decimal.from(50), YUSD_MINIMUM_DEBT.sub(1));

      await expect(liquity.openTrove(Trove.recreate(withTooLittleDebt))).to.eventually.be.rejected;
    });

    const withSomeBorrowing = { depositCollateral: 50, borrowYUSD: YUSD_MINIMUM_NET_DEBT.add(100) };
    ...
    it("should create a Trove with some borrowing", async () => {
      ...
      expect(`${fee}`).to.equal(`${MINIMUM_BORROWING_RATE.mul(withSomeBorrowing.borrowYUSD)}`);
    });
    ...
    const repaySomeDebt = { repayYUSD: 10 };

    it("should repay some debt", async () => {
      const { newTrove, fee } = await liquity.repayYUSD(repaySomeDebt.repayYUSD);
      expect(newTrove).to.deep.equal(Trove.create(withSomeBorrowing).adjust(repaySomeDebt));
      expect(`${fee}`).to.equal("0");
    });

    const borrowSomeMore = { borrowYUSD: 20 };

    it("should borrow some more", async () => {
      const { newTrove, fee } = await liquity.borrowYUSD(borrowSomeMore.borrowYUSD);
      expect(newTrove).to.deep.equal(
        Trove.create(withSomeBorrowing).adjust(repaySomeDebt).adjust(borrowSomeMore)
      );
      expect(`${fee}`).to.equal(`${MINIMUM_BORROWING_RATE.mul(borrowSomeMore.borrowYUSD)}`);
    });
    ...
    const repayAndWithdraw = { repayYUSD: 60, withdrawCollateral: 0.5 };
    ...
    const borrowAndDeposit = { borrowYUSD: 60, depositCollateral: 0.5 };
    ...
    it("should borrow more and deposit some collateral at the same time", async () => {
      ...
      expect(`${fee}`).to.equal(`${MINIMUM_BORROWING_RATE.mul(borrowAndDeposit.borrowYUSD)}`);
    ...
    it("should close the Trove with some YUSD from another user", async () => {
      const price = await liquity.getPrice();
      const initialTrove = await liquity.getTrove();
      const yusdBalance = await liquity.getLQTYBalance();
      const yusdShortage = initialTrove.netDebt.sub(yusdBalance);

      let funderTrove = Trove.create({ depositCollateral: 1, borrowYUSD: yusdShortage });
      funderTrove = funderTrove.setDebt(Decimal.max(funderTrove.debt, YUSD_MINIMUM_DEBT));
      ...
      await funderLiquity.sendYUSD(await user.getAddress(), yusdShortage);

      const { params } = await liquity.closeTrove();

      expect(params).to.deep.equal({
        withdrawCollateral: initialTrove.collateral,
        repayYUSD: initialTrove.netDebt
      });
  ...
  describe("SendableEthersLiquity", () => {
    it("should parse failed transactions without throwing", async () => {
      ...
      const tx = await liquity.send.openTrove(
        { depositCollateral: 0.01, borrowYUSD: 0.01 },
        undefined,
        { gasLimit: 1e6 }
      );
  ...
  describe("Frontend", () => {
    ...
    it("other user's deposit should be tagged with the frontend's address", async () => {
      ...
      await otherLiquity.openTrove({ depositCollateral: 20, borrowYUSD: YUSD_MINIMUM_DEBT });

      await otherLiquity.depositYUSDInStabilityPool(YUSD_MINIMUM_DEBT);
  ...
  describe("StabilityPool", () => {
    before(async () => {
      ...
      await funder.sendTransaction({
        to: otherUsers[0].getAddress(),
        value: YUSD_MINIMUM_DEBT.div(170).hex
      });
    });

    const initialTroveOfDepositor = Trove.create({
      depositCollateral: YUSD_MINIMUM_DEBT.div(100),
      borrowYUSD: YUSD_MINIMUM_NET_DEBT
    });
    ...
    it("should make a small stability deposit", async () => {
      ...
      const details = await liquity.depositYUSDInStabilityPool(smallStabilityDeposit);

      expect(details).to.deep.equal({
        yusdLoss: Decimal.from(0),
        newYUSDDeposit: smallStabilityDeposit,
        collateralGain: Decimal.from(0),
        lqtyReward: Decimal.from(0),

        change: {
          depositYUSD: smallStabilityDeposit
        }
      });
    });

    const troveWithVeryLowICR = Trove.create({
      depositCollateral: YUSD_MINIMUM_DEBT.div(180),
      borrowYUSD: YUSD_MINIMUM_NET_DEBT
    });
    ...
    it("should liquidate other user's Trove", async () => {
      ...
      expect(details).to.deep.equal({
        ...
        yusdGasCompensation: YUSD_LIQUIDATION_RESERVE,
    ...
    it("should transfer the gains to the Trove", async () => {
      ...
      expect(details).to.deep.equal({
        yusdLoss: smallStabilityDeposit,
        newYUSDDeposit: Decimal.ZERO,
    ...
    describe("when people overstay", () => {
      before(async () => {
        ...
        // Use this account to print YUSD
        await liquity.openTrove({ depositCollateral: 50, borrowYUSD: 5000 });

        // otherLiquities[0-2] will be independent stability depositors
        await liquity.sendYUSD(await otherUsers[0].getAddress(), 3000);
        await liquity.sendYUSD(await otherUsers[1].getAddress(), 1000);
        await liquity.sendYUSD(await otherUsers[2].getAddress(), 1000);

        // otherLiquities[3-4] will be Trove owners whose Troves get liquidated
        await otherLiquities[3].openTrove({ depositCollateral: 21, borrowYUSD: 2900 });
        await otherLiquities[4].openTrove({ depositCollateral: 21, borrowYUSD: 2900 });

        await otherLiquities[0].depositYUSDInStabilityPool(3000);
        await otherLiquities[1].depositYUSDInStabilityPool(1000);
        ...
        await otherLiquities[2].depositYUSDInStabilityPool(1000);
        ...
        expect(`${await liquity.getYUSDInStabilityPool()}`).to.equal("0");
      });
      ...
      it("should still be able to withdraw remaining deposit", async () => {
        for (const l of [otherLiquities[0], otherLiquities[1], otherLiquities[2]]) {
          ...
          await l.withdrawYUSDFromStabilityPool(stabilityDeposit.currentYUSD);
        }
      });
    });
  });
  ...
  describe("Redemption", () => {
    const troveCreations = [
      { depositCollateral: 99, borrowYUSD: 4600 },
      { depositCollateral: 20, borrowYUSD: 2000 }, // net debt: 2010
      { depositCollateral: 20, borrowYUSD: 2100 }, // net debt: 2110.5
      { depositCollateral: 20, borrowYUSD: 2200 } //  net debt: 2211
    ];
    ...
    it("should fail to redeem during the bootstrap phase", async () => {
      ...
      await expect(liquity.redeemYUSD(4326.5)).to.eventually.be.rejected;
    });

    const someYUSD = Decimal.from(4326.5);

    it("should redeem some YUSD after the bootstrap phase", async () => {
      ...
      const expectedDetails = {
        attemptedYUSDAmount: someYUSD,
        actualYUSDAmount: someYUSD,
        collateralTaken: someYUSD.div(200),
        fee: new Fees(0, 0.99, 2, new Date(), new Date(), false)
          .redemptionRate(someYUSD.div(total.debt))
          .mul(someYUSD.div(200))
      };

      const { rawReceipt, details } = await waitForSuccess(liquity.send.redeemYUSD(someYUSD));
      ...
      expect(`${await liquity.getYUSDBalance()}`).to.equal("273.5");
    ...
    it("borrowing rate should be maxed out now", async () => {
      const borrowYUSD = Decimal.from(10);

      const { fee, newTrove } = await liquity.borrowYUSD(borrowYUSD);
      expect(`${fee}`).to.equal(`${borrowYUSD.mul(MAXIMUM_BORROWING_RATE)}`);

      expect(newTrove).to.deep.equal(
        Trove.create(troveCreations[0]).adjust({ borrowYUSD }, MAXIMUM_BORROWING_RATE)
      );
    });
  });

  describe("Redemption (truncation)", () => {
    const troveCreationParams = { depositCollateral: 20, borrowYUSD: 2000 };
    const netDebtPerTrove = Trove.create(troveCreationParams).netDebt;
    const amountToAttempt = Decimal.from(3000);
    const expectedRedeemable = netDebtPerTrove.mul(2).sub(YUSD_MINIMUM_NET_DEBT);
    ...
    beforeEach(async () => {
      ...
      await liquity.openTrove({ depositCollateral: 99, borrowYUSD: 5000 });
    ...
    it("should truncate the amount if it would put the last Trove below the min debt", async () => {
      const redemption = await liquity.populate.redeemYUSD(amountToAttempt);
      expect(`${redemption.attemptedYUSDAmount}`).to.equal(`${amountToAttempt}`);
      expect(`${redemption.redeemableYUSDAmount}`).to.equal(`${expectedRedeemable}`);
      expect(redemption.isTruncated).to.be.true;

      const { details } = await waitForSuccess(redemption.send());
      expect(`${details.attemptedYUSDAmount}`).to.equal(`${expectedRedeemable}`);
      expect(`${details.actualYUSDAmount}`).to.equal(`${expectedRedeemable}`);
    });

    it("should increase the amount to the next lowest redeemable value", async () => {
      const increasedRedeemable = expectedRedeemable.add(YUSD_MINIMUM_NET_DEBT);

      const initialRedemption = await liquity.populate.redeemYUSD(amountToAttempt);
      const increasedRedemption = await initialRedemption.increaseAmountByMinimumNetDebt();
      expect(`${increasedRedemption.attemptedYUSDAmount}`).to.equal(`${increasedRedeemable}`);
      expect(`${increasedRedemption.redeemableYUSDAmount}`).to.equal(`${increasedRedeemable}`);
      expect(increasedRedemption.isTruncated).to.be.false;

      const { details } = await waitForSuccess(increasedRedemption.send());
      expect(`${details.attemptedYUSDAmount}`).to.equal(`${increasedRedeemable}`);
      expect(`${details.actualYUSDAmount}`).to.equal(`${increasedRedeemable}`);
    });

    it("should fail to increase the amount if it's not truncated", async () => {
      const redemption = await liquity.populate.redeemYUSD(netDebtPerTrove);
  ...
  describe("Redemption (gas checks)", function () {
    ...
    before(async function () {
      ...
      for (const otherLiquity of otherLiquities) {
        await otherLiquity.openTrove({
          ...
          borrowYUSD: amountToBorrowPerTrove
        });
      }
    ...
    it("should redeem using the maximum iterations and almost all gas", async () => {
      await liquity.openTrove({
        ...
        borrowYUSD: amountToRedeem
      });

      const { rawReceipt } = await waitForSuccess(liquity.send.redeemYUSD(amountToRedeem));
  ...
  // Test workarounds related to https://github.com/0xDyeus/yeti2-app-staging/issues/600
  describe("Hints (adjustTrove)", () => {
    ...
    before(async () => {
      ...
      await openTroves(eightOtherUsers, [
        { depositCollateral: 30, borrowYUSD: 2000 }, // 0
        { depositCollateral: 30, borrowYUSD: 2100 }, // 1
        { depositCollateral: 30, borrowYUSD: 2200 }, // 2
        { depositCollateral: 30, borrowYUSD: 2300 }, // 3
        // Test 1:           30,             2400
        { depositCollateral: 30, borrowYUSD: 2500 }, // 4
        { depositCollateral: 30, borrowYUSD: 2600 }, // 5
        { depositCollateral: 30, borrowYUSD: 2700 }, // 6
        { depositCollateral: 30, borrowYUSD: 2800 } //  7
        // Test 2:           30,             2900
    ...
    it("should not traverse the whole list when bottom Trove moves", async () => {
      ...
      const targetTrove = Trove.create({ depositCollateral: 30, borrowYUSD: 2900 });
      const interferingTrove = Trove.create({ depositCollateral: 30, borrowYUSD: 3000 });
    ...
    it("should not traverse the whole list when lowering ICR of bottom Trove", async () => {
      ...
      const targetTrove = [
        Trove.create({ depositCollateral: 30, borrowYUSD: 3100 }),
        Trove.create({ depositCollateral: 30, borrowYUSD: 3200 })
      ];
  ...
  describe("Gas estimation", () => {
    ...
    before(async function () {
      ...
      await openTroves(fiveOtherUsers, [
        { depositCollateral: 20, borrowYUSD: 2040 },
        { depositCollateral: 20, borrowYUSD: 2050 },
        { depositCollateral: 20, borrowYUSD: 2060 },
        { depositCollateral: 20, borrowYUSD: 2070 },
        { depositCollateral: 20, borrowYUSD: 2080 }
      ]);
    ...
    it("should include enough gas for updating lastFeeOperationTime", async () => {
      await liquity.openTrove({ depositCollateral: 20, borrowYUSD: 2090 });

      // We just updated lastFeeOperationTime, so this won't anticipate having to update that
      // during estimateGas
      const tx = await liquity.populate.redeemYUSD(1);
    ...
    it("should include enough gas for one extra traversal", async () => {
      ...
      expect(adjustment.borrowYUSD).to.be.undefined;
      ...
      assertDefined(rudeCreation.borrowYUSD);
      const yusdShortage = rudeTrove.debt.sub(rudeCreation.borrowYUSD);

      await liquity.sendYUSD(await rudeUser.getAddress(), yusdShortage);
    ...
    it("should include enough gas for both when borrowing", async () => {
      ...
      const adjustment = trove.adjustTo(newTrove);
      expect(adjustment.borrowYUSD).to.not.be.undefined;
  ...
  describe("Gas estimation (LQTY issuance)", () => {
    ...
    it("should include enough gas for issuing LQTY", async function () {
      ...
      await liquity.openTrove({ depositCollateral: 40, borrowYUSD: 4000 });
      await liquity.depositYUSDInStabilityPool(19);
      ...
      const deposit = await liquity.populate.depositYUSDInStabilityPool(1);
      const withdraw = await liquity.populate.withdrawYUSDFromStabilityPool(1);
      ...
      await deployerLiquity.depositYUSDInStabilityPool(creation.borrowYUSD);
    ...
    before(async function () {
      ...
      await openTroves(
        ...
        someMoreUsers.map((_, i) => ({
          ...
          borrowYUSD: YUSD_MINIMUM_NET_DEBT.add(i / 10)
        }))
      );
      ...
      await Promise.all(
        otherLiquities.map(async otherLiquity =>
          otherLiquity.sendYUSD(await user.getAddress(), await otherLiquity.getYUSDBalance())
        )
      );
      ...
      const redeemedTroveDebt = await liquity
        .getYUSDBalance()
        .then(x => x.div(10).add(YUSD_LIQUIDATION_RESERVE));
      ...
      const { actualYUSDAmount } = await liquity.redeemYUSD(redeemedTrove.netDebt);

      expect(`${actualYUSDAmount}`).to.equal(`${redeemedTrove.netDebt}`);
