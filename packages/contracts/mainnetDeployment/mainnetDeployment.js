...
async function mainnetDeploy(configParams) {
  ...
  if (deployerLPTokenBal.toString() == "0") {
    ...
    const minPREONAmount = PREONAmount.sub(toBigNum(dec(100, 18)));
  ...
  await mdh.sendAndWaitForTransaction(
    liquityCore.preonController.addCollateral(
      ...
      "1000000000000000000",
      "1000000000000000000",
      liquityCore.avaxPriceFeed.address,
      18,
  ...
  if (troveStatus.toString() != "1") {
    ...
    let _3ETHcoll = th.dec(500, "ether"); // 500 ETH
    await mdh.sendAndWaitForTransaction(
      liquityCore.borrowerOperations.openTrove(
        th.dec(5, 15), // max fee percentage
  ...
  if (deployerLPTokenBal.toString() == "0") {
    ...
    const minSTARAmount = STARAmount.sub(toBigNum(dec(10, 18)));
    ...
    await mdh.sendAndWaitForTransaction(
      uniswapV2Router02.addLiquidityAVAX(
        ...
        minSTARAmount, // minimum STAR provision
        LP_ETH, // minimum ETH provision
        deployerWallet.address, // address to send LP tokens to
        tenMinsFromNow, // deadline for this tx
        {
  ...
  console.log("@preon minter: reward:", _reward, +_reward);
