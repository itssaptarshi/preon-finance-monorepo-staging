...
const STAR = "0x5f98805A4E8be255a32880FDeC7F6728C6568bA0";
const CHAINLINK_ETHUSD_PROXY = "0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419";

const tokens = [STAR, WETH];
const weights = [toBigNum(dec(4, 17)), toBigNum(dec(6, 17))];

const NAME = "WETH/STAR Pool";
const SYMBOL = "60WETH-40STAR";
...
async function main() {
  ...
  */
  ...
  const star_balance = INITIAL_FUNDING.mul(weights[0]).div(
    toBigNum(dec(1, 18))
  );
  const initialBalances = [star_balance, weth_balance];
  th.logBN("Initial STAR", star_balance);
  ...
  // Approve STAR
  const star = new ethers.Contract(STAR, ERC20.abi, deployerWallet);
  const txApprovestar = await star.approve(VAULT, star_balance);
  const approvestarReceipt = await txApprovestar.wait();
  console.log("Approve STAR gas: ", approvestarReceipt.gasUsed.toString());
  ...
  console.log(
    ...
    receipt1.gasUsed
      ...
      .add(approvestarReceipt.gasUsed)
