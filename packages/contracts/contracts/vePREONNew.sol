    ...
    ReentrancyGuardUpgradeable
{
    ...
    string public constant override name = "vePREON";
    string public constant override symbol = "vePREON";
    ...
    function attachToken(uint _tokenId) external override {
        require(msg.sender == boostedFarm, "!boostedFarm");
    ...
    function merge(uint _from, uint _to) external {
        ...
        require(
            _locked0.end > block.timestamp && _locked1.end > block.timestamp,
            "expired"
        ); //<== CHECK NOT EXPIRED
    ...
    /// @notice Unlock all tokens for `_tokenId`
    /// @dev Only possible if the lock has expired
    function unlock(uint _tokenId) external nonReentrant {
        require(_isApprovedOrOwner(msg.sender, _tokenId), "!owner");
    ...
    function rageQuit(uint _tokenId) external nonReentrant {
        require(_isApprovedOrOwner(msg.sender, _tokenId), "!owner");
        require(attachments[_tokenId] == 0, "attached");
        LockedBalance memory _locked = locked[_tokenId];
        ...
        locked[_tokenId] = LockedBalance(0, 0);
        uint supplyBefore = supply;
        uint rageFee = value / 2;
        supply = supplyBefore - value;

        // send back 50% to user
        IERC20(token).safeTransfer(msg.sender, value - rageFee);

        // distribute 50% to holders
        IERC20(token).safeTransfer(vePreonEmissions, rageFee);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "lock #',
                        _toString(_tokenId),
                        '", "description": "Locked PREON which can be used to earn protocol rewards and boosted farming", "image": "data:image/svg+xml;base64,', //change needed
