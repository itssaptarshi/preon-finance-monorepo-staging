...
contract Router is Ownable {
    ...
    function swapPreonVaultToken(
        ...
        if (_tokenIn == _receiptToken) {
            ...
            return IPreonVaultToken(_receiptToken).redeem(_amount);
        } else {
            // Swap _tokenIn for _receiptToken, aka deposit() that amount.
            return IPreonVaultToken(_receiptToken).deposit(_amount);
        }
    }
    ...
    function swap(
        ...
        for (uint256 i; i < path.length; i++) {
            if (path[i].nodeType == 1) {
                ...
                // Is Preon Vault Token
                _amount = swapPreonVaultToken(
                    path[i].tokenIn,
                    path[i].protocolSwapAddress,
                    _amount
                );
