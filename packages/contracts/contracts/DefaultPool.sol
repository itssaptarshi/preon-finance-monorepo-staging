...
/**
 * @notice The Default Pool holds the collateral and STAR debt (but not STAR tokens) from liquidations that have been redistributed
 * to active troves but not yet "applied", i.e. not yet recorded on a recipient active trove's struct.
 *
 * @dev When a trove makes an operation that applies its pending collateral and STAR debt, its pending collateral and STAR debt is moved
...
contract DefaultPool is IDefaultPool, PoolBase2 {
    ...
    /**
     * @notice Increases the STAR Debt of this pool. Called when new STAR is sent to this contract
     *   by redistribution.
     */
    function increaseSTARDebt(uint256 _amount) external override {
        _requireCallerIsTroveManager();
        STARDebt = STARDebt.add(_amount);
        emit DefaultPoolSTARDebtUpdated(STARDebt);
    }

    /**
     * @notice Decreases the STAR Debt of this pool. Called when STAR is sent to active pool when 'rewards'
     *   are claimed.
     */
