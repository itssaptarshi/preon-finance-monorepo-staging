...
// Incremental changes (commit by commit) from the original to this version: https://github.com/0xDyeus/preon2-app-staging/pull/271
...
/*
 * On deployment a new Uniswap pool will be created for the pair STAR/ETH and its token will be set here.
...
contract Pool2Unipool is LPTokenWrapper, Ownable, CheckContract, IUnipool {
    ...
    function setReward(uint _duration) external onlyOwner {
        duration = _duration;
        require(
    ...
    function _updatePeriodFinish() internal {
        if (totalSupply() == 0) {
            require(
                periodFinish != 0,
                "_updatePeriodFinish: periodFinish <= 0"
            );
            /*
    ...
    function _updateAccountReward(address account) internal {
        ...
        require(
            account != address(0),
            "_updateAccountReward: account is address(0)"
        );
