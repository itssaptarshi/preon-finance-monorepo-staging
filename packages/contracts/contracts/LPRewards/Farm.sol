 ...
 * for providing liquidity in Preon-related LP tokens.
 * Preon tokens are allocated to this contract and a reward rate
 * is set by the team in PREON / sec. These PREON are distributed
...
contract Farm is OwnableUpgradeable, LPTokenWrapper {
    ...
    function initialize(IERC20 _LP, IERC20 _PREON) external {
    ...
    function rewardToEarn(uint _amount, uint _time)
        public
        view
        returns (uint256)
    {
        ...
        return
            rewardRate.mul(_time).mul(_amount).div(totalSupply().add(_amount));
    }
}
