...
import "../Vaults/interfaces/IPriceFeed.sol";
...
contract UniswapV2LPTokenPriceFeed is IPriceFeed {
    ...
    constructor(
        IPriceFeed _base0,
        uint32 _base0Decimals,
        IPriceFeed _base1,
        uint32 _base1Decimals,
        address _pair,
        string memory _name
    ) public {
        name = _name;
        ...
        require(
            decimalSum.div(2) != (decimalSum.add(2)).div(2),
            "Decimals must be even"
        );
    ...
    function _getPrice() internal view returns (uint) {
        ...
        uint sqrtK = HomoraMath.sqrt(r0.mul(r1)).fdiv(
