...
interface IStabilityPool is ICollateralReceiver {
    ...
    function claimRewardsSwap(
        uint256 _starMinAmountTotal
    ) external returns (uint256 amountFromSwap);
     ...
     */
    function getCompoundedSTARDeposit(
        address _depositor
    ) external view returns (uint);
    ...
    function getEstimatedPREONPoolRewards(
        uint _amount,
        uint _time
