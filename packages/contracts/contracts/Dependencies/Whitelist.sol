...
/**
 UPDATE : Added whitelistedForInteractions to allow only whitelisted wallets to interact with protocol
 */
...
contract Whitelist is Ownable, IWhitelist, IBaseOracle, CheckContract {
    ...
    mapping(address => bool) whitelistedForInteractions; // UPDATE : whitelistedForInteractions
    ...
    event WalletWhitelisted(address _addressToWhitelist);    // UPDATE : whitelistedForInteractions
    ...
    // ----------Only Owner Setter Functions----------
     ...
     // UPDATE : whitelistedForInteractions
     function setwhitelistedForInteractions(address _addressToWhitelist) external onlyOwner {
        require(!whitelistedForInteractions[_addressToWhitelist]  && _addressToWhitelist != address(0), "Already Whitelisted or Zero Address");
        whitelistedForInteractions[_addressToWhitelist] = true;
        emit WalletWhitelisted(_addressToWhitelist);
    }
    ...
    function addCollateral(
        ...
        require(_safetyRatio < 11e17, "ratio must be less than 1.10"); //=> greater than 1.1 would mean taking out more STAR than collateral VC
        ...
        onlyOwner
    {
        ...
        (uint256 lastFeePercent, uint256 lastFeeTime) = IPriceCurve(
            collateralParams[_collateral].priceCurve
        ).getFeeCapAndTime();
    ...
    function changeSafetyRatio(address _collateral, uint256 _newSafetyRatio)
        ...
        onlyOwner
    {
        require(_newSafetyRatio < 11e17, "ratio must be less than 1.10"); //=> greater than 1.1 would mean taking out more STAR than collateral VC
        require(
    ...
    // UPDATE : whitelistedForInteractions
    function verifyWhitelistWallet(address _whitelistedAddress) public view returns(bool) {
      bool userIsWhitelisted = whitelistedForInteractions[_whitelistedAddress];
        ...
        returns (uint256 VC, uint256 VCforTCR)
    {
        ...
        VCforTCR = price.mul(_amount).mul(recoveryRatio).div(
            10**(18 + decimals)
        );
    }
    ...
    function getValidCaller(address _contract)
        external
        view
        override
        returns (address)
    ...
    function updateValidCaller(address _contract, address _caller)
        external
        onlyOwner
    {
        require(
            !cannotUpdateValidCaller[_contract],
            "cannot update valid caller of this contract"
        );
        validCallers[_contract] = _caller;
    }

    function updatePendingLockCaller(address _contract, bool _lock)
        external
        onlyOwner
    {
    ...
    function lockCaller(address _contract) external onlyOwner {
        cannotUpdateValidCaller[_contract] = pendingLockCallers[_contract];
    }

    // Max Colls in Trove Functions

    function updateMaxCollsInTrove(uint _newMax) external onlyOwner {
