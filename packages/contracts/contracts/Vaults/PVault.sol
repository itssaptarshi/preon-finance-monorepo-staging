...
import "./dependencies/DystopiaExchange.sol";
    ...
    Ownable
{
    ...
    function initialize(
        ...
        token0 = IDystPair(_underlying).token0();
        token1 = IDystPair(_underlying).token1();
    ...
    function _triggerWithdrawAction(uint256 amtToReturn) internal virtual {
        return;
    }
    ...
    function _compound() internal returns (uint256) {
            ...
            if (rewardTokens[i] != address(0)) {
                ...
                if (rewardTokens[i] == address(1)) {
                    ...
                    if (nativeBalance > MIN_SWAP) {
                        ...
                        // WAVAX -> token0 + token1 -> LP
                        // WAVAX -> token0
                        if (address(WAVAX) == token0) {
                            uint256 nativeToSwapForToken1 = nativeBalance / 2;
                            uint256 token1Balance = _swapExactWMATICForTokens(
                                address(WAVAX),
                                address(token1),
                                false,
                                nativeToSwapForToken1,
                                address(this)
                            );
                        } else if (address(WAVAX) == token1) {
                            uint256 nativeToSwapForToken0 = nativeBalance / 2;
                            uint256 token0Balance = _swapExactWMATICForTokens(
                                address(WAVAX),
                                address(token0),
                                false,
                                nativeToSwapForToken0,
                                address(this)
                            );
                        } else {
                            uint256 nativeToSwapForToken1 = nativeBalance / 2;
                    ...
                    if (
                        rewardBalance *
                            (10**(18 - IERC20(rewardTokens[i]).decimals())) >
                        MIN_SWAP
                    ) {
                        if (rewardTokens[i] == token0) {
                            uint256 tokenToSwapForToken1 = rewardBalance / 2;
                            uint256 token1Balance = _swapExactTokensForTokens(
                                address(rewardTokens[i]),
                                address(WAVAX),
                                address(token1),
                                false,
                                false,
                                tokenToSwapForToken1,
                                address(this)
                            );
                        } else if (address(WAVAX) == token1) {
                            uint256 tokenToSwapForToken0 = rewardBalance / 2;
                            uint256 token0Balance = _swapExactTokensForTokens(
                                address(rewardTokens[i]),
                                address(WAVAX),
                                address(token0),
                                false,
                                false,
                                tokenToSwapForToken0,
                                address(this)
                            );
                        } else {
                            uint256 tokenToSwapForToken1 = rewardBalance / 2;
                            ...
                            uint256 token0Balance = _swapExactTokensForTokens(
                                ...
                                false,
                                false,
                                rewardBalance - tokenToSwapForToken1,
                                address(this)
                            );
                        }
                    }
        ...
        uint256 profitInValue = postCompoundUnderlyingValue -
            preCompoundUnderlyingValue;
        if (profitInValue > 0) {
            // convert the profit in value to profit in underlying
            uint256 profitInUnderlying = (profitInValue *
