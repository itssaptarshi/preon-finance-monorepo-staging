...
contract DysonSTMaticStrategy is
    ERC20Upgradeable,
    Initializable,
    ...
    OwnableUpgradeable
{
    ...
    function initialize(
        ...
        require(
            _dystopiaRouter != address(0),
            "_dystopiaRouter: Zero address not allowed"
        );
        require(
            _uniswapRouter != address(0),
            "_uniswapRouter: Zero address not allowed"
        );
        require(
            _rewardToken != address(0),
            "_rewardToken: Zero address not allowed"
        );
    ...
    function getValueOfAllUnderlying() internal view returns (uint256) {
        uint256 amountOut;
        (amountOut, ) = IDystopiaRouter(dystRouter).getAmountOut(
            1 ether,
            address(underlying),
            address(WNATIVE)
        );

        return (amountOut * underlying.balanceOf(address(this))) / 1 ether;
    }
    ...
    function _compound() internal {
        ...
        if (currentValueOfAllUnderlying > lastValueOfAllUnderlying) {
            ...
            if (profitInUnderlying > 0) {
                uint256 _compoundingFee = (profitInUnderlying * fee) / 10_000;

                uint256 _feeSplit = _compoundingFee / 2;
                if (_feeSplit > 0) {
