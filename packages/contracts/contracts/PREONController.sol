...
/**
 * @notice PreonController is the contract that controls system parameters.
 * This includes things like enabling leverUp, feeBootstrap, and pausing the system.
 * PreonController also keeps track of all collateral parameters and allos
 ...
 * PreonController also has view functions to get
 * prices and VC, and RVC values.
 */

contract PreonController is
    OwnableUpgradeable,
    IPreonController,
    CheckContract,
    LiquityBase
{
    ...
    ISTARToken private starToken;
    ISortedTroves private sortedTroves;
    IVePREON private vePREON;
    ...
    address public STARFeeRecipient;
    address public preonFinanceTreasury;
    uint256 public preonFinanceTreasurySplit;
    ...
    event GlobalSTARMintOn(bool _canMint);
    event STARMinterChanged(address _minter, bool _canMint);
    event DefaultRouterChanged(address _collateral, address _newDefaultRouter);
    event PreonFinanceTreasuryChanged(address _newTreasury);
    event ClaimAddressChanged(address _newClaimAddress);
    event PreonFinanceTreasurySplitChanged(uint256 _newSplit);
    event RedemptionBorrowerFeeSplitChanged(uint256 _newSplit);
    event STARFeeRecipientChanged(address _newFeeRecipient);
    ...
    event UpdateVePreonCallers(address _contractAddress, bool _isWhitelisted);
    ...
    function _exists(address _collateral) internal view {
        if (validCollateral[0] != _collateral) {
            require(
                collateralParams[_collateral].index != 0,
    ...
    function _requireThreeDayTimelock() internal view {
        if (bootstrapEnded) {
            require(
                msg.sender == threeDayTimelock,
                "Caller Not Three Day Timelock"
            );
        } else {
    ...
    function _requireTwoWeekTimelock() internal view {
        if (bootstrapEnded) {
            require(
                msg.sender == twoWeekTimelock,
                "Caller Not Two Week Timelock"
            );
        } else {
            require(msg.sender == owner(), "Caller Not Owner");
        }
    ...
    function setAddresses(Addresses memory _addresses) external override {
        require(addressSet == false, "Addresses already set");
        addressSet = true;
        _transferOwnership(msg.sender);

        activePool = IActivePool(_addresses._activePoolAddress);
        defaultPool = IDefaultPool(_addresses._defaultPoolAddress);
        stabilityPool = IStabilityPool(_addresses._stabilityPoolAddress);
        collSurplusPool = ICollSurplusPool(_addresses._collSurplusPoolAddress);
        starToken = ISTARToken(_addresses._starTokenAddress);
        STARFeeRecipient = _addresses._STARFeeRecipientAddress; // vePreonEmissions address.
        borrowerOperationsAddress = _addresses._borrowerOperationsAddress;
        _requireAddressNonzero(_addresses._preonFinanceTreasury != address(0));
        preonFinanceTreasury = _addresses._preonFinanceTreasury;
        sortedTroves = ISortedTroves(_addresses._sortedTrovesAddress);
        vePREON = IVePREON(_addresses._vePREONAddress);
        troveManagerRedemptions = ITroveManagerRedemptions(
            _addresses._troveManagerRedemptionsAddress
        );
        _requireAddressNonzero(_addresses._claimAddress != address(0));
        claimAddress = _addresses._claimAddress;
        threeDayTimelock = _addresses._threeDayTimelock;
        twoWeekTimelock = _addresses._twoWeekTimelock;
        emit ClaimAddressChanged(_addresses._claimAddress);
        emit PreonFinanceTreasuryChanged(_addresses._preonFinanceTreasury);

        feeBootstrapPeriodEnabled = true;
        preonFinanceTreasurySplit = 1e18;
     ...
     * Preon in the event of a potential hack.
     */
    ...
    function setFeeBootstrapPeriodEnabled(bool _enabled)
        external
        override
        onlyOwner
    {
    ...
    function updateGlobalSTARMinting(bool _canMint)
        external
        override
        onlyOwner
    {
        starToken.updateMinting(_canMint);
        emit GlobalSTARMintOn(_canMint);
    }

    function removeValidSTARMinter(address _minter)
        external
        override
        onlyOwner
    {
        require(_minter != borrowerOperationsAddress);
        starToken.removeValidMinter(_minter);
        emit STARMinterChanged(_minter, false);
    }

    // remove vePREON caller
    function removeVePreonCaller(address _contractAddress)
        external
        override
        onlyOwner
    {
        // vePREON.updateWhitelistedCallers(_contractAddress, false);
        emit UpdateVePreonCallers(_contractAddress, false);
    }

    function updateRedemptionsEnabled(bool _enabled)
        external
        override
        onlyOwner
    {
    ...
    function addCollateral(
        ...
        _requireSplitOrRatioValid(_safetyRatio < 11e17); //=> greater than 1.1 would mean taking out more STAR than collateral VC
        _requireSplitOrRatioValid(_recoveryRatio >= _safetyRatio);
        require(
            validCollateral.length < maxSystemColls,
            "Already hit max system colls"
        );

        if (validCollateral.length != 0) {
            require(
                validCollateral[0] != _collateral &&
                    collateralParams[_collateral].index == 0,
    ...
    function changeRatios(
        ...
        _requireThreeDayTimelock();
        _requireSplitOrRatioValid(_newSafetyRatio < 11e17); //=> greater than 1.1 would mean taking out more STAR than collateral VC
    ...
    function changePreonFinanceTreasury(address _newTreasury) external override {
        _requireThreeDayTimelock();
        _requireAddressNonzero(_newTreasury != address(0));
        preonFinanceTreasury = _newTreasury;
        emit PreonFinanceTreasuryChanged(_newTreasury);
    }
    ...
    function changeSTARFeeRecipient(address _newFeeRecipient)
        external
        override
    {
        _requireThreeDayTimelock();
        _requireAddressNonzero(_newFeeRecipient != address(0));
        STARFeeRecipient = _newFeeRecipient;
        emit STARFeeRecipientChanged(_newFeeRecipient);
    }

    function changePreonFinanceTreasurySplit(uint256 _newSplit)
        external
        override
    {
        _requireThreeDayTimelock();
        ...
        preonFinanceTreasurySplit = _newSplit;
        emit PreonFinanceTreasurySplitChanged(_newSplit);
    }
    ...
    function addValidSTARMinter(address _minter) external override {
        _requireTwoWeekTimelock();
        _requireAddressNonzero(_minter != address(0));
        starToken.addValidMinter(_minter);
        emit STARMinterChanged(_minter, true);
    }

    function setMaxSTARMindable(uint256 _maxSTARMintable) external {
        _requireTwoWeekTimelock();
        starToken.setMaxSTARMintable(_maxSTARMintable);
    }

    // update max system collaterals
    function updateMaxSystemColls(uint _newMax) external override {
        _requireTwoWeekTimelock();
        require(_newMax > validCollateral.length, "invalid newMax");
    ...
    function _checkCollateralListSingle(address[] memory _colls, bool _deposit)
        internal
        view
    {
        uint256 len = _colls.length;
        ...
        returns (uint256)
    {
        IPriceFeed collateral_priceFeed = IPriceFeed(
            collateralParams[_collateral].oracle
        );
    ...
    function getValuesVC(
        address[] memory _collaterals,
        uint256[] memory _amounts
    ) external view override returns (uint256 VCValue) {
        uint256 tokensLen = _collaterals.length;
        for (uint256 i; i < tokensLen; ++i) {
     ...
     * of summing them like in getValuesVC.
     */
    function _getValuesVCIndividual(
        address[] memory _collaterals,
        uint256[] memory _amounts
    ) internal view returns (uint256[] memory VCValues) {
        uint256 tokensLen = _collaterals.length;
    ...
    function getValuesRVC(
        address[] memory _collaterals,
        uint256[] memory _amounts
    ) external view override returns (uint256 RVCValue) {
        uint256 tokensLen = _collaterals.length;
        for (uint256 i; i < tokensLen; ++i) {
    ...
    function getValuesVCAndRVC(
        address[] memory _collaterals,
        uint256[] memory _amounts
    ) external view override returns (uint256 VCValue, uint256 RVCValue) {
        uint256 tokensLen = _collaterals.length;
        for (uint256 i; i < tokensLen; ++i) {
    ...
    function getPreonFinanceTreasury() external view override returns (address) {
        return preonFinanceTreasury;
    }

    function getPreonFinanceTreasurySplit()
        external
        view
        override
        returns (uint256)
    {
        return preonFinanceTreasurySplit;
    }

    function getRedemptionBorrowerFeeSplit()
        external
        view
        override
        returns (uint256)
    ...
    function getSTARFeeRecipient() external view override returns (address) {
        return STARFeeRecipient;
    }
    ...
    /**
     * Returns the treasury split, treasury address, and the vePreonEmissions for reward distribution. This is for use of borrower
    ...
    {
        return (
            preonFinanceTreasurySplit,
            preonFinanceTreasury,
            STARFeeRecipient // vePreonEmissions.
        );
    }
     ...
     */
    function getClaimAddress() external view override returns (address) {
        return claimAddress;
    }
    ...
    function getVariableDepositFee(
        ...
        if (feeBootstrapPeriodEnabled) {
            return PreonMath._min(uncappedFee, 1e16); // cap at 1%
        } else {
            return uncappedFee;
        }
    }
     ...
     * @return STARFee the total variable fees for all tokens in this transaction
    ...
    function getTotalVariableDepositFeeAndUpdate(
        ...
        uint256 _VCin,
        uint256 _VCout
    ) external override returns (uint256 STARFee, uint256 boostFactor) {
        ...
        vars.systemCollateralVCs = _getValuesVCIndividual(
            _tokensIn,
            activePool.getAmountsSubsetSystem(_tokensIn)
        );
        for (uint256 i; i < tokensLen; ++i) {
            ...
            // (collateral VC In) * (Collateral's Fee Given Preon Protocol Backed by Given Collateral)
            ...
            if (feeBootstrapPeriodEnabled) {
                controllerFee = PreonMath._min(controllerFee, 1e16); // cap at 1%
            }
            vars.collateralSTARFee = vars
                .collateralInputVC
                .mul(controllerFee)
                .div(DECIMAL_PRECISION);

            // If lower than 1, then it was not leveraged (1x)
            uint256 thisLeverage = PreonMath._max(
                DECIMAL_PRECISION,
                _leverages[i]
            );

            uint256 collBoostFactor = vars
                .collateralSTARFee
                .mul(thisLeverage)
                .div(_VCin);
            boostFactor = boostFactor.add(collBoostFactor);

            STARFee = STARFee.add(vars.collateralSTARFee);
        }
    }
        ...
        returns (uint256)
    {
        ...
        return (
            (getPrice(_collateral))
                .mul(_amount)
                .mul(collateralParams[_collateral].safetyRatio)
                .div(10**(18 + collateralParams[_collateral].decimals))
        );
    }
    ...
    function _requireCollListSortedByIndex(bool _sorted)
        internal
        pure
        returns (uint256)
    {
    ...
    function _requireAddressNonzero(bool _nonzero)
        internal
        pure
        returns (address)
    {
    ...
    function _requireSplitOrRatioValid(bool _valid)
        internal
        pure
        returns (uint256)
    {
    ...
    function _requireInputLengthChange(bool _validLength)
        internal
        pure
        returns (uint256)
    {
