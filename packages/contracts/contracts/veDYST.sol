...
// contract VePreonNew is IVe, Reentrancy {
//     using SafeERC20 for IERC20;

//     uint internal constant WEEK = 1 weeks;
//     uint internal constant MAX_TIME = 4 * 1 weeks;
//     int128 internal constant I_MAX_TIME = 4 * 1 weeks;
...
//     mapping(address => Point[1000000000]) internal _userPointHistory;
...
//     string public constant name = "vePREON";
//     string public constant symbol = "vePREON";
...
//     function initialize(address _boostedFarm) public {
//         require(boostedFarm == address(0), "Already Initialized");
//         boostedFarm = _boostedFarm;
//     }

//     // TODO: Create governance controlled modifier
//     function changeUnderlying(address _newUnderlying) external {
//         token = _newUnderlying;
//     }
...
//         uint uEpoch = userPointEpoch[addr];
...
//     function userPointHistoryTs(address addr, uint _idx)
//         external
//         view
//         returns (uint)
//     {
//         return _userPointHistory[addr][_idx].ts;
//     }

//     // @notice Get timestamp when `_tokenId`'s lock finishes
//     // @param _tokenId User NFT
//     /// @return Epoch time of the lock end
//     function lockedEnd(address addr) external view returns (uint) {
...
//             uint userEpoch = userPointEpoch[addr] + 1;

//             userPointEpoch[addr] = userEpoch;
//             uNew.ts = block.timestamp;
//             uNew.blk = block.number;
//             _userPointHistory[addr][userEpoch] = uNew;
//         }
//     }
...
//         emit Deposit(from, _value, _locked.end, depositType, block.timestamp);
...
//     // function attachToken(uint _tokenId) external {
//     //     require(msg.sender == _voter(), "!voter");
//     //     attachments[_tokenId] = attachments[_tokenId] + 1;
//     // }

//     // function detachToken(uint _tokenId) external {
//     //     require(msg.sender == _voter(), "!voter");
//     //     attachments[_tokenId] = attachments[_tokenId] - 1;
//     // }
...
//     function depositFor(address addr, uint _value) external lock {
...
//         );
//         _depositFor(addr, _value, 0, _locked, DepositType.DEPOSIT_FOR_TYPE);
//     }
...
//     /// @param _value Amount to deposit
//     /// @param _lockDuration Number of seconds to lock tokens for (rounded down to nearest week)
//     function createLock(uint _value, uint _lockDuration) external lock {
//         return _createLock(_value, _lockDuration, msg.sender);
//     }

//     /// @notice Deposit `_value` additional tokens for `_tokenId` without modifying the unlock time
//     /// @param _value Amount of tokens to deposit and add to the lock
//     function increaseAmount(address addr, uint _value) external lock {
//         LockedBalance memory _locked = locked[addr];
//         require(_locked.amount > 0, "No existing lock found");
//         require(
//             _locked.end > block.timestamp,
//             "Cannot add to expired lock. Withdraw"
//         );
//         // require(_isApprovedOrOwner(msg.sender, addr), "!owner");
//         require(_value > 0, "zero value");
...
//     function increaseUnlockTime(address addr, uint _lockDuration)
//         external
//         lock
//     {
//         LockedBalance memory _locked = locked[addr];
...
//     function rageQuit() external lock {
//         // require(_isApprovedOrOwner(msg.sender, _tokenId), "!owner");
//         // require(attachments[_tokenId] == 0 && !voted[_tokenId], "attached");
//         LockedBalance memory _locked = locked[msg.sender];
//         require(block.timestamp >= _locked.end, "The lock did not expire");
...
//         // Both can have >= 0 amount
//         _checkpoint(msg.sender, _locked, LockedBalance(0, 0));
//         uint256 valueReturned = value / 2;
//         uint256 valueForStakers = value - valueReturned;

//         IERC20(token).safeTransfer(msg.sender, valueReturned);
//         IERC20(token).safeTransfer(
//             IPreonController(controller).getPreonFinanceTreasury(),
//             valueForStakers
//         );
...
//     function _balanceOfNFT(address addr, uint _t) internal view returns (uint) {
//         uint _epoch = userPointEpoch[addr];
//         if (_epoch == 0) {
//             return 0;
//         } else {
//             Point memory lastPoint = _userPointHistory[addr][_epoch];
//             lastPoint.bias -=
//                 lastPoint.slope *
...
//     function balanceOfNFT(address addr) external view returns (uint) {
//         // flash NFT protection
//         // if (ownershipChange[_tokenId] == block.number) {
//         //     return 0;
//         // }
//         return _balanceOfNFT(addr, block.timestamp);
//     }

//     function balanceOfNFTAt(address addr, uint _t)
//         external
//         view
//         returns (uint)
//     {
//         return _balanceOfNFT(addr, _t);
//     }
...
//     /// @return Voting power
//     function _balanceOfAtNFT(address addr, uint _block)
//         internal
//         view
//         returns (uint)
//     {
...
//             }
//             uint _mid = (_min + _max + 1) / 2;
//             if (_userPointHistory[addr][_mid].blk <= _block) {
//                 _min = _mid;
//             } else {
//                 _max = _mid - 1;
//             }
...
//         Point memory uPoint = _userPointHistory[addr][_min];
...
//     function balanceOfAtNFT(address addr, uint _block)
//         external
//         view
//         returns (uint)
//     {
//         return _balanceOfAtNFT(addr, _block);
//     }
...
//                         '{"name": "lock #',
//                         _toString(_tokenId),
//                         '", "description": "Dystopia locks, can be used to boost gauge yields, vote on token emission, and receive bribes", "image": "data:image/svg+xml;base64,',
...
//     function userPointHistory(address addr, uint _loc)
//         external
//         view
//         returns (Point memory)
//     {
//         return _userPointHistory[addr][_loc];
//     }

//     function pointHistory(uint _loc) external view returns (Point memory) {
