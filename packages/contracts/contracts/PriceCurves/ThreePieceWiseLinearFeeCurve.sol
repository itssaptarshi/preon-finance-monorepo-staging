...
contract ThreePieceWiseLinearFeeCurve is IFeeCurve, Ownable {
    ...
    function setAddresses(address _controllerAddress)
        external
        override
        onlyOwner
    {
    ...
    /**
     * Function for getting the fee for a particular collateral type based on percent of STAR backed
     * by this asset.
    ...
    ) external view override returns (uint256 feeCalculated) {
        feeCalculated = _getFee(
            _collateralVCInput,
            _totalCollateralVCBalance,
            _totalVCBalancePre,
            _totalVCBalancePost
        );
    }
    ...
    function getFeeAndUpdate(
        ...
        uint256 _totalVCBalancePre,
        uint256 _totalVCBalancePost
    ) external override returns (uint256 feeCalculated) {
        require(
            msg.sender == controllerAddress,
            "Only controller can update fee"
        );
        feeCalculated = _getFee(
            _collateralVCInput,
            _totalCollateralVCBalance,
    ...
    function _getFee(
        ...
        uint256 _totalVCBalancePre,
        uint256 _totalVCBalancePost
    ) internal view returns (uint256 feeCalculated) {
        ...
        if (cachedDollarCap != 0) {
            require(
                _totalCollateralVCBalance.add(_collateralVCInput) <=
                    cachedDollarCap,
                "Collateral input exceeds cap"
            );
        }

        uint feePre = _getFeePoint(
            _totalCollateralVCBalance,
            _totalVCBalancePre
        );
        uint feePost = _getFeePoint(
            _totalCollateralVCBalance.add(_collateralVCInput),
            _totalVCBalancePost
        );
        ...
        feeCalculated = _min(
            1e18,
            _max((feePre.add(feePost)).div(2), decayedLastFee)
        );
    }
     ...
     * by this asset.
     */
    function _getFeePoint(uint256 _collateralVCBalance, uint256 _totalVCBalance)
        internal
        view
        returns (uint256)
    {
        ...
        uint256 percentBacked = _collateralVCBalance.mul(1e18).div(
            _totalVCBalance
        );
    ...
    function calculateDecayedFee() public view override returns (uint256 fee) {
        ...
        if (timeSinceLastFee <= cachedDecayTime) {
            ...
            fee = lastFeePercent.sub(
                lastFeePercent.mul(timeSinceLastFee).div(cachedDecayTime)
            );
        } else {
