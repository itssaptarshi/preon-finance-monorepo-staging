 ...
 * redemptions, to be claimed by the trove owner. If they have been liquidated, their STAR debt has been paid back, and a max
...
contract CollSurplusPool is ICollSurplusPool, PoolBase {
    ...
    // STAR token balance from redemption bonus
     ...
     * @param _starTokenAddress address
     */
    function setAddresses(
        ...
        starToken = IERC20(_starTokenAddress);
    }
    ...
    function claimCollateral() external override {
        ...
        if (_collsIsNonZero(claimableColl)) {
            ...
            poolColl.amounts = _leftSubColls(
                poolColl,
                claimableColl.tokens,
                claimableColl.amounts
            );
    ...
    function accountSurplus(
        ...
        balances[_account] = _sumColls(
            balances[_account],
            newColls(_tokens, _amounts)
        );
    ...
    function _sendRedemptionBonus(address _to) internal {
        // Send STAR Redemption bonus if applicable
        uint256 thisRedemptionBonus = redemptionBonus[_to];
        if (thisRedemptionBonus != 0) {
            redemptionBonus[_to] = 0;
            totalRedemptionBonus = totalRedemptionBonus.sub(
                thisRedemptionBonus
            );
            starToken.safeTransfer(_to, thisRedemptionBonus);
     ...
     */
    function hasClaimableCollateral(
        address _account
     ...
     * @dev this is the total amonut of STAR in this contract that is
     * claimable by users who have been redeemed against.
     * @return uint256
     */
    function getTotalRedemptionBonus()
        external
        view
        override
        returns (uint256)
