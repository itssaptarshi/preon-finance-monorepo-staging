...
contract EchidnaTester {
    ...
    uint private MCR;
    uint private CCR;
    uint private STAR_GAS_COMPENSATION;
    ...
    constructor() public payable {
        ...
        STAR_GAS_COMPENSATION = borrowerOperations.STAR_GAS_COMPENSATION();
        require(MCR != 0, "MCR <= 0");
        require(CCR != 0, "CCR <= 0");
    ...
    //    ) external {
    //        uint actor = _i % NUMBER_OF_ACTORS;
    //        echidnaProxies[actor].redeemCollateralPrx(_STARAmount, _firstRedemptionHint, _upperPartialRedemptionHint, _lowerPartialRedemptionHint, _partialRedemptionHintNICR, 0, 0);
    //    }
    //
    //    // Borrower Operations
    //
    ...
    //        require(price != 0);
    //        uint minETH = ratio.mul(STAR_GAS_COMPENSATION).div(price);
    ...
    //    function getAdjustedSTAR(address[] memory _tokens, uint[] memory _amounts, uint _STARAmount, uint ratio) internal view returns (uint) {
    //        uint VC = _getVC(_tokens, _amounts);
    //        uint STARAmount = _STARAmount;
    //        uint compositeDebt = STARAmount.add(STAR_GAS_COMPENSATION);
    //        uint ICR = LiquityMath._computeCR(VC, compositeDebt);
    //        if (ICR < ratio) {
    //            compositeDebt = VC.div(ratio);
    //            STARAmount = compositeDebt.sub(STAR_GAS_COMPENSATION);
    //        }
    //        return STARAmount;
    //    }
    //
    //    // @KingPreon: changed parameters
    //    function openTroveExt(uint _i, address[] memory _tokens, uint[] memory _amounts, uint _STARAmount) public payable {
    //        uint actor = _i % NUMBER_OF_ACTORS;
    ...
    //        uint STARAmount = getAdjustedSTAR(_tokens, _amounts, _STARAmount, CCR);
    //
    //        //console.log('ETH', ETH);
    //        //console.log('STARAmount', STARAmount);
    //
    //        echidnaProxy.openTrovePrx(_tokens, _amounts, STARAmount, address(0), address(0), 0);
    //
    ...
    //    }
    //
    //    function openTroveRawExt(uint _i, uint _ETH, uint _STARAmount, address _upperHint, address _lowerHint, uint _maxFee) public payable {
    //        uint actor = _i % NUMBER_OF_ACTORS;
    //        echidnaProxies[actor].openTrovePrx(_ETH, _STARAmount, _upperHint, _lowerHint, _maxFee);
    //    }
    //
    ...
    //    }
    //
    //    function withdrawSTARExt(uint _i, uint _amount, address _upperHint, address _lowerHint, uint _maxFee) external {
    //        uint actor = _i % NUMBER_OF_ACTORS;
    //        echidnaProxies[actor].withdrawSTARPrx(_amount, _upperHint, _lowerHint, _maxFee);
    //    }
    //
    //    function repaySTARExt(uint _i, uint _amount, address _upperHint, address _lowerHint) external {
    //        uint actor = _i % NUMBER_OF_ACTORS;
    //        echidnaProxies[actor].repaySTARPrx(_amount, _upperHint, _lowerHint);
    //    }
    //
    ...
    //        if (_isDebtIncrease) {
    //            debtChange = getAdjustedSTAR(ETH, uint(_debtChange), MCR);
    //        }
    ...
    //
    //            // Minimum debt (gas compensation)
    //            if (troveManager.getTroveDebt(currentTrove) < STAR_GAS_COMPENSATION) {
    //                return false;
    //            }
    //            // Uncomment to check that the condition is meaningful
    //            //else return false;
    ...
    //        }
    //
    //        if (address(starToken).balance != 0) {
    //            return false;
    //        }
    //
    ...
    //
    //    // Total STAR matches
    //    function echidna_STAR_global_balances() public view returns(bool) {
    //        uint totalSupply = starToken.totalSupply();
    //        uint gasPoolBalance = starToken.balanceOf(address(gasPool));
    //
    //        uint activePoolBalance = activePool.getSTARDebt();
    //        uint defaultPoolBalance = defaultPool.getSTARDebt();
    //        if (totalSupply != activePoolBalance + defaultPoolBalance) {
    //            return false;
    //        }
    //
    //        uint stabilityPoolBalance = stabilityPool.getTotalSTARDeposits();
    //        address currentTrove = sortedTroves.getFirst();
    //        uint trovesBalance;
    //        while (currentTrove != address(0)) {
    //            trovesBalance += starToken.balanceOf(address(currentTrove));
    //            currentTrove = sortedTroves.getNext(currentTrove);
    //        }
