...
contract TroveManagerTester is TroveManager {
    ...
    function getSTARGasCompensation() external pure returns (uint) {
    ...
    function callGetRedemptionFee(uint _STARRedeemed)
        external
        view
        returns (uint)
    {
        _getRedemptionFee(_STARRedeemed);
    }

    function getActualDebtFromComposite(uint _debtVal)
        external
        pure
        returns (uint)
    {
    ...
    {
        (newColls memory colls, uint STARdebt) = _getCurrentTroveState(
            _troveAddress
        );
    ...
    function getCollGasCompensation(address _token, uint _amount)
        external
        pure
        returns (uint)
    {
    ...
    function getUSD(address[] memory _tokens, uint[] memory _amounts)
        external
        view
        returns (uint)
    {
