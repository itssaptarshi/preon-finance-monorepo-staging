...
contract WJLPRouter is IPreonRouter {
    ...
    function route(
        ...
        require(
            _endingTokenAddress == WJLPAddress,
            "Ending token address must be WJLP"
        );
        ...
        // Other ERC20 -> JLP -> WJLP then send to active pool
            ...
            uint256 initial_balance = IERC20(JLPAddress).balanceOf(
                address(this)
            );
            ...
            require(
                differenceJLP >= _minSwapAmount,
                "Zap did not produce enough JLP"
            );
    ...
    function _zapInToken(address _startingTokenAddress, uint256 _amount)
        internal
    {
        // _to is the resultJLP
