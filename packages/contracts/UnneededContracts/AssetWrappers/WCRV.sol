...
contract WCRV is IWAsset, ERC20_8 {
    ...
    constructor(
        ...
        uint256 _indexOfCoinToDepositInCurvePool // uint256 poolPid
    ) {
        ...
        require(
            crvDepIndex < _numberOfCoinsInCurvePool,
            "Index of coin to deposit in curve pool is out of bounds"
        );
        crvDepIndex = _indexOfCoinToDepositInCurvePool;
        require(
            swapOutput == POOL.underlying_coins(crvDepIndex),
            "Swap output address does not match the address of the coin to deposit in the curve pool"
        );
        ...
        for (uint i = 0; i < _rewardTokens.length; i++) {
            require(
                _rewardTokens[i].swapPath[
                    _rewardTokens[i].swapPath.length - 1
                ] == swapOutput,
                "Swap output must be last element in swap path"
            );
    ...
    function _setRewardToken(
        ...
        require(
            _path.length == 0 || _path[_path.length - 1] == swapOutput,
            "Swap output must be last element in swap path or path must be empty"
        );
    ...
    function compound() public {
        ...
        for (uint i = 0; i < nLen; i++) {
            ...
            if (
                info.swapPath.length == 0 ||
                IERC20(info.token).balanceOf(address(this)) < info.minAmt
            ) {
                continue;
            }
            swapTraderJoe(info);
    ...
    function swapTraderJoe(rewardToken memory _rewardToken) internal {
        ...
        if (_rewardToken.token != address(0)) {
            uint256 balance = IERC20(_rewardToken.token).balanceOf(
                address(this)
            );
            IJoeRouter01(traderJoe).swapExactTokensForTokens(
                balance,
                0,
                _rewardToken.swapPath,
                address(this),
                block.timestamp + 30
            );
        } else {
            uint256 balance = address(this).balance;
            IJoeRouter01(traderJoe).swapExactAVAXForTokens{value: balance}(
                0,
                _rewardToken.swapPath,
                address(this),
                block.timestamp + 30
    ...
    function _requireCallerIsAPorSP() internal view {
        require(
            (msg.sender == activePool || msg.sender == stabilityPool),
