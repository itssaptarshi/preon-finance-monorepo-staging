...
contract WJLP is ERC20_8, IWAsset {
    ...
    function wrap(
        ...
        if (msg.sender != borrowerOperations) {
            ...
            require(
                msg.sender == _from,
                "WJLP: msg.sender and _from must be the same"
            );
        }
        ...
        returns (address[] memory, uint[] memory)
    {
        ...
        uint256 accJoePerShare = _MasterChefJoe
            .poolInfo(_poolPid)
            .accJoePerShare;
        UserInfo storage user = userInfo[_for];

        uint unclaimed = user.unclaimedJOEReward;
        uint pending = ((user.amount * accJoePerShare) / 1e12) -
            user.rewardDebt;
    ...
    function _userUpdate(
        ...
        if (cachedUserAmount != 0) {
            user.unclaimedJOEReward =
                ((cachedUserAmount * accJoePerShare) / 1e12) -
                user.rewardDebt;
        }
