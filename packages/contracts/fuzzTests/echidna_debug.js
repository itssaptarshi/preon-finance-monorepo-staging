...
contract("Echidna debugger", async (accounts) => {
  ...
  before(async () => {
    ...
    starToken = await STARToken.at(await echidnaTester.starToken());
  ...
  it("STAR balance", async () => {
    ...
    const gasPoolBalance = await starToken.balanceOf(GAS_POOL_ADDRESS);
    const activePoolBalance = await activePool.getSTARDebt();
    const defaultPoolBalance = await defaultPool.getSTARDebt();
    const stabilityPoolBalance = await stabilityPool.getTotalSTARDeposits();
    const currentTrove = await echidnaTester.echidnaProxies(0);
    const troveBalance = starToken.balanceOf(currentTrove);
