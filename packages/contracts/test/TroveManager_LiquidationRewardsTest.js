...
contract(
  ...
  async (accounts) => {
    ...
    it("redistribution: A, B Open. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards", async () => {
      ...
      console.log(
        "aliceVC: ",
        (
          await contracts.troveManager.getVC(aliceCTS[0], aliceCTS[1])
        ).toString()
      );
    ...
    */
    ...
    it("redistribution: Sequence of alternate opening/liquidation: final surviving trove has ETH from all previously liquidated troves", async () => {
      ...
      // console.log('debt in active pool: ', (await activePool.getSTARDebt()).toString())
      ...
      // console.log('debt in default pool: ', (await defaultPool.getSTARDebt()).toString())
      // console.log('TCR: ', (await contracts.troveManager.getTCR()).toString())
      ...
      console.log(
        "carolVC: ",
        (
          await contracts.troveManager.getVC(carolCTS[0], carolCTS[1])
        ).toString()
      );
      ...
      bobCTS = await contracts.troveManager.getEDC(bob);
      const bob_Coll = (
        await contracts.troveManager.getVC(bobCTS[0], bobCTS[1])
      ).toString();
      ...
      const dennis_Coll = (
        await contracts.troveManager.getVC(dennisCTS[0], dennisCTS[1])
      ).toString();

      erinCTS = await contracts.troveManager.getEDC(erin);
      const erin_Coll = (
        await contracts.troveManager.getVC(erinCTS[0], erinCTS[1])
      ).toString();
      ...
      const freddy_rawColl = (await troveManager.getTroveColls(freddy))[1][
        wethIDX
      ].toString();
      const freddy_ETHReward = (
        await troveManager.getPendingCollRewards(freddy)
      )[1][wethIDX].toString();
    ...
    */
      ...
      // check STAR gas compensation
      assert.equal(
        (await starToken.balanceOf(owner)).toString(),
        dec(1000, 18)
      );
    });
    ...
    it("redistribution: A,B,C,D open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
      ...
      console.log(
        "bobICR: ",
        (await contracts.troveManager.getCurrentICR(A)).toString()
      );
      console.log(
        "bobVC: ",
        (await contracts.troveManager.getVC(bobCTS[0], bobCTS[1])).toString()
      ...
      const denominatorColl_1 = (await weth.balanceOf(activePool.address))
        .add(await weth.balanceOf(defaultPool.address))
        .sub(A_entireColl_0);
      ...
      const B_ETHGain_1 = (await troveManager.getPendingCollRewards(B))[1][
        wethIDX
      ];
      const C_ETHGain_1 = (await troveManager.getPendingCollRewards(C))[1][
        wethIDX
      ];
      const D_ETHGain_1 = (await troveManager.getPendingCollRewards(D))[1][
        wethIDX
      ];
      const E_ETHGain_1 = (await troveManager.getPendingCollRewards(E))[1][
        wethIDX
      ];

      // Check gains are what we'd expect from a distribution proportional to each trove's entire coll
      const B_expectedPendingETH_1 = A_collRedistribution.mul(
        B_entireColl_0
      ).div(denominatorColl_1);
      const C_expectedPendingETH_1 = A_collRedistribution.mul(
        C_entireColl_0
      ).div(denominatorColl_1);
      const D_expectedPendingETH_1 = A_collRedistribution.mul(
        D_entireColl_0
      ).div(denominatorColl_1);
      const E_expectedPendingETH_1 = A_collRedistribution.mul(
        E_entireColl_0
      ).div(denominatorColl_1);
      ...
      await weth.approve(borrowerOperations.address, dec(1, "ether"), {
        from: B,
      });
      await borrowerOperations.addColl(
        [weth.address],
      ...
      const denominatorColl_2 = (await weth.balanceOf(activePool.address))
        .add(await weth.balanceOf(defaultPool.address))
        .sub(C_entireColl_1);
      ...
      const B_ETHGain_2 = (await troveManager.getPendingCollRewards(B))[1][
        wethIDX
      ];
      const D_ETHGain_2 = (await troveManager.getPendingCollRewards(D))[1][
        wethIDX
      ];
      const E_ETHGain_2 = (await troveManager.getPendingCollRewards(E))[1][
        wethIDX
      ];

      // Since B topped up, he has no previous pending ETH gain
      const B_expectedPendingETH_2 = C_collRedistribution.mul(
        B_entireColl_1
      ...
      const denominatorColl_3 = (await weth.balanceOf(activePool.address))
        .add(await weth.balanceOf(defaultPool.address))
        .sub(E_entireColl_2);
      ...
      const B_ETHGain_3 = (await troveManager.getPendingCollRewards(B))[1][
        wethIDX
      ];
      const D_ETHGain_3 = (await troveManager.getPendingCollRewards(D))[1][
        wethIDX
      ];

      // Since B topped up, he has no previous pending ETH gain
      const B_expectedPendingETH_3 = E_collRedistribution.mul(
        B_entireColl_2
    ...
    it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
      ...
      // Expect Bob now holds all Ether and STARDebt in the system: 2 + 0.4975+0.4975*0.995+0.995 Ether and 110*3 STAR (10 each for gas compensation)
      const bob_Coll = (await troveManager.getTroveColls(bob))[1][wethIDX]
        .add((await troveManager.getPendingCollRewards(bob))[1][wethIDX])
        .toString();

      const bob_STARDebt = (
        await th.getEntireCollAndDebt(contracts, bob)
      ).entireDebt.toString();
    ...
    it("redistribution: A,B,C Open. Liq(C). B tops up coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
      ...
      /* Bob rewards:
     L1: 1/2*0.995 ETH, 55 STAR
     L2: (2.4975/3.995)*0.995 = 0.622 ETH , 110*(2.4975/3.995)= 68.77 STARDebt

    coll: 3.1195 ETH
    debt: 233.77 STARDebt

     Alice rewards:
    L1 1/2*0.995 ETH, 55 STAR
    L2 (1.4975/3.995)*0.995 = 0.3730 ETH, 110*(1.4975/3.995) = 41.23 STARDebt

    coll: 1.8705 ETH
    debt: 146.23 STARDebt

    totalColl: 4.99 ETH
    totalDebt 380 STAR (includes 50 each for gas compensation)
    */
      const bob_Coll = (await troveManager.getTroveColls(bob))[1][wethIDX]
        .add((await troveManager.getPendingCollRewards(bob))[1][wethIDX])
        .toString();
      ...
      const alice_STARDebt = (
        await th.getEntireCollAndDebt(contracts, alice)
      ).entireDebt.toString();
      ...
      assert.isAtMost(th.getDifference(bob_Coll, expected_B_coll), 1000);
      assert.isAtMost(th.getDifference(bob_STARDebt, expected_B_debt), 10000);
      ...
      assert.isAtMost(th.getDifference(alice_STARDebt, expected_A_debt), 10000);
    ...
    it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). C tops up. E Enters, Liq(E). Distributes correct rewards", async () => {
      ...
      await weth.approve(borrowerOperations.address, C_addedColl, {
        from: carol,
      });
      await borrowerOperations.addColl(
        [weth.address],
    ...
    */
      ...
      const entireSystemColl_3 = (await activePool.getCollateral(weth.address))
        .add(await defaultPool.getCollateral(weth.address))
        .toString();
      th.assertIsApproximatelyEqual(
        entireSystemColl_3,
    ...
    bringing them to 2.995, 2.995, 1992.01 total coll each. */
      ...
      await weth.approve(borrowerOperations.address, addedColl, {
        from: alice,
      });
      await borrowerOperations.addColl(
        [weth.address],
      ...
      const entireSystemColl_2 = (await activePool.getCollateral(weth.address))
        .add(await defaultPool.getCollateral(weth.address))
        .toString();
      th.assertIsApproximatelyEqual(
        entireSystemColl_2,
    ...
    */
    ...
    it("redistribution: A,B,C Open. Liq(C). B withdraws coll. Liq(A). B acquires all coll and debt", async () => {
      ...
      // Expect Bob now holds all Ether and STARDebt in the system: 2.5 Ether and 300 STAR
    ...
    it("redistribution: A,B,C Open. Liq(C). B withdraws coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
      ...
      /* Bob rewards:
     L1: 0.4975 ETH, 55 STAR
     L2: (0.9975/2.495)*0.995 = 0.3978 ETH , 110*(0.9975/2.495)= 43.98 STARDebt

    coll: (1 + 0.4975 - 0.5 + 0.3968) = 1.3953 ETH
    debt: (110 + 55 + 43.98 = 208.98 STARDebt

     Alice rewards:
    L1 0.4975, 55 STAR
    L2 (1.4975/2.495)*0.995 = 0.5972 ETH, 110*(1.4975/2.495) = 66.022 STARDebt

    coll: (1 + 0.4975 + 0.5972) = 2.0947 ETH
    debt: (50 + 55 + 66.022) = 171.022 STAR Debt

    totalColl: 3.49 ETH
    totalDebt 380 STAR (Includes 50 in each trove for gas compensation)
    */
      const bob_Coll = (await troveManager.getTroveColls(bob))[1][wethIDX]
        .add((await troveManager.getPendingCollRewards(bob))[1][wethIDX])
        .toString();
      ...
      const entireSystemDebt = (await activePool.getSTARDebt()).add(
    ...
    */
    ...
    it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards", async () => {
      ...
      assert.isAtMost(
        th.getDifference(
          (await troveManager.getPendingCollRewards(bob))[1][wethIDX],
      ...
      assert.isAtMost(
        th.getDifference(
          (await troveManager.getPendingCollRewards(carol))[1][wethIDX],
      ...
      await weth.approve(borrowerOperations.address, B_addedColl, {
        from: bob,
      });
      await borrowerOperations.addColl(
        [weth.address],
      ...
      assert.isAtMost(
        th.getDifference(
          (await troveManager.getPendingCollRewards(dennis))[1][wethIDX],
      ...
      await weth.approve(borrowerOperations.address, D_addedColl, {
        from: dennis,
      });
      await borrowerOperations.addColl(
        [weth.address],
      ...
      const carol_rawColl = (await troveManager.getTroveColls(carol))[1][
        wethIDX
      ].toString();
      ...
      const dennis_rawColl = (await troveManager.getTroveColls(dennis))[1][
        wethIDX
      ].toString();
      const dennis_pendingETHReward = (
        await troveManager.getPendingCollRewards(dennis)
      )[1][wethIDX].toString();

      const erin_rawColl = (
        await troveManager.getTroveColls(erin)
      )[1].toString();
      const erin_pendingETHReward = (
        await troveManager.getPendingCollRewards(erin)
      )[1][wethIDX].toString();
      ...
      // check STAR gas compensation
      assert.equal((await starToken.balanceOf(owner)).toString(), dec(600, 18));
    });
    ...
    */
    ...
    it("multi redistribution: A, B Open. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards", async () => {
      ...
      const { amounts: A_amounts } = await th.openTroveWithCollsFixedSTAR(
        contracts,
        {
          ICR: toBN(dec(400, 16)),
          colls: a_colls,
      ...
      const { amounts: B_amounts } = await th.openTroveWithCollsFixedSTAR(
        contracts,
        {
          ICR: toBN(dec(210, 16)),
          colls: b_colls,
      ...
      console.log(
        "total amount wavax in active pool",
        (await contracts.wavax.balanceOf(activePool.address)).toString()
      );
      console.log(
        "debt in active pool: ",
        (await activePool.getSTARDebt()).toString()
      ...
      console.log(
        "total amount wavax in default pool",
        (await contracts.wavax.balanceOf(defaultPool.address)).toString()
      );
      console.log(
        "debt in default pool: ",
        (await defaultPool.getSTARDebt()).toString()
      ...
      console.log(
        "entire system avax amount: ",
        (await contracts.wavax.balanceOf(activePool.address))
          .add(await contracts.wavax.balanceOf(defaultPool.address))
          .toString()
      );
      console.log(
        "entire system debt: ",
      ...
      const { amounts: C_amounts } = await th.openTroveWithCollsFixedSTAR(
        contracts,
        {
          ICR: toBN(dec(400, 16)),
          colls: a_colls,
      ...
      const { amounts: D_amounts } = await th.openTroveWithCollsFixedSTAR(
        contracts,
        {
          ICR: toBN(dec(210, 16)),
          colls: b_colls,
      ...
      // Total coll = 4 + 2 * 0.995 ETH
      // */
      const A_ETHAfterL1 = A_amounts[0].add(
        th.applyLiquidationFee(B_amounts[0])
      );
      assert.isAtMost(
        th.getDifference(
          alice_ETH,
          A_ETHAfterL1.add(
            A_ETHAfterL1.mul(th.applyLiquidationFee(D_amounts[0])).div(
              A_ETHAfterL1.add(C_amounts[0])
            )
          )
        ),
        Number(dec(150, 20))
      ...
      assert.isAtMost(
        th.getDifference(
          ...
          C_amounts[0].add(
            C_amounts[0]
              .mul(th.applyLiquidationFee(D_amounts[0]))
              .div(A_ETHAfterL1.add(C_amounts[0]))
          )
        ),
        Number(dec(100, 20))
      );
      ...
      const entireSystemETH = (await activePool.getCollateral(wavax.address))
        .add(await defaultPool.getCollateral(weth.address))
        .toString();
      assert.equal(
        entireSystemETH,
        A_amounts[0]
          .add(C_amounts[0])
          .add(th.applyLiquidationFee(B_amounts[0].add(D_amounts[0])))
      );
      ...
      // Total coll = 4 + 2 * 0.995 ETH
      // */
      const A_AVAXAfterL1 = A_amounts[1].add(
        th.applyLiquidationFee(B_amounts[1])
      );
      assert.isAtMost(
        th.getDifference(
          alice_AVAX,
          A_AVAXAfterL1.add(
            A_AVAXAfterL1.mul(th.applyLiquidationFee(D_amounts[1])).div(
              A_AVAXAfterL1.add(C_amounts[1])
            )
          )
        ),
        Number(dec(150, 20))
      ...
      assert.isAtMost(
        th.getDifference(
          ...
          C_amounts[1].add(
            C_amounts[0]
              .mul(th.applyLiquidationFee(D_amounts[1]))
              .div(A_AVAXAfterL1.add(C_amounts[1]))
          )
        ),
        Number(dec(100, 20))
      );
      ...
      const entireSystemAVAX = (await activePool.getCollateral(wavax.address))
        .add(await defaultPool.getCollateral(wavax.address))
        .toString();
      assert.equal(
        entireSystemAVAX,
        A_amounts[1]
          .add(C_amounts[1])
          .add(th.applyLiquidationFee(B_amounts[1].add(D_amounts[1])))
      );
    ...
    it("multi redistribution: A open with ETH, B open with AVAX, C open with both, liquidate C", async () => {
      ...
      const { amounts: A_amounts } = await th.openTroveWithCollsFixedSTAR(
        contracts,
        {
          ...
          colls: a_colls,
          amounts: a_amounts,
          oracles: [contracts.priceFeedETH],
          from: alice,
        }
      );
      ...
      const { amounts: B_amounts } = await th.openTroveWithCollsFixedSTAR(
        contracts,
        {
          ...
          colls: b_colls,
          amounts: b_amounts,
          oracles: [contracts.priceFeedAVAX],
          from: bob,
        }
      );
      ...
      const { amounts: C_amounts } = await th.openTroveWithCollsFixedSTAR(
        contracts,
        {
          ...
          colls: c_colls,
          amounts: c_amounts,
          oracles: [contracts.priceFeedETH, contracts.priceFeedAVAX],
          from: carol,
        }
      );
  ...
  Total coll = 4 + 2 * 0.995 ETH
  */
      const A_ETHAfterL1 = A_amounts[wethIDX].add(
        th.applyLiquidationFee(C_amounts[wethIDX])
      );
      assert.isAtMost(
        th.getDifference(alice_ETH, A_ETHAfterL1),
        Number(dec(150, 20))
      );

      const entireSystemETH = (await activePool.getCollateral(weth.address))
        .add(await defaultPool.getCollateral(weth.address))
        .toString();
      assert.equal(
        entireSystemETH,
        A_amounts[wethIDX].add(th.applyLiquidationFee(C_amounts[wethIDX]))
      );

      const B_AVAXAfterL1 = B_amounts[0].add(
        th.applyLiquidationFee(C_amounts[1])
      );
      assert.isAtMost(
        th.getDifference(bob_AVAX, B_AVAXAfterL1),
        Number(dec(150, 20))
      );
      ...
      assert.equal(
        entireSystemAVAX,
        A_amounts[0].add(th.applyLiquidationFee(C_amounts[wavaxIDX]))
      );

      assert.equal((await starToken.balanceOf(owner)).toString(), dec(200, 18));
    });
