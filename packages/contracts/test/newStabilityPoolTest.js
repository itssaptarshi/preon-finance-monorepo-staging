...
contract("StabilityPool", async (accounts) => {
  ...
  describe("Stability Pool Mechanisms", async () => {
    ...
    beforeEach(async () => {
      ...
      result = await deploymentHelper.deployExtraCollateral(
        contracts,
        paramsRisky
      );
    ...
    describe("Single Collateral, Normal Mode, one SP depositor liquidations, full offset", async () => {
       ...
       * while taking on 2000 of extraSTAR debt.
       * Then bob mints 500k weth and deposits it into a trove while borrowing 2000 STAR.
      ...
      it("provideToSP(): Liquidation single collateral low ratio full offset", async () => {
        ...
        await th.oneCollLiquidationTest(
          contracts,
          aliceTokenParams,
          collsAlice,
          amountsAlice,
          postPriceChangeICRAlice,
          bobTokenParams,
          collsBob,
          amountsBob,
          bob_SP_Deposit,
    ...
    describe("Single Collateral Normal Mode, one SP depositor liquidations, partial offset", async () => {
       ...
       * while taking on 2000 of extraSTAR debt.
       * Then bob mints 500k weth and deposits it into a trove while borrowing 2000 STAR.
       * Bob only deposits 500 STAR into SP
       * Then Alice gets liquidated
       */
    ...
    describe("Multi Collateral, Normal Mode, one SP depositor liquidations, full offset", async () => {
      /* Alice mints 1000 WAVAX and 1000 WETH, then deposits both into a trove will taking at 1000 STAR debt.
       * Then bob mints 500k weth and deposits it into a trove while borrowing STAR.
       * Then bob deposits 5000 STAR into stability pool. Then Alice gets liquidated.
       * Checks that her collateral is transferred into the
       * stability pool and that bob can withdraw it successfully
       * TODO: error on Bob's received STAR is quite large
       */
      it("provideToSP(): Liquidation multicollateral full offset ratio = 1, one SP depositor", async () => {
        ...
        await th.openTroveWithColls(contracts, {
          colls: collsBob,
          amounts: amountsBob,
          extraSTARAmount: SP_Deposit_Bob,
          from: bob,
        });
        ...
        // provideToSP()
        await stabilityPool.provideToSP(SP_Deposit_Bob, ZERO_ADDRESS, {
          from: bob,
        });
        ...
        assert.equal(
          stabilityPool_STAR_After.toString(),
          SP_Deposit_Bob.toString()
        );
        ...
        assert.equal(
          aliceTroveWETH.mul(toBN(995)).div(toBN(1000)).toString(),
          SP_WETH.toString()
        );
        assert.equal(
          aliceTroveWAVAX.mul(toBN(995)).div(toBN(1000)).toString(),
          SP_WAVAX.toString()
        );
        ...
        assert.equal(
          aliceTroveWETH.mul(toBN(5)).div(toBN(1000)).toString(),
          dave_WETH.toString()
        );
        assert.equal(
          aliceTroveWAVAX.mul(toBN(5)).div(toBN(1000)).toString(),
          dave_WAVAX.toString()
        );
        ...
        const bob_pre_STAR = await contracts.starToken.balanceOf(bob);
        console.log("Bob Pre STAR", bob_pre_STAR.toString());

        await contracts.stabilityPool.withdrawFromSP(SP_Deposit_Bob, {
          from: bob,
        });

        const bob_post_WETH = await contracts.weth.balanceOf(bob);
        const bob_post_WAVAX = await contracts.wavax.balanceOf(bob);
        const bob_post_STAR = await contracts.starToken.balanceOf(bob);
        console.log("Bob Post STAR", bob_post_STAR.toString());

        const bob_WETH_gain = bob_post_WETH - bob_pre_WETH;
        const bob_WAVAX_gain = bob_post_WAVAX - bob_pre_WAVAX;
        const bob_STAR_gain_fromWithdrawal = bob_post_STAR.sub(bob_pre_STAR);

        const bob_expected_STAR_gain_fromWithdrawal = SP_Deposit_Bob.sub(
          aliceTotalTroveDebt
        );

        console.log(
          "expected STAR GAIN BOB",
          bob_expected_STAR_gain_fromWithdrawal.toString()
        );
        console.log(
          "Actual STAR GAIN BOB",
          bob_STAR_gain_fromWithdrawal.toString()
        );

        th.assertIsApproximatelyEqual(
          bob_STAR_gain_fromWithdrawal,
          bob_expected_STAR_gain_fromWithdrawal,
          10000000
        );
        ...
        const aliceWETHPendingRewards = await contracts.collSurplusPool.getAmountClaimable(
          alice,
          contracts.weth.address
        );
        const aliceWAVAXPendingRewards = await contracts.collSurplusPool.getAmountClaimable(
          alice,
      ...
      it("provideToSP(): Liquidation multicollateral full offset ratio = 1, one SP depositor", async () => {
        ...
        const collsAlice = [
          aliceTokenParams[0].token,
          aliceTokenParams[1].token,
        ];
     ...
     * Then Bob mints 500k weth and deposits it into a trove while borrowing 2000 STAR.
     * Bob deposits 200 e-18 STAR into stability pool.
    ...
    xit("provideToSP(): alice is liquidated with ICR < 100% so all colls/debt redistributed", async () => {
      ...
      const aliceLiquidatedWAVAX = alicePreLiquidateTroveWAVAX
        .mul(995)
        .div(1000);

      const bobExpectedWETH = bobPreLiquidateTroveWETH.add(aliceLiquidatedWETH);
      const bobExpectedWAVAX = bobPreLiquidateTroveWAVAX.add(
        aliceLiquidatedWAVAX
      );
    ...
    it("provideToSP(): Liquidation multicollateral full offset ratio = 1, 0.5", async () => {
      ...
      await th.openTroveWithCollsOld(contracts, {
        ...
        extraSTARAmount: toBN(dec(2000, 18)),
        from: alice,
        oracles: [priceFeedETH, priceFeedRisky],
      });
      ...
      await th.openTroveWithColls(contracts, {
        ...
        colls: collsBob,
        amounts: amountsBob,
        extraSTARAmount: bob_SP_Deposit,
        from: bob,
        oracles: [priceFeedETH],
      });

      // --- TEST ---

      // provideToSP()
      await stabilityPool.provideToSP(bob_SP_Deposit, ZERO_ADDRESS, {
        from: bob,
      });
      ...
      const aliceTroveUSD = await troveManager.getUSD(
        aliceColls[0],
        aliceColls[1]
      );
      ...
      assert.equal(
        SP_WETH.toString(),
        aliceWETH_InTrove.mul(toBN(995)).div(toBN(1000)).toString()
      );
      assert.equal(
        SP_Risky.toString(),
        aliceRisky_InTrove.mul(toBN(995)).div(toBN(1000)).toString()
      );
      //
      // for (let i = 0; i < stabilityPoolAssets[0].length; i++) {
      //   // checks that no collateral has transferred to the stability pool
    ...
    it("provideToSP(): alice is liquidated-enough in stability pool for full offset-one SP depositor", async () => {
      ...
      await th.openTroveWithColls(contracts, {
        ...
        colls: colls,
        amounts: amounts,
        extraSTARAmount: aliceExtraTroveDebt,
        from: alice,
        oracles: [priceFeedETH, priceFeedAVAX],
      });
      ...
      console.log(
        "Alice star debt",
        (await getTroveEntireDebt(alice)).toString()
      );
      ...
      assert.equal(
        aliceData[2][0].mul(toBN(995)).div(toBN(1000)).toString(),
        SP_WETH.toString()
      );
      assert.equal(
        aliceData[2][1].mul(toBN(995)).div(toBN(1000)).toString(),
        SP_WAVAX.toString()
      );
      ...
      const dave_WAVAX = await contracts.wavax.balanceOf(dave);
      assert.equal(
        aliceData[2][0].mul(toBN(5)).div(toBN(1000)).toString(),
        dave_WETH.toString()
      );
      assert.equal(
        aliceData[2][1].mul(toBN(5)).div(toBN(1000)).toString(),
        dave_WAVAX.toString()
      );
      ...
      const bob_WETH_gain = bob_post_WETH - bob_pre_WETH;
      const bob_WAVAX_gain = bob_post_WAVAX - bob_pre_WAVAX;
      const bob_STAR_gain_fromWithdrawal = bob_post_STAR - bob_pre_STAR;
      const bob_expected_STAR_gain_fromWithdrawal =
        SP_Deposit_Bob - aliceTotalTroveDebt;
      const bob_STAR_error = toBN(
        Math.abs(
          bob_STAR_gain_fromWithdrawal - bob_expected_STAR_gain_fromWithdrawal
        )
      );
      ...
      // TODO: the received STAR amount is slightly less than expected. Confirm why
      //assert.isTrue(bob_STAR_error < toBN(400000));
    ...
    it("provideToSP(): Liquidation multicollateral full offset ratio = 1, 0.5, one SP depositor", async () => {
      ...
      await th.openTroveWithCollsOld(contracts, {
        ...
        extraSTARAmount: aliceExtraTroveDebt,
        from: alice,
        oracles: [priceFeedETH, priceFeedSuperRisky],
      });
      ...
      const [
        ...
        liquidatedColl,
        gasComp,
      ] = th.getEmittedLiquidationValues(tx1, wethIDX);
      const [
        liquidatedDebt2,
        liquidatedColl2,
        gasComp2,
      ] = th.getEmittedLiquidationValues(tx1, 1);
      ...
      const wethCollSurplusPool = await contracts.collSurplusPool.getCollateral(
        contracts.weth.address
      );
      const riskyCollSurplusPool = await contracts.collSurplusPool.getCollateral(
        tokenSuperRisky.address
      );
      ...
      th.assertIsApproximatelyEqual(
        aliceData[2][0].mul(toBN(995)).div(toBN(1000)),
        SP_WETH.add(wethCollSurplusPool)
      );
      th.assertIsApproximatelyEqual(
        aliceData[2][1].mul(toBN(995)).div(toBN(1000)),
        SP_SR.add(riskyCollSurplusPool)
      );
    ...
    it("provideToSP(): alice is liquidated with low decimal and normal asset--full offset-one SP depositor", async () => {
      ...
      await th.openTroveWithCollsOld(contracts, {
        ...
        extraSTARAmount: aliceExtraTroveDebt,
        from: alice,
        oracles: [priceFeedETH, priceFeedLowDecimal],
        includeOne: false,
      });
      ...
      //   colls: collsBob, amounts: amountsBob, extraSTARAmount: SP_Deposit_Bob, from: bob, oracles: [priceFeedETH, priceFeedLowDecimal] })
      ...
      const lowDecimalCollSurplusPool = await contracts.collSurplusPool.getCollateral(
        tokenLowDecimal.address
      );
      console.log("stability pool weth", SP_WETH.toString());
      console.log("stability pool low decimal", SP_LD.toString());
      console.log("wethCollSurplusPool", wethCollSurplusPool.toString());
      console.log(
        "lowDecimalCollSurplusPool",
        lowDecimalCollSurplusPool.toString()
      );
      ...
      const aliceLDPendingRewards = await contracts.collSurplusPool.getAmountClaimable(
        alice,
        tokenLowDecimal.address
      );
    ...
    it("provideToSP(): alice is liquidated with low decimal and normal asset--enough in stability pool for partial offset-one SP depositor", async () => {
      ...
      // assert 2440.2375 STAR entered stability pool: min(USD value of Coll not redistributed, 1.1 * debt offseted by stability pool) = 2440.2375
      console.log(
        SP_WETH.mul(toBN(109))
          .add(SP_LD.mul(toBN(109)).mul(toBN(dec(1, 12))))
          .toString()
      );
      console.log(toBN(dec(24402375, 14)).toString());

      th.assertIsApproximatelyEqual(
        toBN(dec(24402375, 14)),
        SP_WETH.mul(toBN(109)).add(SP_LD.mul(toBN(109)).mul(toBN(dec(1, 12)))),
        10000
      );

      // assert 0 STAR entered coll surplus pool: offsetting collat - collat sent to stability pool = coll surplus --> 2440.2375 - 2440.2375 = 0
      th.assertIsApproximatelyEqual(
        toBN(0),
        wethCollSurplusPool
          .mul(toBN(109))
          .add(lowDecimalCollSurplusPool.mul(toBN(109)).mul(toBN(dec(1, 12))))
      );
    ...
    it("provideToSP(): alice is liquidated with risky asset and normal asset--enough in stability pool for partial offset-one SP depositor", async () => {
      ...
      // assert 2475 STAR entered stability pool: min(USD value of Coll not redistributed, 1.1 * debt offseted by stability pool) = 2475
      th.assertIsApproximatelyEqual(
        toBN(dec(2475, 18)),
        SP_WETH.mul(toBN(109)).add(SP_SR.mul(toBN(109)))
      );

      // assert 1185.35625 STAR entered coll surplus pool: offsetting collat - collat sent to stability pool = coll surplus --> 3660.35625 - 2475 = 1185.35625
      th.assertIsApproximatelyEqual(
        toBN(dec(118535625, 13)),
        wethCollSurplusPool
          .mul(toBN(109))
          .add(riskyCollSurplusPool.mul(toBN(109)))
      );
    });
    ...
    it("provideToSP(): alice is liquidated with normal assets--enough in stability pool for partial offset-one SP depositor", async () => {
      ...
      const wavaxCollSurplusPool = await contracts.collSurplusPool.getCollateral(
        contracts.wavax.address
      );
      console.log("stability pool weth", SP_WETH.toString());
      ...
      th.assertIsApproximatelyEqual(
        toBN(dec(24402375, 14)),
        SP_WETH.mul(toBN(109)).add(SP_WAVAX.mul(toBN(109))),
        10000
      );
      ...
      th.assertIsApproximatelyEqual(
        ...
        wethCollSurplusPool
          .mul(toBN(109))
          .add(wavaxCollSurplusPool.mul(toBN(109)))
      );
    });

    /* Alice mints 1000 WAVAX and 1000 WETH, then deposits both into a trove will taking STAR debt.
     * Then she mints another 1000 WETH and deposits that into the trove as well.
     * Then bob mints 500k weth and deposits it into a trove while borrowing STAR.
     * Then bob deposits 5000 STAR into stability pool.
     * Then carol mints 500k weth and deposits it into a trove while borrowing STAR.
     * Then carol deposits 5000 STAR into stability pool.
     * Then Alice gets liquidated.
     * Checks that her collateral is transferred into the
     * stability pool and that bob can withdraw it successfully
    ...
    it("provideToSP(): alice is liquidated-enough in stability pool for full offset-two equivalent SP depositors", async () => {
      ...
      await th.openTroveWithColls(contracts, {
        ICR: toBN(dec(2, 18)),
        colls: collsCarol,
        amounts: amountsCarol,
        extraSTARAmount: SP_Deposit_Carol,
        from: carol,
        oracles: [priceFeedAVAX],
      });
      ...
      await stabilityPool.provideToSP(SP_Deposit_Carol, ZERO_ADDRESS, {
        from: carol,
      });
      ...
      assert.equal(
        stabilityPool_STAR_After.toString(),
        SP_Deposit_Bob.add(SP_Deposit_Carol).toString()
      );
      ...
      console.log(
        "alice new icr",
        (await troveManager.getCurrentICR(alice)).toString()
      );
      ...
      const bob_STAR_gain_fromWithdrawal = bob_post_STAR - bob_pre_STAR;
      const bob_expected_STAR_gain_fromWithdrawal =
        SP_Deposit_Bob - aliceTotalTroveDebt / 2;
      const bob_STAR_error = toBN(
        Math.abs(
          bob_STAR_gain_fromWithdrawal - bob_expected_STAR_gain_fromWithdrawal
        )
      ...
      const carol_pre_STAR = await contracts.starToken.balanceOf(carol);

      await contracts.stabilityPool.withdrawFromSP(SP_Deposit_Carol, {
        from: carol,
      });

      const carol_post_WETH = await contracts.weth.balanceOf(carol);
      const carol_post_WAVAX = await contracts.wavax.balanceOf(carol);
      const carol_post_STAR = await contracts.starToken.balanceOf(carol);

      const carol_WETH_gain = carol_post_WETH - carol_pre_WETH;
      const carol_WAVAX_gain = carol_post_WAVAX - carol_pre_WAVAX;
      const carol_STAR_gain_fromWithdrawal = carol_post_STAR - carol_pre_STAR;
      const carol_expected_STAR_gain_fromWithdrawal =
        SP_Deposit_Carol - aliceTotalTroveDebt / 2;
      const carol_STAR_error = toBN(
        Math.abs(
          carol_STAR_gain_fromWithdrawal -
            carol_expected_STAR_gain_fromWithdrawal
        )
      );
      ...
      // TODO: the received STAR amount is slightly less than expected. Confirm why
      //assert.isTrue(carol_STAR_error < toBN(400000));
     ...
     * Then bob deposits 5000 STAR into stability pool.
     * Then carol mints 500k weth and deposits it into a trove while borrowing STAR.
     * Then carol deposits 10,000 STAR into stability pool.
     * Then Alice gets liquidated.
     * Checks that her collateral is transferred into the
     * stability pool and that bob can withdraw it successfully
     * Also checks collateral is properly split between Carol and Bob.
    ...
    it("provideToSP(): alice is liquidated-enough in stability pool for full offset-two unequal SP depositors with unusual ratios", async () => {
      ...
      await th.openTroveWithColls(contracts, {
        ...
        colls: collsErin,
        amounts: amountsErin,
        extraSTARAmount: SP_Deposit_Erin,
        from: erin,
        oracles: [priceFeedAVAX],
      });
      await stabilityPool.provideToSP(SP_Deposit_Erin, ZERO_ADDRESS, {
        from: erin,
      });

      const erin_pre_WETH = await contracts.weth.balanceOf(erin);
      const erin_pre_WAVAX = await contracts.wavax.balanceOf(erin);
      const erin_pre_STAR = await contracts.starToken.balanceOf(erin);
      ...
      const bob_STAR_gain_fromWithdrawal = bob_post_STAR - bob_pre_STAR;
      const bob_expected_STAR_gain_fromWithdrawal =
        SP_Deposit_Bob - aliceTotalTroveDebt / 3;
      const bob_STAR_error = toBN(
        Math.abs(
          bob_STAR_gain_fromWithdrawal - bob_expected_STAR_gain_fromWithdrawal
        )
      ...
      const carol_STAR_gain_fromWithdrawal = carol_post_STAR - carol_pre_STAR;
      const carol_expected_STAR_gain_fromWithdrawal =
        SP_Deposit_Carol - 2 * (aliceTotalTroveDebt / 3);
      const carol_STAR_error = toBN(
        Math.abs(
          carol_STAR_gain_fromWithdrawal -
            carol_expected_STAR_gain_fromWithdrawal
      ...
      // Erin Withdraws from Stability Pool and receives initial STAR with no gains
      await contracts.stabilityPool.withdrawFromSP(SP_Deposit_Erin, {
        from: erin,
      });

      const erin_post_WETH = await contracts.weth.balanceOf(erin);
      const erin_post_WAVAX = await contracts.wavax.balanceOf(erin);
      const erin_post_STAR = await contracts.starToken.balanceOf(erin);
      ...
      assert.equal(
        erin_post_STAR.sub(erin_pre_STAR).toString(),
        SP_Deposit_Erin.toString()
      );
    });
    ...
    describe("Multi Collateral, Recovery Mode, one SP depositor liquidations, full offset", async () => {
      it("liquidate() recovery mode, with one trove ICR <= 100%", async () => {
        ...
        const weth_totalStakes_Before = await troveManager.getTotalStake(
          weth.address
        );
        const risky_totalStakes_Before = await troveManager.getTotalStake(
          tokenSuperRisky.address
        );
        console.log(
          "weth_totalStakes_Before",
          weth_totalStakes_Before.toString()
        );
        console.log(
          "risky_totalStakes_Before",
          risky_totalStakes_Before.toString()
        );
        ...
        const alice_weth_Stake_After = await troveManager.getTroveStake(
          alice,
          weth.address
        );
        const alice_risky_Stake_After = await troveManager.getTroveStake(
          alice,
          tokenSuperRisky.address
        );
        const weth_totalStakes_After = await troveManager.getTotalStake(
          weth.address
        );
        const risky_totalStakes_After = await troveManager.getTotalStake(
          tokenSuperRisky.address
        );
        assert.equal(alice_weth_Stake_After, 0);
        assert.equal(alice_risky_Stake_After, 0);
        assert.equal(
          weth_totalStakes_After.toString(),
          bobWETH_InTrove.toString()
        );
        assert.equal(
          risky_totalStakes_After.toString(),
          bobRisky_InTrove.toString()
        );

        const weth_totalStakesSnaphot = (
          await troveManager.totalStakesSnapshot(weth.address)
        ).toString();
        const risky_totalStakesSnaphot = (
          await troveManager.totalStakesSnapshot(tokenSuperRisky.address)
        ).toString();
        const weth_totalCollateralSnapshot = (
          await troveManager.totalCollateralSnapshot(weth.address)
        ).toString();
        const risky_totalCollateralSnapshot = (
          await troveManager.totalCollateralSnapshot(tokenSuperRisky.address)
        ).toString();
        ...
        assert.equal(
          weth_totalCollateralSnapshot,
          bobWETH_InTrove
            .add(th.applyLiquidationFee(aliceWETH_InTrove))
            .toString()
        );
        assert.equal(
          risky_totalCollateralSnapshot,
          bobRisky_InTrove
            .add(th.applyLiquidationFee(aliceRisky_InTrove))
            .toString()
        );
      });

      it("liquidate() recovery mode, 100% < ICR < MCR & SP STAR > Trove debt, full offset ratio = 1, 0.5", async () => {
        ...
        // provideToSP()
        await stabilityPool.provideToSP(toBN(dec(5000, 18)), ZERO_ADDRESS, {
          from: bob,
        });
        ...
        // STAR in stability pool > Alice debt --> 4010 x 1.1 = 4411 sent to stability pool.
        // 5814.5 - 4411 = 1374.4275 sent to collsurplus pool

        // assert correct amount of collateral entered stability pool
        console.log(
          SP_WETH.mul(toBN(130))
            .add(SP_Risky.mul(toBN(80)))
            .add(SP_WAVAX.mul(toBN(200)))
            .toString()
        );
        th.assertIsApproximatelyEqual(
          toBN(dec(4411, 18)),
        ...
        console.log(
          wethCollSurplusPool
            .mul(toBN(130))
            .add(riskyCollSurplusPool.mul(toBN(80)))
            .add(wavaxCollSurplusPool.mul(toBN(200)))
            .toString()
        );
        th.assertIsApproximatelyEqual(
          toBN(dec(13744275, 14)),
          wethCollSurplusPool
            .mul(toBN(130))
            .add(riskyCollSurplusPool.mul(toBN(80)))
            .add(wavaxCollSurplusPool.mul(toBN(200)))
      ...
      it("liquidate() recovery mode, 100% < ICR < MCR & SP STAR < Trove debt, partial offset ratio = 1, 0.5", async () => {
        // --- SETUP ---
        const colls = [contracts.weth, tokenSuperRisky];
        const aliceWETH_InTrove = toBN(dec(2005, 16));
        const aliceRisky_InTrove = toBN(dec(4010, 16));
        ...
        // provideToSP()
        await stabilityPool.provideToSP(toBN(dec(2000, 18)), ZERO_ADDRESS, {
          from: bob,
        });
        ...
        th.assertIsApproximatelyEqual(
          toBN(dec(2200, 18)),
          SP_WETH.mul(toBN(109)).add(SP_Risky.mul(toBN(109)))
        );
      ...
      it("liquidate() recovery mode, MCR <= ICR < 150% & SP STAR >= Trove debt, full offset ratio = 1, 1, 0.5", async () => {
        const collsAlice = [contracts.weth, tokenSuperRisky];
        const aliceWETH_InTrove = toBN(dec(2005, 16));
        const aliceRisky_InTrove = toBN(dec(4010, 16));
        const amountsAlice = [aliceWETH_InTrove, aliceRisky_InTrove];
        ...
        // STAR in stability pool > Alice debt --> 4010 x 1.1 x 0.995 = 4,388.95 sent to stability pool.
        // 6616.5 - 4411 = 2205.5 sent to collsurplus pool

        // assert correct amount of collateral entered stability pool
        console.log(
          SP_WETH.mul(toBN(130))
            .add(SP_Risky.mul(toBN(100)))
            .add(SP_WAVAX.mul(toBN(200)))
            .toString()
        );
        th.assertIsApproximatelyEqual(
          toBN(dec(4411, 18)).mul(toBN(995)).div(toBN(1000)),
          SP_WETH.mul(toBN(130))
            .add(SP_Risky.mul(toBN(100)))
            .add(SP_WAVAX.mul(toBN(200))),
          10000
        ...
        console.log(
          wethCollSurplusPool
            .mul(toBN(130))
            .add(riskyCollSurplusPool.mul(toBN(100)))
            .add(wavaxCollSurplusPool.mul(toBN(200)))
            .toString()
        );
        th.assertIsApproximatelyEqual(
      ...
      it("liquidate() recovery mode, MCR <= ICR < 150% & SP STAR < Trove debt, nothing happens, ratio = 1, 0.5", async () => {
        const collsAlice = [contracts.weth, tokenSuperRisky];
        const aliceWETH_InTrove = toBN(dec(2005, 16));
        const aliceRisky_InTrove = toBN(dec(4010, 16));
        const amountsAlice = [aliceWETH_InTrove, aliceRisky_InTrove];
