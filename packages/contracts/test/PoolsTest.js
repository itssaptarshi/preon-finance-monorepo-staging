...
contract("StabilityPool", async (accounts) => {
  ...
  */
  ...
  it("getTotalSTARDeposits(): gets the recorded STAR balance", async () => {
    const recordedETHBalance = await stabilityPool.getTotalSTARDeposits();
    assert.equal(recordedETHBalance, 0);
  });
});
...
contract("ActivePool", async (accounts) => {
  ...
  it("getSTARDebt(): gets the recorded STAR balance", async () => {
    const recordedETHBalance = await activePool.getSTARDebt();
    assert.equal(recordedETHBalance, 0);
  });

  it("increaseSTAR(): increases the recorded STAR balance by the correct amount", async () => {
    const recordedSTAR_balanceBefore = await activePool.getSTARDebt();
    assert.equal(recordedSTAR_balanceBefore, 0);

    // await activePool.increaseSTARDebt(100, { from: mockBorrowerOperationsAddress })
    const increaseSTARDebtData = th.getTransactionData(
      "increaseSTARDebt(uint256)",
      ["0x64"]
    );
    const tx = await mockBorrowerOperations.forward(
      activePool.address,
      increaseSTARDebtData
    );
    assert.isTrue(tx.receipt.status);
    const recordedSTAR_balanceAfter = await activePool.getSTARDebt();
    assert.equal(recordedSTAR_balanceAfter, 100);
  });
  // Decrease
  it("decreaseSTAR(): decreases the recorded STAR balance by the correct amount", async () => {
    ...
    const recordedSTAR_balanceBefore = await activePool.getSTARDebt();
    assert.equal(recordedSTAR_balanceBefore, 100);

    //await activePool.decreaseSTARDebt(100, { from: mockBorrowerOperationsAddress })
    const decreaseSTARDebtData = th.getTransactionData(
      "decreaseSTARDebt(uint256)",
      ["0x64"]
    );
    const tx2 = await mockBorrowerOperations.forward(
      activePool.address,
      decreaseSTARDebtData
    );
    assert.isTrue(tx2.receipt.status);
    const recordedSTAR_balanceAfter = await activePool.getSTARDebt();
    assert.equal(recordedSTAR_balanceAfter, 0);
  });
...
contract("DefaultPool", async (accounts) => {
  ...
  it("getETH(): gets the recorded STAR balance", async () => {
    const recordedETHBalance = await defaultPool.getCollateral(weth.address);
    assert.equal(recordedETHBalance, 0);
  });

  it("getSTARDebt(): gets the recorded STAR balance", async () => {
    const recordedETHBalance = await defaultPool.getSTARDebt();
    assert.equal(recordedETHBalance, 0);
  });

  it("increaseSTAR(): increases the recorded STAR balance by the correct amount", async () => {
    const recordedSTAR_balanceBefore = await defaultPool.getSTARDebt();
    assert.equal(recordedSTAR_balanceBefore, 0);

    // await defaultPool.increaseSTARDebt(100, { from: mockTroveManagerAddress })
    const increaseSTARDebtData = th.getTransactionData(
      "increaseSTARDebt(uint256)",
      ["0x64"]
    );
    ...
    const recordedSTAR_balanceAfter = await defaultPool.getSTARDebt();
    assert.equal(recordedSTAR_balanceAfter, 100);
  });
  ...
  it("decreaseSTAR(): decreases the recorded STAR balance by the correct amount", async () => {
    ...
    // await defaultPool.decreaseSTARDebt(100, { from: mockTroveManagerAddress })
    const decreaseSTARDebtData = th.getTransactionData(
      "decreaseSTARDebt(uint256)",
      ["0x64"]
    );
