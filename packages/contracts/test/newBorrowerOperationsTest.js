...
contract("newBorrowerOperations", async (accounts) => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    it("addColl(), basic sanity", async () => {
      ...
      const wavaxMint = await th.addERC20(
        contracts.wavax,
        alice,
        borrowerOperations.address,
        amountToMint,
      ...
      //     extraParams: { from: alice, _colls: colls, _amounts: amounts, _starAmount: toBN(dec(2000, 18)), _priceFeeds: priceFeeds } })
      ...
      console.log(
        "WETH active pool has:",
        activePoolWeth.div(toBN(10 ** 18)).toNumber()
      );
      ...
      console.log(
        "wavax activepool has:",
        activePoolWavax.div(toBN(10 ** 18)).toNumber()
      );
      ...
      const wethMint2 = await th.addERC20(
        contracts.weth,
        alice,
        borrowerOperations.address,
        amountToMint,
      ...
      const wethMint3 = await th.addERC20(
        contracts.weth,
        alice,
        borrowerOperations.address,
        amountToMint,
      ...
      const mintedSTARb = await contracts.starToken.balanceOf(alice);
      console.log(th.toNormalBase(mintedSTARb));

      const result = await th.mintAndApproveSTARToken(
        contracts,
        alice,
        borrowerOperations.address,
        amountToMint
      );
      const mintedSTAR = await contracts.starToken.balanceOf(alice);
      console.log(th.toNormalBase(mintedSTAR));
      console.log("result " + result);

      // const amountToMint = toBN(dec(1000, 18));
      // mint weth for bob and approve borrowerOperations to use it
      const wethMintBob = await th.addERC20(
        contracts.weth,
        bob,
        borrowerOperations.address,
        amountToMint,
      ...
      const wavaxMintBob = await th.addERC20(
        contracts.wavax,
        bob,
        borrowerOperations.address,
        amountToMint,
      ...
      const activePoolWeth2 = await contracts.weth.balanceOf(
        activePool.address
      );
      console.log(
        "WETH active pool has:",
        activePoolWeth2.div(toBN(10 ** 18)).toNumber()
      );

      const activePoolWavax2 = await contracts.wavax.balanceOf(
        activePool.address
      );
      console.log(
        "wavax activepool has:",
        activePoolWavax2.div(toBN(10 ** 18)).toNumber()
      );

      const aliceSTAR2 = await starToken.balanceOf(alice);
      console.log("star MINTED:", aliceSTAR2.div(toBN(10 ** 18)).toNumber());

      const aliceAvax2 = await contracts.wavax.balanceOf(alice);
      console.log(
        "wavax alice has:",
        aliceAvax2.div(toBN(10 ** 18)).toNumber()
      );
