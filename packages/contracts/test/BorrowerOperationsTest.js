...
contract("BorrowerOperations", async (accounts) => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    it("addColl(): reverts when top-up would leave trove with ICR < MCR", async () => {
      ...
      assert.isTrue(
        (await troveManager.getCurrentICR(alice)).lt(toBN(dec(110, 16)))
      );

      const collTopUp = toBN(dec(1, 18)); // 1 wei top up

      const wethMint = await th.addERC20(
        contracts.weth,
        alice,
    ...
    it("addColl(), active Trove: adds the correct collateral amount to the Trove", async () => {
      ...
      assert.isTrue(
        coll_After[1][0].eq(coll_before[1][0].add(toBN(dec(1, "ether"))))
      );
    ...
    it("addColl(), active Trove: updates the stake and updates the total stakes", async () => {
      ...
      const alice_Stake_Before = await troveManager.getTroveStake(
        alice,
        contracts.weth.address
      );
      ...
      // const alice_Trove_After = await troveManager.Troves(alice)
      const alice_Stake_After = await troveManager.getTroveStake(
        alice,
        contracts.weth.address
      );
    ...
    it("addColl(), active Trove: applies pending rewards and updates user's L_ETH, L_STARDebt snapshots", async () => {
      // --- SETUP ---
      ...
      // price drops to 1ETH:100STAR, reducing Carol's ICR below MCR
      ...
      const L_STARDebt = await troveManager.getL_STAR(contracts.weth.address);
      ...
      const alice_ETHrewardSnapshot_Before = await troveManager.getRewardSnapshotColl(
        alice,
        contracts.weth.address
      );
      const alice_STARDebtRewardSnapshot_Before = await troveManager.getRewardSnapshotSTAR(
        alice,
        contracts.weth.address
      );
      ...
      // const bob_STARDebtRewardSnapshot_Before = bob_rewardSnapshot_Before[1]
      const bob_ETHrewardSnapshot_Before = await troveManager.getRewardSnapshotColl(
        bob,
        contracts.weth.address
      );
      const bob_STARDebtRewardSnapshot_Before = await troveManager.getRewardSnapshotSTAR(
        bob,
        contracts.weth.address
      );

      assert.equal(alice_ETHrewardSnapshot_Before, 0);
      assert.equal(alice_STARDebtRewardSnapshot_Before, 0);
      assert.equal(bob_ETHrewardSnapshot_Before, 0);
      assert.equal(bob_STARDebtRewardSnapshot_Before, 0);

      const alicePendingETHReward = (
        await troveManager.getPendingCollRewards(alice)
      )[1][0];
      const bobPendingETHReward = (
        await troveManager.getPendingCollRewards(bob)
      )[1][0];
      const alicePendingSTARDebtReward = await troveManager.getPendingSTARDebtReward(
        alice
      );
      const bobPendingSTARDebtReward = await troveManager.getPendingSTARDebtReward(
        bob
      );
      for (reward of [
        alicePendingETHReward,
        bobPendingETHReward,
        alicePendingSTARDebtReward,
        bobPendingSTARDebtReward,
      ]) {
      ...
      const aliceMint = await th.addERC20(
        contracts.weth,
        alice,
        contracts.borrowerOperations.address,
        aliceTopUp,
      ...
      const bobMint = await th.addERC20(
        contracts.weth,
        bob,
        contracts.borrowerOperations.address,
        aliceTopUp,
      ...
      assert.isTrue(
        aliceNewDebt.eq(aliceDebtBefore.add(alicePendingSTARDebtReward))
      );
      assert.isTrue(
        bobNewColl.eq(bobCollBefore.add(bobPendingETHReward).add(bobTopUp))
      );
      assert.isTrue(bobNewDebt.eq(bobDebtBefore.add(bobPendingSTARDebtReward)));

      /* Check that both Alice and Bob's snapshots of the rewards-per-unit-staked metrics should be updated
       to the latest values of L_ETH and L_STARDebt */
      ...
      // const alice_STARDebtRewardSnapshot_After = alice_rewardSnapshot_After[1]
      const alice_ETHrewardSnapshot_After = await troveManager.getRewardSnapshotColl(
        alice,
        contracts.weth.address
      );
      const alice_STARDebtRewardSnapshot_After = await troveManager.getRewardSnapshotSTAR(
        alice,
        contracts.weth.address
      );
      ...
      // const bob_STARDebtRewardSnapshot_After = bob_rewardSnapshot_After[1]
      const bob_ETHrewardSnapshot_After = await troveManager.getRewardSnapshotColl(
        bob,
        contracts.weth.address
      );
      const bob_STARDebtRewardSnapshot_After = await troveManager.getRewardSnapshotSTAR(
        bob,
        contracts.weth.address
      );
      ...
      assert.isAtMost(
        th.getDifference(alice_STARDebtRewardSnapshot_After, L_STARDebt),
        100
      );
      assert.isAtMost(
      ...
      assert.isAtMost(
        th.getDifference(bob_STARDebtRewardSnapshot_After, L_STARDebt),
        100
      );
    });
    ...
    //   // --- SETUP ---
    //   // A,B,C add 15/5/5 ETH, withdraw 100/100/900 STAR
    ...
    //   // close Carol's Trove, liquidating her 5 ether and 900STAR.
    ...
    it("addColl(), reverts if trove is non-existent or closed", async () => {
      ...
      try {
        ...
        assert.isTrue(wethMint);
        txCarol = await borrowerOperations.addColl(
          [contracts.weth.address],
          [collTopUp],
          th.ZERO_ADDRESS,
          th.ZERO_ADDRESS,
    ...
    it("addColl(): can add collateral in Recovery Mode", async () => {
      ...
      const coll_After = await troveManager.getTroveColls(alice);
      assert.isTrue(
        coll_After[1][0].eq(coll_before.add(toBN(dec(1, "ether"))))
      );
    });
    ...
    it("withdrawColl(): reverts if system is in Recovery Mode", async () => {
      ...
      // price drops to 1ETH:150STAR, reducing TCR below 150%
    ...
    it("withdrawColl(): reduces the Trove's collateral by the correct amount", async () => {
      ...
      assert.isTrue(
        toBN(aliceCollAfter).eq(
          toBN(aliceCollBefore).sub(toBN(dec(1, "ether")))
        )
      );
    });
    ...
    it("withdrawColl(): applies pending rewards and updates user's L_ETH, L_STARDebt snapshots", async () => {
      // --- SETUP ---
      ...
      // close Carol's Trove, liquidating her 1 ether and 180STAR.
      await troveManager.liquidate(carol, { from: owner });

      // const L_ETH = await troveManager.L_ETH()
      const L_ETH = await troveManager.getL_Coll(contracts.weth.address);
      const L_STARDebt = await troveManager.L_STARDebt(contracts.weth.address);

      // check Alice and Bob's reward snapshots are zero before they alter their Troves
      // const alice_rewardSnapshot_Before = await troveManager.rewardSnapshots(alice)
      // const alice_ETHrewardSnapshot_Before = alice_rewardSnapshot_Before[0]
      // const alice_STARDebtRewardSnapshot_Before = alice_rewardSnapshot_Before[1]
      const alice_ETHrewardSnapshot_Before = await troveManager.getRewardSnapshotColl(
        alice,
        contracts.weth.address
      );
      ...
      const pendingDebtReward_A = await troveManager.getPendingSTARDebtReward(
        alice
      );
      const pendingCollReward_B = (
        await troveManager.getPendingCollRewards(bob)
      )[1][0];
      const pendingDebtReward_B = await troveManager.getPendingSTARDebtReward(
        bob
      );
      for (reward of [
        pendingCollReward_A,
      ...
      th.assertIsApproximatelyEqual(
        aliceCollAfter[0],
        aliceCollBefore[0].add(pendingCollReward_A).sub(aliceCollWithdrawal),
        10000
      );
      th.assertIsApproximatelyEqual(
        aliceDebtAfter,
      ...
      th.assertIsApproximatelyEqual(
        bobCollAfter[0],
        bobCollBefore[0].add(pendingCollReward_B).sub(bobCollWithdrawal),
        10000
      );
      th.assertIsApproximatelyEqual(
        bobDebtAfter,
    ...
    it("withdrawSTAR(): reverts when withdrawal would leave trove with ICR < MCR", async () => {
      // alice creates a Trove and adds first collateral
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } });
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob } });
      ...
      const STARwithdrawal = 1; // withdraw 1 wei STAR
    ...
    it("withdrawSTAR(): decays a non-zero base rate", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });

      await openTrove({
        extraSTARAmount: toBN(dec(20, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      });
      await openTrove({
      ...
      const A_STARBal = await starToken.balanceOf(A);
      ...
      // D withdraws STAR
      await borrowerOperations.withdrawSTAR(th._100pct, dec(1, 18), A, A, {
        from: D,
      });
    ...
    it("withdrawSTAR(): reverts if max fee > 100%", async () => {
      await openTrove({
      ...
      await openTrove({
        extraSTARAmount: toBN(dec(40, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C },
      });
      await openTrove({
      ...
      await assertRevert(
        borrowerOperations.withdrawSTAR(dec(2, 18), dec(1, 18), A, A, {
          from: A,
        }),
    ...
    it("withdrawSTAR(): reverts if max fee < 0.5% in Normal mode", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(10, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      ...
      await assertRevert(
        borrowerOperations.withdrawSTAR(0, dec(1, 18), A, A, { from: A }),
        "Max fee percentage must be between 0.5% and 100%"
      );
      await assertRevert(
        borrowerOperations.withdrawSTAR(1, dec(1, 18), A, A, { from: A }),
        "Max fee percentage must be between 0.5% and 100%"
      );
      await assertRevert(
        borrowerOperations.withdrawSTAR("4999999999999999", dec(1, 18), A, A, {
          from: A,
        }),
        "Max fee percentage must be between 0.5% and 100%"
      );
    ...
    it("withdrawSTAR(): reverts if fee exceeds max fee percentage", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(60, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      });
      await openTrove({
      ...
      await openTrove({
        extraSTARAmount: toBN(dec(70, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C },
      });
      await openTrove({
        extraSTARAmount: toBN(dec(80, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: D },
      });
      await openTrove({
        extraSTARAmount: toBN(dec(180, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: E },
      });
      ...
      await assertRevert(
        borrowerOperations.withdrawSTAR(lessThan5pct, dec(3, 18), A, A, {
          from: A,
        }),
      ...
      await assertRevert(
        borrowerOperations.withdrawSTAR(dec(1, 16), dec(1, 18), A, A, {
          from: B,
        }),
        "Fee exceeded provided maximum"
      );
      ...
      await assertRevert(
        borrowerOperations.withdrawSTAR(dec(3754, 13), dec(1, 18), A, A, {
          from: C,
        }),
        "Fee exceeded provided maximum"
      );
      ...
      await assertRevert(
        borrowerOperations.withdrawSTAR(dec(5, 15), dec(1, 18), A, A, {
          from: D,
        }),
        "Fee exceeded provided maximum"
      );
    ...
    it("withdrawSTAR(): succeeds when fee is less than max fee percentage", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(60, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      ...
      const tx1 = await borrowerOperations.withdrawSTAR(
        moreThan5pct,
        dec(1, 18),
        A,
        A,
      ...
      // Attempt with maxFee = 5%
      const tx2 = await borrowerOperations.withdrawSTAR(
        dec(5, 16),
        dec(1, 18),
        A,
        A,
      ...
      const tx3 = await borrowerOperations.withdrawSTAR(
        dec(1, 17),
        dec(1, 18),
        A,
        A,
      ...
      const tx4 = await borrowerOperations.withdrawSTAR(
        dec(37659, 13),
        dec(1, 18),
        A,
        A,
      ...
      const tx5 = await borrowerOperations.withdrawSTAR(
        dec(1, 18),
        dec(1, 18),
        A,
        A,
    ...
    it("withdrawSTAR(): doesn't change base rate if it is already zero", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });
      ...
      await openTrove({
        extraSTARAmount: toBN(dec(50, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C },
      });
      await openTrove({
      ...
      // D withdraws STAR
      await borrowerOperations.withdrawSTAR(th._100pct, dec(37, 18), A, A, {
        from: D,
      });
      ...
      // E opens trove
      await borrowerOperations.withdrawSTAR(th._100pct, dec(12, 18), A, A, {
        from: E,
      });
    ...
    it("withdrawSTAR(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });
      ...
      await borrowerOperations.withdrawSTAR(th._100pct, dec(1, 18), C, C, {
        from: C,
      });
    ...
    it("withdrawSTAR(): borrower can't grief the baseRate and stop it decaying by issuing debt at higher frequency than the decay granularity", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });
      await openTrove({
        extraSTARAmount: toBN(dec(30, 18)),
        ICR: toBN(dec(2, 18)),
    ...
    it("withdrawSTAR(): borrowing at non-zero base rate sends STAR fee to sPREON contract", async () => {
      ...
      // Check PREON STAR balance before == 0
      const sPREON_STARBalance_Before = await starToken.balanceOf(
        sPREON.address
      );
      assert.equal(sPREON_STARBalance_Before, "0");
      ...
      // D withdraws STAR
      await borrowerOperations.withdrawSTAR(th._100pct, dec(37, 18), C, C, {
        from: D,
      });

      // Check PREON STAR balance after has increased
      const sPREON_STARBalance_After = await starToken.balanceOf(
        sPREON.address
      );
      assert.isTrue(sPREON_STARBalance_After.gt(sPREON_STARBalance_Before));
    });

    if (!withProxy) {
      // TODO: use rawLogs instead of logs
      it("withdrawSTAR(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        ...
        const withdrawalTx = await borrowerOperations.withdrawSTAR(
          th._100pct,
          toBN(dec(37, 18)),
          D,
          D,
        ...
        const emittedFee = toBN(
          th.getSTARFeeFromSTARBorrowingEvent(withdrawalTx)
        );
    ...
    it("withdrawSTAR(): Borrowing at non-zero base rate increases the sPREON contract STAR fees-per-unit-staked", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 PREON
      await th.fastForwardTime(
        timeValues.SECONDS_IN_ONE_YEAR,
        web3.currentProvider
      ...
      // @KingPreon: no F_STAR() function
      // Check PREON contract STAR fees-per-unit-staked is zero
      // const F_STAR_Before = await sPREON.F_STAR()
      ...
      // @KingPreon: F_STAR() function no longer exists
      // // Check PREON contract STAR fees-per-unit-staked has increased
      // const F_STAR_After = await sPREON.F_STAR()
      // assert.isTrue(F_STAR_After.gt(F_STAR_Before))
    });

    it("withdrawSTAR(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      // time fast-forwards 1 year, and E stakes 1 PREON
      await th.fastForwardTime(
        timeValues.SECONDS_IN_ONE_YEAR,
        web3.currentProvider
      ...
      const D_STARBalanceBefore = await starToken.balanceOf(D);

      // D withdraws STAR
      const D_STARRequest = toBN(dec(37, 18));
      await borrowerOperations.withdrawSTAR(th._100pct, D_STARRequest, D, D, {
        from: D,
      });

      // Check sPREON STAR balance has increased
      const sPREON_STARBalance_After = await starToken.balanceOf(
        sPREON.address
      );
      assert.isTrue(sPREON_STARBalance_After.gt(sPREON_STARBalance_Before));

      // Check D's STAR balance now equals their initial balance plus request STAR
      const D_STARBalanceAfter = await starToken.balanceOf(D);
      assert.isTrue(
        D_STARBalanceAfter.eq(D_STARBalanceBefore.add(D_STARRequest))
      );
    });

    it("withdrawSTAR(): Borrowing at zero base rate increases STAR in sPREON contract", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });
      await openTrove({
        extraSTARAmount: toBN(dec(30, 18)),
        ICR: toBN(dec(2, 18)),
      ...
      // Check PREON STAR balance before == 0
      const F_STAR_Before = await starToken.balanceOf(sPREON.address);

      // D withdraws STAR
      await borrowerOperations.withdrawSTAR(th._100pct, dec(37, 18), D, D, {
        from: D,
      });

      // Check PREON STAR balance after > 0
      const F_STAR_After = await starToken.balanceOf(sPREON.address);
      assert.isTrue(F_STAR_After.gt("0"));
    });

    it("withdrawSTAR(): Borrowing at zero base rate sends debt request to user", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });
      await openTrove({
        extraSTARAmount: toBN(dec(30, 18)),
        ICR: toBN(dec(2, 18)),
      ...
      // Check D's STAR balance now equals their requested STAR
      const D_STARBalanceAfter = await starToken.balanceOf(D);

      // Check D's trove debt == D's STAR balance + liquidation reserve
      assert.isTrue(
        D_STARBalanceAfter.eq(D_STARBalanceBefore.add(D_STARRequest))
      );
    });

    it("withdrawSTAR(): reverts when calling address does not have active trove", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: alice } });
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } });

      // Bob successfully withdraws STAR
      const txBob = await borrowerOperations.withdrawSTAR(
        th._100pct,
        dec(100, 18),
        bob,
        bob,
      ...
      // Carol with no active trove attempts to withdraw STAR
      try {
        const txCarol = await borrowerOperations.withdrawSTAR(
          th._100pct,
          dec(100, 18),
          carol,
          carol,
    ...
    it("withdrawSTAR(): reverts when requested withdrawal amount is zero STAR", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } });
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } });

      // Bob successfully withdraws 1e-18 STAR
      const txBob = await borrowerOperations.withdrawSTAR(
        th._100pct,
        1,
        bob,
      ...
      // Alice attempts to withdraw 0 STAR
      try {
        const txAlice = await borrowerOperations.withdrawSTAR(
          th._100pct,
          0,
          alice,
          alice,
    ...
    it("withdrawSTAR(): reverts when system is in Recovery Mode", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } });
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } });
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: carol } });
      ...
      //Check STAR withdrawal impossible when recoveryMode == true
      try {
        const txBob = await borrowerOperations.withdrawSTAR(
          th._100pct,
          1,
    ...
    it("withdrawSTAR(): reverts when withdrawal would bring the trove's ICR < MCR", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: alice } });
      await openTrove({ ICR: toBN(dec(11, 17)), extraParams: { from: bob } });

      // Bob tries to withdraw STAR that would bring his ICR < MCR
      try {
        const txBob = await borrowerOperations.withdrawSTAR(
          th._100pct,
          1,
    ...
    it("withdrawSTAR(): reverts when a withdrawal would cause the TCR of the system to fall below the CCR", async () => {
      await priceFeed.setPrice(dec(100, 18));
      const price = await priceFeed.getPrice();
      ...
      // Bob attempts to withdraw 1 STAR.
    ...
    it("withdrawSTAR(): reverts if system is in Recovery Mode", async () => {
      // --- SETUP ---
      await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: alice } });
      await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: bob } });
      ...
      try {
        const txData = await borrowerOperations.withdrawSTAR(
          th._100pct,
          "200",
          alice,
          alice,
    ...
    it("withdrawSTAR(): increases the Trove's STAR debt by the correct amount", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } });
    ...
    it("withdrawSTAR(): increases STAR debt in ActivePool by correct amount", async () => {
      await openTrove({
        ICR: toBN(dec(10, 18)),
        extraParams: { from: alice, value: toBN(dec(100, "ether")) },
      });
      ...
      // check before
      const activePool_STAR_Before = await activePool.getSTARDebt();
      assert.isTrue(activePool_STAR_Before.eq(aliceDebtBefore));
      ...
      // check after
      const activePool_STAR_After = await activePool.getSTARDebt();
      th.assertIsApproximatelyEqual(
        activePool_STAR_After,
        activePool_STAR_Before.add(toBN(dec(10000, 18)))
      );
    });

    it("withdrawSTAR(): increases user STARToken balance by correct amount", async () => {
      await openTrove({
        ICR: toBN(dec(10, 18)),
        extraParams: { from: alice, value: toBN(dec(100, "ether")) },
      });
      ...
      // check before
      const alice_STARTokenBalance_Before = await starToken.balanceOf(alice);
      assert.isTrue(alice_STARTokenBalance_Before.gt(toBN("0")));
      ...
      // check after
      const alice_STARTokenBalance_After = await starToken.balanceOf(alice);
      assert.isTrue(
        alice_STARTokenBalance_After.eq(
          alice_STARTokenBalance_Before.add(toBN(dec(10000, 18)))
        )
      );
    });

    // --- repaySTAR() ---
    it("repaySTAR(): reverts when repayment would leave trove with ICR < MCR", async () => {
      // alice creates a Trove and adds first collateral
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } });
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob } });
      ...
      const STARRepayment = 1; // 1 wei repayment

      await assertRevert(
        borrowerOperations.repaySTAR(STARRepayment, alice, alice, {
          from: alice,
        }),
        "BorrowerOps: An operation that would result in ICR < MCR is not permitted"
      );
    ...
    it("repaySTAR(): Succeeds when it would leave trove with net debt >= minimum net debt", async () => {
      // Make the STAR request 2 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
      await th.addERC20(
        contracts.weth,
        A,
        contracts.borrowerOperations.address,
      ...
      const repayTxA = await borrowerOperations.repaySTAR(1, A, A, { from: A });
      ...
      const repayTxB = await borrowerOperations.repaySTAR(dec(19, 25), B, B, {
        from: B,
      });
      assert.isTrue(repayTxB.receipt.status);
    });

    it("repaySTAR(): reverts when it would leave trove with net debt < minimum net debt", async () => {
      // Make the STAR request 2 wei above min net debt to correct for floor division, and make net debt = min net debt - 1 wei
      await th.addERC20(
        contracts.weth,
        A,
      ...
      console.log(
        "This is the repaid amount ",
        (await troveManager.getTroveDebt(A)).toString()
      );

      const repayTxAPromise = borrowerOperations.repaySTAR(2, A, A, {
        from: A,
      });
      await assertRevert(
        repayTxAPromise,
    ...
    it("repaySTAR(): reverts when calling address does not have active trove", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      // Bob successfully repays some STAR
      const txBob = await borrowerOperations.repaySTAR(dec(10, 18), bob, bob, {
        from: bob,
      });
      assert.isTrue(txBob.receipt.status);

      // Carol with no active trove attempts to repaySTAR
      try {
        const txCarol = await borrowerOperations.repaySTAR(
          dec(10, 18),
          carol,
          carol,
          { from: carol }
    ...
    it("repaySTAR(): reverts when attempted repayment is > the debt of the trove", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      try {
        const txAlice = await borrowerOperations.repaySTAR(
    ...
    //repaySTAR: reduces STAR debt in Trove
    it("repaySTAR(): reduces the Trove's STAR debt by the correct amount", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      await borrowerOperations.repaySTAR(
    ...
    it("repaySTAR(): decreases STAR debt in ActivePool by correct amount", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      // Check before
      const activePool_STAR_Before = await activePool.getSTARDebt();
      assert.isTrue(activePool_STAR_Before.gt(toBN("0")));
      ...
      th.assertIsApproximatelyEqual(
        activePool_STAR_After,
        activePool_STAR_Before.sub(aliceDebtBefore.div(toBN(10)))
      );
    });

    it("repaySTAR(): decreases user STARToken balance by correct amount", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      const alice_STARTokenBalance_After = await starToken.balanceOf(alice);
      th.assertIsApproximatelyEqual(
        alice_STARTokenBalance_After,
        alice_STARTokenBalance_Before.sub(aliceDebtBefore.div(toBN(10)))
      );
    });

    it("repaySTAR(): can repay debt in Recovery Mode", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      const tx = await borrowerOperations.repaySTAR(
        aliceDebtBefore.div(toBN(10)),
        alice,
        alice,
        { from: alice }
    ...
    it("repaySTAR(): Reverts if borrower has insufficient STAR balance to cover his debt repayment", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      const bobBalBefore = await starToken.balanceOf(B);
      assert.isTrue(bobBalBefore.gt(toBN("0")));

      // Bob transfers all but 5 of his STAR to Carol
      await starToken.transfer(C, bobBalBefore.sub(toBN(dec(5, 18))), {
        from: B,
      });

      //Confirm B's STAR balance has decreased to 5 STAR
      const bobBalAfter = await starToken.balanceOf(B);

      assert.isTrue(bobBalAfter.eq(toBN(dec(5, 18))));

      // Bob tries to repay 6 STAR
      const repaySTARPromise_B = borrowerOperations.repaySTAR(
        toBN(dec(6, 18)),
        B,
        B,
        { from: B }
      ...
      await assertRevert(
        repaySTARPromise_B,
        "Caller doesnt have enough STAR to make repayment"
      );
    });
    ...
    it("adjustTrove(): reverts when adjustment would leave trove with ICR < MCR", async () => {
      ...
      await assertRevert(
        ...
        // borrowerOperations.adjustTrove(th._100pct, 0, STARRepayment, false, alice, alice, { from: alice, value: collTopUp }),
        "BorrowerOps: An operation that would result in ICR < MCR is not permitted"
      );
    });
    ...
    it("adjustTrove(): borrowing at non-zero base rate sends STAR fee to sPREON contract", async () => {
      // time fast-forwards 1 year, and E stakes 1 PREON
      await th.fastForwardTime(
        timeValues.SECONDS_IN_ONE_YEAR,
        web3.currentProvider
      ...
      // D adjusts trove
      await openTrove({
        extraSTARAmount: toBN(dec(37, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: D },
      });
    ...
    if (!withProxy) {
      ...
      it("adjustTrove(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        ...
        const emittedFee = toBN(
          th.getSTARFeeFromSTARBorrowingEvent(adjustmentTx)
        );
        assert.isTrue(emittedFee.gt(toBN("0")));
    ...
    it("adjustTrove(): Borrowing at non-zero base rate increases the sPREON contract STAR fees-per-unit-staked", async () => {
      // time fast-forwards 1 year, and E stakes 1 PREON
      await th.fastForwardTime(
        timeValues.SECONDS_IN_ONE_YEAR,
        web3.currentProvider
    ...
    it("adjustTrove(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      // D adjusts trove
      const STARRequest_D = toBN(dec(40, 18));
      await borrowerOperations.adjustTrove(
        [],
        [],
        [],
      ...
      // await borrowerOperations.adjustTrove(th._100pct, 0, STARRequest_D, true, D, D, { from: D })
      ...
      // Check D's STAR balance has increased by their requested STAR
      const D_STARBalanceAfter = await starToken.balanceOf(D);
      assert.isTrue(
        D_STARBalanceAfter.eq(D_STARBalanceBefore.add(STARRequest_D))
      );
    });

    it("adjustTrove(): Borrowing at zero base rate changes STAR balance of sPREON contract", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });
      await openTrove({
        extraSTARAmount: toBN(dec(30, 18)),
        ICR: toBN(dec(2, 18)),
      ...
      // Check staking STAR balance before > 0
      const sPREON_STARBalance_Before = await starToken.balanceOf(
        sPREON.address
      );
      assert.isTrue(sPREON_STARBalance_Before.gt(toBN("0")));
      ...
      // Check staking STAR balance after > staking balance before
      const sPREON_STARBalance_After = await starToken.balanceOf(
        sPREON.address
      );
      assert.isTrue(sPREON_STARBalance_After.gt(sPREON_STARBalance_Before));
    ...
    it("adjustTrove(): Borrowing at zero base rate increases STAR in sPREON contract", async () => {
      await openTrove({
      ...
      // Check staking STAR balance increases
      const F_STAR_After = await starToken.balanceOf(sPREON.address);
      assert.isTrue(F_STAR_After.gt(F_STAR_Before));
    });

    it("adjustTrove(): Borrowing at zero base rate sends total requested STAR to the user", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(20000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale, value: toBN(dec(100, "ether")) },
      ...
      const D_STARBalBefore = await starToken.balanceOf(D);
      // Check baseRate is zero
      const baseRate_1 = await troveManager.baseRate();
      assert.equal(baseRate_1, "0");
      ...
      const DUSDBalanceBefore = await starToken.balanceOf(D);
      ...
      // Check D's STAR balance increased by their requested STAR
      const STARBalanceAfter = await starToken.balanceOf(D);
      assert.isTrue(STARBalanceAfter.eq(D_STARBalBefore.add(STARRequest_D)));
    });
    ...
    it("adjustTrove(): reverts when calling address has no active trove", async () => {
      ...
      // Alice coll and debt increase(+1 ETH, +50STAR)
    ...
    it("adjustTrove(): debt increase that would reduce the ICR reverts in Recovery Mode", async () => {
      ...
      const newICR_A = await troveManager.computeICR(
        [contracts.weth.address],
        [aliceColl[0].add(aliceCollIncrease)],
    ...
    it("adjustTrove(): debt increase in Recovery Mode charges no fee", async () => {
      ...
      await openTrove({
        extraSTARAmount: toBN(dec(200000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: bob },
      });
      ...
      const sPREONSTARBalanceBefore = await starToken.balanceOf(sPREON.address);
      assert.isTrue(sPREONSTARBalanceBefore.gt(toBN("0")));
      ...
      // Check emitted fee = 0
      const emittedFee = toBN(
        await th.getEventArgByName(txAlice, "STARBorrowingFeePaid", "_STARFee")
      );
      ...
      const sPREONSTARBalanceAfter = await starToken.balanceOf(sPREON.address);
      assert.equal(
        sPREONSTARBalanceAfter.toString(),
        sPREONSTARBalanceBefore.toString()
      );
    });
    ...
    it("adjustTrove(): reverts when STAR repaid is > debt of the trove", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } });
      const bobOpenTx = (
        await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      ).tx;
      ...
      const bobFee = toBN(
        await th.getEventArgByIndex(bobOpenTx, "STARBorrowingFeePaid", 1)
      );
      assert.isTrue(bobFee.gt(toBN("0")));

      // Alice transfers STAR to bob to compensate borrowing fees
      await starToken.transfer(bob, bobFee, { from: alice });
    ...
    it("adjustTrove(): reverts when change would cause the ICR of the trove to fall below the MCR", async () => {
      ...
      // Bob attempts to increase debt by 100 STAR and 1 ether, i.e. a change that constitutes a 100% ratio of coll:debt.
    ...
    it("adjustTrove(): With 0 coll change, doesnt change borrower's coll or ActivePool coll", async () => {
      ...
      // Alice adjusts trove. No coll change, and a debt increase (+50STAR)
      await borrowerOperations.adjustTrove(
        [],
        [],
        [],
    ...
    it("adjustTrove(): With 0 debt change, doesnt change borrower's debt or ActivePool debt", async () => {
      ...
      const aliceDebtBefore = await getTroveEntireDebt(alice);
      const activePoolDebtBefore = await activePool.getSTARDebt();
      ...
      const aliceDebtAfter = await getTroveEntireDebt(alice);
      const activePoolDebtAfter = await activePool.getSTARDebt();
    ...
    it("adjustTrove(): updates borrower's debt and coll with an increase in both", async () => {
      ...
      // Alice adjusts trove. Coll and debt increase(+1 ETH, +50STAR)
      await th.addERC20(
        contracts.weth,
        alice,
        contracts.borrowerOperations.address,
      ...
      th.assertIsApproximatelyEqual(
        collAfter[0],
        collBefore[0].add(toBN(dec(1, 18))),
        10000
      );
    });
    ...
    it("adjustTrove(): updates borrower's debt and coll with a decrease in both", async () => {
      ...
      // Alice adjusts trove coll and debt decrease (-0.5 ETH, -50STAR)
      await borrowerOperations.adjustTrove(
        [],
        [],
        [contracts.weth.address],
    ...
    it("adjustTrove(): updates borrower's  debt and coll with coll increase, debt decrease", async () => {
      ...
      // Alice adjusts trove - coll increase and debt decrease (+0.5 ETH, -50STAR)
      await th.addERC20(
        contracts.weth,
        alice,
        contracts.borrowerOperations.address,
      ...
      th.assertIsApproximatelyEqual(
        collAfter[0],
        collBefore[0].add(toBN(dec(5, 17))),
        10000
      );
    });
    ...
    it("adjustTrove(): updates borrower's debt and coll with coll decrease, debt increase", async () => {
      ...
      // Alice adjusts trove - coll decrease and debt increase (0.1 ETH, 10STAR)
      await th.addERC20(
        contracts.weth,
        alice,
        contracts.borrowerOperations.address,
      ...
      th.assertIsApproximatelyEqual(
        collAfter[0],
        collBefore[0].sub(toBN(dec(1, 17))),
        10000
      );
    });
    ...
    it("adjustTrove(): updates borrower's stake and totalStakes with a coll increase", async () => {
      ...
      const stakeBefore = await troveManager.getTroveStake(
        alice,
        contracts.weth.address
      );
      const totalStakesBefore = await troveManager.getTotalStake(
        contracts.weth.address
      );
      assert.isTrue(stakeBefore.gt(toBN("0")));
      assert.isTrue(totalStakesBefore.gt(toBN("0")));

      // Alice adjusts trove - coll and debt increase (+1 ETH, +50 STAR)
      await th.addERC20(
        contracts.weth,
        alice,
        contracts.borrowerOperations.address,
      ...
      const stakeAfter = await troveManager.getTroveStake(
        alice,
        contracts.weth.address
      );
      const totalStakesAfter = await troveManager.getTotalStake(
        contracts.weth.address
      );
    ...
    it("adjustTrove(): changes STARToken balance by the requested decrease", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      assert.isTrue(
        alice_STARTokenBalance_After.eq(
          alice_STARTokenBalance_Before.sub(toBN(dec(10, 18)))
        )
      );
    });

    it("adjustTrove(): changes STARToken balance by the requested increase", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      assert.isTrue(
        alice_STARTokenBalance_After.eq(
          alice_STARTokenBalance_Before.add(toBN(dec(100, 18)))
        )
      );
    });
    ...
    it("adjustTrove(): Changes the STAR debt in ActivePool by requested decrease", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      const activePool_STARDebt_Before = await activePool.getSTARDebt();
      assert.isTrue(activePool_STARDebt_Before.gt(toBN("0")));
      ...
      const activePool_STARDebt_After = await activePool.getSTARDebt();
      assert.isTrue(
        activePool_STARDebt_After.eq(
          activePool_STARDebt_Before.sub(toBN(dec(30, 18)))
        )
      );
    });

    it("adjustTrove(): Changes the STAR debt in ActivePool by requested increase", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      await borrowerOperations.adjustTrove(
        ...
        [],
        [],
        toBN(await getNetBorrowingAmount(dec(100, 18))),
        true,
        alice,
        alice,
        th._100pct,
      ...
      th.assertIsApproximatelyEqual(
        activePool_STARDebt_After,
        activePool_STARDebt_Before.add(toBN(dec(100, 18)))
      );
    });
    ...
    it("adjustTrove(): Reverts if requested coll withdrawal is greater than trove's collateral", async () => {
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(
          ...
          [],
          [contracts.weth.address],
          [aliceColl[0].add(toBN(dec(37, "ether")))],
          0,
          false,
          bob,
          bob,
    ...
    it("adjustTrove(): Reverts if borrower has insufficient STAR balance to cover his debt repayment", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      // Bob transfers some STAR to carol
      await starToken.transfer(C, dec(10, 18), { from: B });

      //Confirm B's STAR balance is less than 50 STAR
      const B_STARBal = await starToken.balanceOf(B);
      assert.isTrue(B_STARBal.lt(bobDebt));

      const repaySTARPromise_B = borrowerOperations.adjustTrove(
        [],
        [],
        [],
        [],
      ...
      await assertRevert(repaySTARPromise_B, "revert");
    });
    ...
    //     await openTrove({ extraSTARAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    //     await openTrove({ extraSTARAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
    ...
    it("closeTrove(): reverts when it would lower the TCR below CCR", async () => {
      ...
      await starToken.transfer(alice, dec(300, 18), { from: bob });
    ...
    it("closeTrove(): reverts when system is in Recovery Mode", async () => {
      ...
      // Alice transfers her STAR to Bob and Carol so they can cover fees
      const aliceBal = await starToken.balanceOf(alice);
      await starToken.transfer(bob, aliceBal.div(toBN(2)), { from: alice });
      await starToken.transfer(carol, aliceBal.div(toBN(2)), { from: alice });
    ...
    it("closeTrove(): reverts when trove is the only one in the system", async () => {
      ...
      await starToken.unprotectedMint(alice, dec(100000, 18));

      // Check she has more STAR than her trove debt
      const aliceBal = await starToken.balanceOf(alice);
      const aliceDebt = await getTroveEntireDebt(alice);
    ...
    it("closeTrove(): reduces a Trove's collateral to zero", async () => {
      ...
      const dennisSTAR = await starToken.balanceOf(dennis);
      assert.isTrue(aliceCollBefore.gt(toBN("0")));
      assert.isTrue(dennisSTAR.gt(toBN("0")));

      // To compensate borrowing fees
      await starToken.transfer(alice, dennisSTAR.div(toBN(2)), {
        from: dennis,
      });
    ...
    it("closeTrove(): sets Trove's stake to zero", async () => {
      ...
      const aliceStakeBefore = await troveManager.getTroveStake(
        alice,
        contracts.weth.address
      );
      ...
      await borrowerOperations.closeTrove({ from: alice });
      const stakeAfter = (
        await troveManager.getTroveStake(alice, contracts.weth.address)
      ).toString();
    ...
    it("closeTrove(): zero's the troves reward snapshots", async () => {
      ...
      const L_ETH_A_Snapshot = await troveManager.getRewardSnapshotColl(
        alice,
        contracts.weth.address
      );
      const L_STARDebt_A_Snapshot = await troveManager.getRewardSnapshotSTAR(
        alice,
        contracts.weth.address
      );

      // const L_ETH_A_Snapshot = (await troveManager.rewardSnapshots(alice))[0]
      // const L_STARDebt_A_Snapshot = (await troveManager.rewardSnapshots(alice))[1]
      assert.isTrue(L_ETH_A_Snapshot.gt(toBN("0")));
      assert.isTrue(L_STARDebt_A_Snapshot.gt(toBN("0")));
      ...
      const L_ETH_Snapshot_A_AfterLiquidation = await troveManager.getRewardSnapshotColl(
        alice,
        contracts.weth.address
      );
      const L_STARDebt_Snapshot_A_AfterLiquidation = await troveManager.getRewardSnapshotSTAR(
        alice,
        contracts.weth.address
      );

      assert.isTrue(L_ETH_Snapshot_A_AfterLiquidation.gt(toBN("0")));
      assert.isTrue(L_STARDebt_Snapshot_A_AfterLiquidation.gt(toBN("0")));

      // to compensate borrowing fees
      await starToken.transfer(alice, await starToken.balanceOf(dennis), {
        from: dennis,
      });
      ...
      const L_ETH_Snapshot_A_afterAliceCloses = await troveManager.getRewardSnapshotColl(
        alice,
        contracts.weth.address
      );
      const L_STARDebt_Snapshot_A_afterAliceCloses = await troveManager.getRewardSnapshotSTAR(
        alice,
        contracts.weth.address
      );

      assert.equal(L_ETH_Snapshot_A_afterAliceCloses, "0");
      assert.equal(L_STARDebt_Snapshot_A_afterAliceCloses, "0");
    });
    ...
    it("closeTrove(): reduces ActivePool debt by correct amount", async () => {
      ...
      // Check before
      const activePool_Debt_before = await activePool.getSTARDebt();
      ...
      // Check after
      const activePool_Debt_After = (await activePool.getSTARDebt()).toString();
    ...
    it("closeTrove(): updates the the total stakes", async () => {
      ...
      const bobStakeBefore = await troveManager.getTroveStake(
        bob,
        contracts.weth.address
      );
      const dennisStakeBefore = await troveManager.getTroveStake(
        dennis,
        contracts.weth.address
      );
      ...
      // Check stake and total stakes get updated
      const aliceStakeAfter = await troveManager.getTroveStake(
        alice,
        contracts.weth.address
      );
      const totalStakesAfter = await troveManager.getTotalStake(
    ...
    it("closeTrove(): subtracts the debt of the closed Trove from the Borrower's STARToken balance", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: dennis },
      ...
      assert.isTrue(alice_STARBalance_Before.gt(toBN("0")));

      // close trove
      await borrowerOperations.closeTrove({ from: alice });

      // check alice STAR balance after
      const alice_STARBalance_After = await starToken.balanceOf(alice);
      th.assertIsApproximatelyEqual(
        alice_STARBalance_After,
        alice_STARBalance_Before.sub(aliceDebt.sub(STAR_GAS_COMPENSATION))
      );
    });
    ...
    it("closeTrove(): applies pending rewards", async () => {
      ...
      await starToken.transfer(alice, dec(10000, 18), { from: whale });
      await starToken.transfer(bob, dec(10000, 18), { from: whale });
      ...
      const [
        ...
        liquidatedColl_C,
        gasComp_C,
      ] = th.getEmittedLiquidationValues(liquidationTx, wethIDX);
      ...
      const defaultPool_STARDebt = await defaultPool.getSTARDebt();

      // Carol's liquidated coll (1 ETH) and drawn debt should have entered the Default Pool
      assert.isAtMost(th.getDifference(defaultPool_ETH, liquidatedColl_C), 100);
      assert.isAtMost(
        th.getDifference(defaultPool_STARDebt, liquidatedDebt_C),
        100
      );
      ...
      const defaultPool_STARDebt_afterAliceCloses = await defaultPool.getSTARDebt();
      ...
      assert.isAtMost(
        th.getDifference(
          defaultPool_STARDebt_afterAliceCloses,
          defaultPool_STARDebt.sub(pendingDebtReward_A)
        ),
        1000
      );
      ...
      const defaultPool_STARDebt_afterBobCloses = await defaultPool.getSTARDebt();
      ...
      assert.isAtMost(
        th.getDifference(defaultPool_STARDebt_afterBobCloses, 0),
        100000
      );
    });

    it("closeTrove(): reverts if borrower has insufficient STAR balance to repay his entire debt", async () => {
      await openTrove({
        extraSTARAmount: toBN(dec(15000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      ...
      //Confirm Bob's STAR balance is less than his trove debt
      const B_STARBal = await starToken.balanceOf(B);
      const B_troveDebt = await getTroveEntireDebt(B);

      assert.isTrue(B_STARBal.lt(B_troveDebt));
      ...
      await assertRevert(
        closeTrovePromise_B,
        "BorrowerOps: Caller doesnt have enough STAR to make repayment"
      );
    });
    ...
    if (!withProxy) {
      ...
      it("openTrove(): emits a TroveUpdated event with the correct collateral and debt", async () => {
        ...
        const D_emittedColl = toBN(
          th.getEventArgByName(txD, "TroveUpdated", "_amounts")[0]
        );
        ...
        const E_emittedColl = toBN(
          th.getEventArgByName(txE, "TroveUpdated", "_amounts")[0]
        );
    ...
    it("openTrove(): Opens a trove with net debt >= minimum net debt", async () => {
      ...
      const txA = await contracts.borrowerOperations.openTrove(
        th._100pct,
        await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN(1))),
        A,
        A,
      ...
      const txC = await contracts.borrowerOperations.openTrove(
        th._100pct,
    ...
    it("openTrove(): reverts if net debt < minimum net debt", async () => {
      ...
      const txAPromise = contracts.borrowerOperations.openTrove(
        th._100pct,
        0,
        A,
        A,
      ...
      const txBPromise = contracts.borrowerOperations.openTrove(
        th._100pct,
      ...
      const txCPromise = contracts.borrowerOperations.openTrove(
        th._100pct,
    ...
    it("openTrove(): decays a non-zero base rate", async () => {
      ...
      // E opens trove
      await openTrove({
        extraSTARAmount: toBN(dec(12, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: E },
      });
    ...
    xit("openTrove(): reverts if fee exceeds max fee percentage", async () => {
      // new fee system takes into account the max of ether and debt.
      await openTrove({
        extraSTARAmount: toBN(dec(20000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
    ...
    it("openTrove(): succeeds when fee is less than max fee percentage", async () => {
      ...
      const tx1 = await contracts.borrowerOperations.openTrove(
        moreThan5pct,
        dec(10000, 18),
        A,
        A,
      ...
      const tx2 = await contracts.borrowerOperations.openTrove(
        dec(5, 16),
        dec(10000, 18),
        A,
        A,
      ...
      const tx3 = await contracts.borrowerOperations.openTrove(
        dec(1, 17),
        dec(10000, 18),
        A,
        A,
      ...
      const tx4 = await contracts.borrowerOperations.openTrove(
        dec(37659, 13),
        dec(10000, 18),
        A,
        A,
      ...
      const tx5 = await contracts.borrowerOperations.openTrove(
        dec(1, 18),
        dec(10000, 18),
        A,
        A,
    ...
    it("openTrove(): borrowing at non-zero base rate sends STAR fee to sPREON contract", async () => {
      // time fast-forwards 1 year, and E stakes 1 PREON
      await th.fastForwardTime(
        timeValues.SECONDS_IN_ONE_YEAR,
        web3.currentProvider
    ...
    if (!withProxy) {
      ...
      it("openTrove(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Trove struct", async () => {
        ...
        const D_STARRequest = toBN(dec(20000, 18));
        ...
        const openTroveTx = await contracts.borrowerOperations.openTrove(
          th._100pct,
          D_STARRequest,
          ZERO_ADDRESS,
          ZERO_ADDRESS,
        ...
        // borrowerOperations.openTrove(th._100pct, D_STARRequest, ZERO_ADDRESS, ZERO_ADDRESS, { from: D, value: dec(200, 'ether') })

        const emittedFee = toBN(
          th.getSTARFeeFromSTARBorrowingEvent(openTroveTx)
        );
        ...
        th.assertIsApproximatelyEqual(
          newDebt,
          D_STARRequest.add(emittedFee).add(STAR_GAS_COMPENSATION),
          100000
        );
      });
    }
    ...
    // @KingPreon: F_STAR() function no longer exists
    // it("openTrove(): Borrowing at non-zero base rate increases the sPREON contract STAR fees-per-unit-staked", async () => {
    //   // time fast-forwards 1 year, and E stakes 1 PREON
    ...
    //
    //   await openTrove({ extraSTARAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    //   await openTrove({ extraSTARAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
    //   await openTrove({ extraSTARAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
    //   await openTrove({ extraSTARAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    //
    //   // Artificially make baseRate 5%
    //   await troveManager.setBaseRate(dec(5, 16))
    //   await troveManager.setLastFeeOpTimeToNow()
    ...
    //
    //   // D opens trove
    //   await openTrove({ extraSTARAmount: toBN(dec(37, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    //
    //   // Check PREON contract STAR fees-per-unit-staked has increased
    //   const F_STAR_After = await sPREON.F_STAR()
    //   assert.isTrue(F_STAR_After.gt(F_STAR_Before))
    ...
    it("openTrove(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      // D opens trove
      const STARRequest_D = toBN(dec(40000, 18));
      await th.addERC20(
        contracts.weth,
        D,
        contracts.borrowerOperations.address,
      ...
      // await borrowerOperations.openTrove(th._100pct, STARRequest_D, D, D, { from: D, value: dec(500, 'ether') })
      ...
      // Check D's STAR balance now equals their requested STAR
      const STARBalance_D = await starToken.balanceOf(D);
      assert.isTrue(STARRequest_D.eq(STARBalance_D));
    });

    // @KingPreon: no longer using F_STAR
    // it("openTrove(): Borrowing at zero base rate changes the sPREON contract STAR fees-per-unit-staked", async () => {
    //   await openTrove({ extraSTARAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
    //   await openTrove({ extraSTARAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
    //   await openTrove({ extraSTARAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    //
    //   // Check baseRate is zero
    //   const baseRate_1 = await troveManager.baseRate()
    //   assert.equal(baseRate_1, '0')
    ...
    //   th.fastForwardTime(7200, web3.currentProvider)
    //
    //   // Check STAR reward per PREON staked == 0
    //   const F_STAR_Before = await starToken.balanceOf(sPREON.address)
    //   assert.equal(F_STAR_Before, '0')
    //
    //   // A stakes PREON
    ...
    //   // Check STAR reward per PREON staked > 0
    //   const F_STAR_After = await starToken.balanceOf(sPREON.address)
    //   assert.isTrue(F_STAR_After.gt(toBN('0')))
    // })
    ...
    it("openTrove(): Borrowing at zero base rate charges minimum fee", async () => {
      ...
      const STARRequest = toBN(dec(10000, 18));
      await th.addERC20(
        contracts.weth,
        C,
        contracts.borrowerOperations.address,
      ...
      // const txC = await borrowerOperations.openTrove(th._100pct, STARRequest, ZERO_ADDRESS, ZERO_ADDRESS, { value: dec(100, 'ether'), from: C })
      const _STARFee = toBN(
        th.getEventArgByName(txC, "STARBorrowingFeePaid", "_STARFee")
      );

      const expectedFee = BORROWING_FEE_FLOOR.mul(toBN(STARRequest)).div(
        toBN(dec(1, 18))
      );
      assert.isTrue(_STARFee.eq(expectedFee));
    });
    ...
    it("openTrove(): Can open a trove with ICR >= CCR when system is in Recovery Mode", async () => {
      ...
      // price drops to 1ETH:100STAR, reducing TCR below 150%
      await priceFeed.setPrice("100000000000000000000");
      const price = await priceFeed.getPrice();
    ...
    it("openTrove(): creates a new Trove and assigns the correct collateral and debt amount", async () => {
      ...
      const STARRequest = MIN_NET_DEBT;
      await th.addERC20(
        contracts.weth,
        alice,
        contracts.borrowerOperations.address,
      ...
      // Get the expected debt based on the STAR request (adding fee and liq. reserve on top)
      const expectedDebt = STARRequest.add(
        await troveManager.getBorrowingFee(STARRequest)
      ).add(STAR_GAS_COMPENSATION);
    ...
    // TODO
    it("openTrove(): records up-to-date initial snapshots of L_ETH and L_STARDebt", async () => {
      // --- SETUP ---
      ...
      /* with total stakes = 10 ether, after liquidation, L_ETH should equal 1/10 ether per-ether-staked,
       and L_STAR should equal 18 STAR per-ether-staked. */

      // const L_ETH = await troveManager.L_ETH()
      const L_ETH = await troveManager.getL_Coll(contracts.weth.address);
      const L_STAR = await troveManager.L_STARDebt(contracts.weth.address);

      assert.isTrue(L_ETH.gt(toBN("0")));
      assert.isTrue(L_STAR.gt(toBN("0")));
      ...
      // Check Bob's snapshots of L_ETH and L_STAR equal the respective current values
      // const bob_rewardSnapshot = await troveManager.rewardSnapshots(bob)
      const bob_ETHrewardSnapshot = await troveManager.getRewardSnapshotColl(
        bob,
        contracts.weth.address
      ); // bob_rewardSnapshot[0]
      const bob_STARDebtRewardSnapshot = await troveManager.getRewardSnapshotSTAR(
        bob,
        contracts.weth.address
      );
      ...
      assert.isAtMost(
        th.getDifference(bob_STARDebtRewardSnapshot, L_STAR),
        1000
      );
    });
    ...
    it("openTrove(): increases the Trove's STAR debt by the correct amount", async () => {
      // check before
      // const alice_Trove_Before = await troveManager.Troves(alice)
      ...
      // borrowerOperations.openTrove(th._100pct, await getOpenTroveSTARAmount(dec(10000, 18)), alice, alice, { from: alice, value: dec(100, 'ether') })
    ...
    it("openTrove(): increases STAR debt in ActivePool by the debt of the trove", async () => {
      const activePool_STARDebt_Before = await activePool.getSTARDebt();
      assert.equal(activePool_STARDebt_Before, 0);
      ...
      const activePool_STARDebt_After = await activePool.getSTARDebt();
      assert.isTrue(activePool_STARDebt_After.eq(aliceDebt));
    });

    it("openTrove(): increases user STARToken balance by correct amount", async () => {
      // check before
      const alice_STARTokenBalance_Before = await starToken.balanceOf(alice);
      assert.equal(alice_STARTokenBalance_Before, 0);
      ...
      // check after
      const alice_STARTokenBalance_After = await starToken.balanceOf(alice);
      assert.equal(alice_STARTokenBalance_After, dec(10000, 18));
    });
    ...
    describe("getNewICRFromTroveChange() returns the correct ICR", async () => {
      ...
      it("collChange = 0, debtChange = 0", async () => {
        ...
        const newVC = (
          await borrowerOperations.getVC(
            [contracts.weth.address],
            [initialColl]
          )
        ).toString();
    ...
    it("getCompositeDebt(): returns debt + gas comp", async () => {
      ...
      assert.equal(res1, STAR_GAS_COMPENSATION.toString());

      const res2 = await borrowerOperations.getCompositeDebt(dec(90, 18));
      th.assertIsApproximatelyEqual(
        res2,
        STAR_GAS_COMPENSATION.add(toBN(dec(90, 18)))
      );
      ...
      th.assertIsApproximatelyEqual(
        res3,
        STAR_GAS_COMPENSATION.add(toBN(dec(24423422357345049, 12)))
      );
    });
    ...
    describe("getNewTCRFromTroveChange() returns the correct TCR", async () => {
      ...
      it("collChange = 0, debtChange = 0", async () => {
        ...
        const troveSTARAmount = await getOpenTroveSTARAmount(troveTotalDebt);
        ...
        // await borrowerOperations.openTrove(th._100pct, troveSTARAmount, alice, alice, { from: alice, value: troveColl })
        // await borrowerOperations.openTrove(th._100pct, troveSTARAmount, bob, bob, { from: bob, value: troveColl })
        ...
        const newVC = await borrowerOperations.getVC(
          [contracts.weth.address],
          [troveColl.add(liquidatedCollAmount)]
        );
        const expectedTCR = newVC
          .mul(toBN(dec(1, 18)))
      ...
      it("collChange is positive, debtChange is 0", async () => {
        ...
        const collChangeVC = await borrowerOperations.getVC(
          [contracts.weth.address],
          [collChange]
        );
        const newTCR = await borrowerOperations.getNewTCRFromTroveChange(
