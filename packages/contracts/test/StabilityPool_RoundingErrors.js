...
contract("Pool Manager: Sum-Product rounding errors", async (accounts) => {
  ...
  it.skip("Rounding errors: 100 deposits of 100STAR into SP, then 200 liquidations of 49STAR", async () => {
    ...
    const SP_TotalDeposits = await stabilityPool.getTotalSTARDeposits();
    const SP_ETH = await stabilityPool.getETH();
    const compoundedDeposit = await stabilityPool.getCompoundedSTARDeposit(
      depositors[0]
    );
