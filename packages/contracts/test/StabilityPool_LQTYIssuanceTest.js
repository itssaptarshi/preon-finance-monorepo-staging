...
contract("StabilityPool - PREON Rewards", async (accounts) => {
  ...
  describe("PREON Rewards", async () => {
    beforeEach(async () => {
      ...
      communityPREONSupply = toBN(
        await preonToken.balanceOf(communityIssuanceTester.address)
      );
      assert.isAtMost(
        getDifference(communityPREONSupply, "32000000000000000000000000"),
        1000
      );
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct PREON gain. No liquidations. No front end.", async () => {
      ...
      const expectedPREONGain_1yr = communityPREONSupply
        .div(toBN("2"))
        .div(toBN("3"));
      ...
      assert.isAtMost(
        getDifference(A_PREONGain_1yr, expectedPREONGain_1yr),
        1e12
      );
      assert.isAtMost(
        getDifference(B_PREONGain_1yr, expectedPREONGain_1yr),
        1e12
      );
      assert.isAtMost(
        getDifference(C_PREONGain_1yr, expectedPREONGain_1yr),
        1e12
      );
      ...
      const expectedPREONGain_2yr = communityPREONSupply
        .mul(toBN("3"))
        .div(toBN("4"))
        .div(toBN("3"));
      ...
      // Check gains are correct, error tolerance = 1e-6 of a token
      assert.isAtMost(
        getDifference(A_PREONGain_2yr, expectedPREONGain_2yr),
        1e12
      );
      assert.isAtMost(
        getDifference(B_PREONGain_2yr, expectedPREONGain_2yr),
        1e12
      );
      assert.isAtMost(
        getDifference(C_PREONGain_2yr, expectedPREONGain_2yr),
        1e12
      );
      ...
      assert.isAtMost(
        getDifference(await preonToken.balanceOf(A), expectedPREONGain_2yr),
        1e12
      );
      assert.isAtMost(
        getDifference(await preonToken.balanceOf(B), expectedPREONGain_2yr),
        1e12
      );
      assert.isAtMost(
        getDifference(await preonToken.balanceOf(C), expectedPREONGain_2yr),
        1e12
      );
    });
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct PREON gain. No liquidations. No front end.", async () => {
      ...
      assert.isAtMost(
        getDifference(A_PREONGain_1yr, A_expectedPREONGain_1yr),
        1e12
      );
      assert.isAtMost(
        getDifference(B_PREONGain_1yr, B_expectedPREONGain_1yr),
        1e12
      );
      assert.isAtMost(
        getDifference(C_PREONGain_1yr, C_expectedPREONGain_1yr),
        1e12
      );
      ...
      // Check gains are correct, error tolerance = 1e-6 of a token
      assert.isAtMost(
        getDifference(A_PREONGain_2yr, A_expectedPREONGain_2yr),
        1e12
      );
      assert.isAtMost(
        getDifference(B_PREONGain_2yr, B_expectedPREONGain_2yr),
        1e12
      );
      assert.isAtMost(
        getDifference(C_PREONGain_2yr, C_expectedPREONGain_2yr),
        1e12
      );
      ...
      // Check PREON balances increase by correct amount
      assert.isAtMost(
        getDifference(await preonToken.balanceOf(A), A_expectedPREONGain_2yr),
        1e12
      );
      assert.isAtMost(
        getDifference(await preonToken.balanceOf(B), B_expectedPREONGain_2yr),
        1e12
      );
      assert.isAtMost(
        getDifference(await preonToken.balanceOf(C), C_expectedPREONGain_2yr),
        1e12
      );
    });
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct PREON gain. No liquidations. No front end.", async () => {
      ...
      assert.equal(await stabilityPool.getTotalSTARDeposits(), dec(60000, 18));
      ...
      // Check gains are correct, error tolerance = 1e-6 of a toke
      assert.isAtMost(
        getDifference(A_PREONGain_Y1, A_expectedPREONGain_Y1),
        1e12
      );
      assert.isAtMost(
        getDifference(B_PREONGain_Y1, B_expectedPREONGain_Y1),
        1e12
      );
      assert.isAtMost(
        getDifference(C_PREONGain_Y1, C_expectedPREONGain_Y1),
        1e12
      );
      ...
      const A_expectedTotalGain = A_expectedPREONGain_Y1.add(
        A_expectedPREONGain_Y2
      );
      const B_expectedTotalGain = B_expectedPREONGain_Y1.add(
        B_expectedPREONGain_Y2
      );
      const C_expectedTotalGain = C_expectedPREONGain_Y1.add(
        C_expectedPREONGain_Y2
      );
      const D_expectedTotalGain = D_expectedPREONGain_Y2;

      // Check gains are correct, error tolerance = 1e-6 of a token
      assert.isAtMost(
        getDifference(A_PREONGain_AfterY2, A_expectedTotalGain),
        1e12
      );
      assert.isAtMost(
        getDifference(B_PREONGain_AfterY2, B_expectedTotalGain),
        1e12
      );
      assert.isAtMost(
        getDifference(C_PREONGain_AfterY2, C_expectedTotalGain),
        1e12
      );
      assert.isAtMost(
        getDifference(D_PREONGain_AfterY2, D_expectedTotalGain),
        1e12
      );
      ...
      // Check PREON balances increase by correct amount
      assert.isAtMost(
        getDifference(await preonToken.balanceOf(A), A_expectedTotalGain),
        1e12
      );
      assert.isAtMost(
        getDifference(await preonToken.balanceOf(B), B_expectedTotalGain),
        1e12
      );
      assert.isAtMost(
        getDifference(await preonToken.balanceOf(C), C_expectedTotalGain),
        1e12
      );
      assert.isAtMost(
        getDifference(await preonToken.balanceOf(D), D_expectedTotalGain),
        1e12
      );
    });
    ...
    it("withdrawFromSP(): Depositor withdraws correct PREON gain after serial pool-emptying liquidations. No front-ends.", async () => {
      ...
      // 4 Defaulters open trove with 200STAR debt, and 200% ICR
      await borrowerOperations.openTrove(
        th._100pct,
      ...
      // Defaulter 2 liquidated. 10k STAR offset
      await troveManager.liquidate(defaulter_2, { from: owner });

      // Erin, Flyn each deposit 100 STAR
      ...
      // Defaulter 3 liquidated. 100 STAR offset
      await troveManager.liquidate(defaulter_3, { from: owner });

      // Graham, Harriet each deposit 10k STAR
      ...
      // Defaulter 4 liquidated. 100 STAR offset
      ...
      for (depositor of [A, B]) {
        ...
        assert.isAtMost(
          getDifference(PREONBalance, expectedPREONGain_M1),
          1e15
        );
      }
      ...
      for (depositor of [C, D]) {
        const PREONBalance = await preonToken.balanceOf(depositor);
        assert.isAtMost(
          getDifference(PREONBalance, expectedPREONGain_M2),
          1e15
        );
      }
      ...
      for (depositor of [E, F]) {
        const PREONBalance = await preonToken.balanceOf(depositor);
        assert.isAtMost(
          getDifference(PREONBalance, expectedPREONGain_M3),
          1e15
        );
      }
      ...
      for (depositor of [G, H]) {
        const PREONBalance = await preonToken.balanceOf(depositor);
        assert.isAtMost(
          getDifference(PREONBalance, expectedPREONGain_M4),
          1e15
        );
      }
    ...
    it("PREON issuance for a given period is not obtainable if the SP was empty during the period", async () => {
      const CIBalanceBefore = await preonToken.balanceOf(
      ...
      const expectedIssuance4Months = issuance_M1
        ...
        .add(issuance_M4);
      assert.isAtMost(
        getDifference(expectedIssuance4Months, totalPREONissuance_4),
        1e15
      );
      ...
      const CIBalanceAfter = await preonToken.balanceOf(
        communityIssuanceTester.address
      );
      const CIBalanceDifference = CIBalanceBefore.sub(CIBalanceAfter);
      assert.isAtMost(
        getDifference(CIBalanceDifference, expectedPREONSentOutFromCI),
        1e15
      );
    });
    ...
    A make deposit 10k STAR
    1 month passes. L1 decreases P: P = 1e-5 P. L1:   9999.9 STAR, 100 ETH
    B makes deposit 9999.9
    1 month passes. L2 decreases P: P =  1e-5 P. L2:  9999.9 STAR, 100 ETH
    C makes deposit  9999.9
    1 month passes. L3 decreases P: P = 1e-5 P. L3:  9999.9 STAR, 100 ETH
    D makes deposit  9999.9
    1 month passes. L4 decreases P: P = 1e-5 P. L4:  9999.9 STAR, 100 ETH
    E makes deposit  9999.9
    1 month passes. L5 decreases P: P = 1e-5 P. L5:  9999.9 STAR, 100 ETH
    =========
    F makes deposit 100
    1 month passes. L6 empties the Pool. L6:  10000 STAR, 100 ETH

    expect A, B, C, D each withdraw ~1 month's worth of PREON */
    it("withdrawFromSP(): Several deposits of 100 STAR span one scale factor change. Depositors withdraw correct PREON gains", async () => {
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct PREON gain. No liquidations. Front ends and kickback rates.", async () => {
      ...
      const F1_PREONGain_Y1 = await stabilityPool.getFrontEndPREONGain(
        frontEnd_1
      );
      const F2_PREONGain_Y1 = await stabilityPool.getFrontEndPREONGain(
        frontEnd_2
      );
      ...
      const expectedFinalIssuance = expectedIssuance_Y1.add(
        expectedIssuance_Y2
      );
      ...
      // Check PREON balances increase by correct amount
      assert.isAtMost(
        getDifference(await preonToken.balanceOf(A), A_expectedFinalGain),
        1e12
      );
      assert.isAtMost(
        getDifference(await preonToken.balanceOf(B), B_expectedFinalGain),
        1e12
      );
      assert.isAtMost(
        getDifference(await preonToken.balanceOf(C), C_expectedFinalGain),
        1e12
      );
      assert.isAtMost(
        getDifference(await preonToken.balanceOf(D), D_expectedFinalGain),
        1e12
      );
      assert.isAtMost(
        getDifference(
          await preonToken.balanceOf(frontEnd_1),
      ...
      assert.isAtMost(
        getDifference(
          await preonToken.balanceOf(frontEnd_2),
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct PREON gain. Front ends and kickback rates", async () => {
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveSTARAmount(dec(50000, 18)),
        defaulter_1,
        defaulter_1,
      ...
      assert.equal(await stabilityPool.getTotalSTARDeposits(), dec(100000, 18)); // total 100k
      ...
      const A_expectedPREONGain_M1 = F1_kickbackRate.mul(A_share_M1).div(
        toBN(dec(1, 18))
      );

      const B_share_M1 = issuance_M1.mul(toBN("20000")).div(toBN("100000"));
      const B_expectedPREONGain_M1 = F2_kickbackRate.mul(B_share_M1).div(
        toBN(dec(1, 18))
      );

      const C_share_M1 = issuance_M1.mul(toBN("30000")).div(toBN("100000"));
      const C_expectedPREONGain_M1 = F2_kickbackRate.mul(C_share_M1).div(
        toBN(dec(1, 18))
      );
      ...
      const F1_PREONGain_M1 = await stabilityPool.getFrontEndPREONGain(
        frontEnd_1
      );
      const F2_PREONGain_M1 = await stabilityPool.getFrontEndPREONGain(
        frontEnd_2
      );

      // Check gains are correct, error tolerance = 1e-3 of a token
      assert.isAtMost(
        getDifference(A_PREONGain_M1, A_expectedPREONGain_M1),
        1e15
      );
      assert.isAtMost(
        getDifference(B_PREONGain_M1, B_expectedPREONGain_M1),
        1e15
      );
      assert.isAtMost(
        getDifference(C_PREONGain_M1, C_expectedPREONGain_M1),
        1e15
      );
      assert.isAtMost(
        getDifference(D_PREONGain_M1, D_expectedPREONGain_M1),
        1e15
      );
      assert.isAtMost(
        getDifference(F1_PREONGain_M1, F1_expectedPREONGain_M1),
        1e15
      );
      assert.isAtMost(
        getDifference(F2_PREONGain_M1, F2_expectedPREONGain_M1),
        1e15
      );
      ...
      const A_expectedPREONGain_M2 = F1_kickbackRate.mul(A_share_M2).div(
        toBN(dec(1, 18))
      );

      const B_share_M2 = issuance_M2.mul(toBN("10000")).div(toBN("80000"));
      const B_expectedPREONGain_M2 = F2_kickbackRate.mul(B_share_M2).div(
        toBN(dec(1, 18))
      );

      const C_share_M2 = issuance_M2.mul(toBN("15000")).div(toBN("80000"));
      const C_expectedPREONGain_M2 = F2_kickbackRate.mul(C_share_M2).div(
        toBN(dec(1, 18))
      );
      ...
      const E_expectedPREONGain_M2 = F1_kickbackRate.mul(E_share_M2).div(
        toBN(dec(1, 18))
      );
      ...
      const F1_PREONGain_After_M2 = await stabilityPool.getFrontEndPREONGain(
        frontEnd_1
      );
      const F2_PREONGain_After_M2 = await stabilityPool.getFrontEndPREONGain(
        frontEnd_2
      );

      assert.isAtMost(
        getDifference(
          A_PREONGain_After_M2,
          A_expectedPREONGain_M2.add(A_expectedPREONGain_M1)
        ),
        1e15
      );
      assert.isAtMost(
        getDifference(
          B_PREONGain_After_M2,
          B_expectedPREONGain_M2.add(B_expectedPREONGain_M1)
        ),
        1e15
      );
      assert.isAtMost(
        getDifference(
          C_PREONGain_After_M2,
          C_expectedPREONGain_M2.add(C_expectedPREONGain_M1)
        ),
        1e15
      );
      assert.isAtMost(
        getDifference(
          D_PREONGain_After_M2,
          D_expectedPREONGain_M2.add(D_expectedPREONGain_M1)
        ),
        1e15
      );
      assert.isAtMost(
        getDifference(E_PREONGain_After_M2, E_expectedPREONGain_M2),
        1e15
      );
      ...
      assert.isAtMost(
        getDifference(F1_PREONBalance_After_M2, F1_expectedPREONGain_M1),
        1e15
      );

      // Check F1's PREON gain in system after M2: Just their gain due to M2
      assert.isAtMost(
        getDifference(F1_PREONGain_After_M2, F1_expectedPREONGain_M2),
        1e15
      );
      ...
      assert.isAtMost(
        getDifference(
          F2_PREONGain_After_M2,
          F2_expectedPREONGain_M2.add(F2_expectedPREONGain_M1)
        ),
        1e15
      );
      ...
      const A_expectedPREONGain_M3 = F1_kickbackRate.mul(A_share_M3).div(
        toBN(dec(1, 18))
      );

      const B_share_M3 = issuance_M3.mul(toBN("47500")).div(toBN("100000"));
      const B_expectedPREONGain_M3 = F2_kickbackRate.mul(B_share_M3).div(
        toBN(dec(1, 18))
      );

      const C_share_M3 = issuance_M3.mul(toBN("11250")).div(toBN("100000"));
      const C_expectedPREONGain_M3 = F2_kickbackRate.mul(C_share_M3).div(
        toBN(dec(1, 18))
      );
      ...
      const E_expectedPREONGain_M3 = F1_kickbackRate.mul(E_share_M3).div(
        toBN(dec(1, 18))
      );
      ...
      const F1_PREONGain_After_M3 = await stabilityPool.getFrontEndPREONGain(
        frontEnd_1
      );
      const F2_PREONGain_After_M3 = await stabilityPool.getFrontEndPREONGain(
        frontEnd_2
      );
      ...
      assert.isAtMost(
        getDifference(
          A_PREONGain_After_M3,
          A_expectedPREONGain_M3.add(A_expectedPREONGain_M2).add(
            A_expectedPREONGain_M1
          )
        ),
        1e15
      );
      assert.isAtMost(
        getDifference(
          C_PREONGain_After_M3,
          C_expectedPREONGain_M3.add(C_expectedPREONGain_M2).add(
            C_expectedPREONGain_M1
          )
        ),
        1e15
      );
      assert.isAtMost(
        getDifference(
          D_PREONGain_After_M3,
          D_expectedPREONGain_M3.add(D_expectedPREONGain_M2).add(
            D_expectedPREONGain_M1
          )
        ),
        1e15
      );
      ...
      assert.isAtMost(
        getDifference(
          E_PREONGain_After_M3,
          E_expectedPREONGain_M3.add(E_expectedPREONGain_M2)
        ),
        1e15
      );
      ...
      assert.isAtMost(
        getDifference(B_PREONGain_After_M3, B_expectedPREONGain_M3),
        1e15
      );
      ...
      assert.isAtMost(
        getDifference(
          F1_PREONGain_After_M3,
          F1_expectedPREONGain_M3.add(F1_expectedPREONGain_M2)
        ),
        1e15
      );
      ...
      assert.isAtMost(
        getDifference(F1_PREONBalance_After_M3, F1_expectedPREONGain_M1),
        1e15
      );

      // Expect F2 PREON system gains to equal their gain from M3
      assert.isAtMost(
        getDifference(F2_PREONGain_After_M3, F2_expectedPREONGain_M3),
        1e15
      );
      ...
      assert.isAtMost(
        getDifference(
          F2_PREONBalance_After_M3,
          F2_expectedPREONGain_M2.add(F2_expectedPREONGain_M1)
        ),
        1e15
      );

      // Expect deposit C now to be 10125 STAR
      const C_compoundedSTARDeposit = await stabilityPool.getCompoundedSTARDeposit(
        C
      );
      assert.isAtMost(
        getDifference(C_compoundedSTARDeposit, dec(10125, 18)),
        1000
      );
      ...
      for (depositor of [A, B, C, D, E]) {
        ...
        const compoundedSTARDeposit = await stabilityPool.getCompoundedSTARDeposit(
          depositor
        );
        assert.equal(compoundedSTARDeposit, "0");
      }
      ...
      const A_expectedPREONGain_M4 = F1_kickbackRate.mul(A_share_M4).div(
        toBN(dec(1, 18))
      );

      const B_share_M4 = issuance_M4.mul(toBN("42750")).div(toBN("80000")); // 42750/80000
      const B_expectedPREONGain_M4 = F2_kickbackRate.mul(B_share_M4).div(
        toBN(dec(1, 18))
      );

      const C_share_M4 = issuance_M4.mul(toBN("125")).div(toBN("80000")); // 125/80000
      const C_expectedPREONGain_M4 = F2_kickbackRate.mul(C_share_M4).div(
        toBN(dec(1, 18))
      );
      ...
      const E_expectedPREONGain_M4 = F1_kickbackRate.mul(E_share_M4).div(
        toBN(dec(1, 18))
      );
      ...
      const A_expectedFinalPREONBalance = A_expectedPREONGain_M1.add(
        A_expectedPREONGain_M2
      )
        .add(A_expectedPREONGain_M3)
        .add(A_expectedPREONGain_M4);

      const B_expectedFinalPREONBalance = B_expectedPREONGain_M1.add(
        B_expectedPREONGain_M2
      )
        .add(B_expectedPREONGain_M3)
        .add(B_expectedPREONGain_M4);

      const C_expectedFinalPREONBalance = C_expectedPREONGain_M1.add(
        C_expectedPREONGain_M2
      )
        .add(C_expectedPREONGain_M3)
        .add(C_expectedPREONGain_M4);

      const D_expectedFinalPREONBalance = D_expectedPREONGain_M1.add(
        D_expectedPREONGain_M2
      )
        .add(D_expectedPREONGain_M3)
        .add(D_expectedPREONGain_M4);

      const E_expectedFinalPREONBalance = E_expectedPREONGain_M2.add(
        E_expectedPREONGain_M3
      ).add(E_expectedPREONGain_M4);

      const F1_expectedFinalPREONBalance = F1_expectedPREONGain_M1.add(
        F1_expectedPREONGain_M2
      )
        .add(F1_expectedPREONGain_M3)
        .add(F1_expectedPREONGain_M4);

      const F2_expectedFinalPREONBalance = F2_expectedPREONGain_M1.add(
        F2_expectedPREONGain_M2
      )
        .add(F2_expectedPREONGain_M3)
        .add(F2_expectedPREONGain_M4);

      assert.isAtMost(
        getDifference(A_FinalPREONBalance, A_expectedFinalPREONBalance),
        1e15
      );
      assert.isAtMost(
        getDifference(B_FinalPREONBalance, B_expectedFinalPREONBalance),
        1e15
      );
      assert.isAtMost(
        getDifference(C_FinalPREONBalance, C_expectedFinalPREONBalance),
        1e15
      );
      assert.isAtMost(
        getDifference(D_FinalPREONBalance, D_expectedFinalPREONBalance),
        1e15
      );
      assert.isAtMost(
        getDifference(E_FinalPREONBalance, E_expectedFinalPREONBalance),
        1e15
      );
      assert.isAtMost(
        getDifference(F1_FinalPREONBalance, F1_expectedFinalPREONBalance),
        1e15
      );
      assert.isAtMost(
        getDifference(F2_FinalPREONBalance, F2_expectedFinalPREONBalance),
        1e15
      );
    });
    ...
    A, B make deposit 5000 STAR via F1
    1 month passes. L1 depletes P: P = 1e-5*P L1:  9999.9 STAR, 1 ETH.  scale = 0
    C makes deposit 10000  via F1
    1 month passes. L2 depletes P: P = 1e-5*P L2:  9999.9 STAR, 1 ETH  scale = 1
    D makes deposit 10000 via F1
    1 month passes. L3 depletes P: P = 1e-5*P L3:  9999.9 STAR, 1 ETH scale = 1
    E makes deposit 10000 via F1
    1 month passes. L3 depletes P: P = 1e-5*P L4:  9999.9 STAR, 1 ETH scale = 2
    A, B, C, D, E withdraw

    =========
    Expect front end withdraws ~3 month's worth of PREON */

    it("withdrawFromSP(): Several deposits of 10k STAR span one scale factor change. Depositors withdraw correct PREON gains", async () => {
     ...
     */
      ...
      const expectedPREONGain_A_and_B = kickbackRate
        .mul(issuance_M1)
        .div(toBN("2"))
      ...
      const expectedPREONGain_C = kickbackRate
        .mul(issuance_M2)
      ...
      const expectedPREONGain_D = kickbackRate
        .mul(issuance_M3)
      ...
      const expectedPREONGain_E = kickbackRate
        .mul(issuance_M4)
      ...
      const issuance1st4Months = issuance_M1
        ...
        .add(issuance_M3)
        .add(issuance_M4);
      const expectedPREONGain_F1 = toBN(dec(1, 18))
        .sub(kickbackRate)
        .mul(issuance1st4Months)
        .div(toBN(dec(1, 18)));

      assert.isAtMost(
        getDifference(expectedPREONGain_A_and_B, PREONGain_A),
        1e15
      );
      assert.isAtMost(
        getDifference(expectedPREONGain_A_and_B, PREONGain_B),
        1e15
      );
