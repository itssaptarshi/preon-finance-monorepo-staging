...
contract("TroveManager", async (accounts) => {
  ...
  const getOpenTroveTotalDebt = async (starAmount) =>
    th.getOpenTroveTotalDebt(contracts, starAmount);
  ...
  const withdrawSTAR = async (params) => th.withdrawSTAR(contracts, params);
  ...
  it("liquidate(): closes a Trove that has ICR < MCR", async () => {
    ...
    // Alice increases debt to 180 STAR, lowering her ICR to 1.11
    const A_STARWithdrawal = await getNetBorrowingAmount(dec(130, 18));

    const targetICR = toBN("1111111111111111111");
    await withdrawSTAR({ ICR: targetICR, extraParams: { from: alice } });
    ...
    // price drops to 1ETH:100STAR, reducing Alice's ICR below MCR
  ...
  it("liquidate(): decreases ActivePool ETH and STARDebt by correct amounts", async () => {
    // --- SETUP ---
    const {
    ...
    // check ActivePool ETH and STAR debt before
    const activePool_ETH_Before = (
      await activePool.getCollateral(weth.address)
    ).toString();
    const activePool_RawEther_Before = (
      await weth.balanceOf(activePool.address)
    ).toString();
    const activePool_STARDebt_Before = (
      await activePool.getSTARDebt()
    ).toString();
    ...
    th.assertIsApproximatelyEqual(
      activePool_STARDebt_Before,
      A_totalDebt.add(B_totalDebt)
    );

    // price drops to 1ETH:100STAR, reducing Bob's ICR below MCR
    await priceFeed.setPrice("100000000000000000000");

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    /* close Bob's Trove. Should liquidate his ether and STAR, 
    leaving Alice’s ether and STAR debt in the ActivePool. */
    await troveManager.liquidate(bob, { from: owner });

    // check ActivePool ETH and STAR debt
    const activePool_ETH_After = (
      await activePool.getCollateral(weth.address)
    ).toString();
    const activePool_RawEther_After = (
      await weth.balanceOf(activePool.address)
    ).toString();
    const activePool_STARDebt_After = (
      await activePool.getSTARDebt()
    ).toString();

    assert.equal(activePool_ETH_After, A_collateral);
    assert.equal(activePool_RawEther_After, A_collateral);
    th.assertIsApproximatelyEqual(activePool_STARDebt_After, A_totalDebt);
  });

  it("liquidate(): increases DefaultPool ETH and STAR debt by correct amounts", async () => {
    // --- SETUP ---
    const {
      collateral: A_collateral,
      totalDebt: A_totalDebt,
    ...
    // check DefaultPool ETH and STAR debt before
    ...
    const defaultPool_STARDebt_Before = (
      await defaultPool.getSTARDebt()
    ).toString();

    assert.equal(defaultPool_ETH_Before, "0");
    assert.equal(defaultPool_RawEther_Before, "0");
    assert.equal(defaultPool_STARDebt_Before, "0");
    ...
    // check after
    const defaultPool_ETH_After = (
      await defaultPool.getCollateral(weth.address)
    ).toString();
    const defaultPool_RawEther_After = (
      await weth.balanceOf(defaultPool.address)
    ).toString();
    const defaultPool_STARDebt_After = (
      await defaultPool.getSTARDebt()
    ).toString();
    ...
    console.log(
      "default POol raw ETH balance after",
      defaultPool_RawEther_After
    );
    ...
    th.assertIsApproximatelyEqual(defaultPool_STARDebt_After, B_totalDebt);
  });
  ...
  it("liquidate(): removes the Trove's stake from the total stakes", async () => {
    ...
    const totalStakes_Before = (
      await troveManager.getTotalStakes(weth.address)
    ).toString();
  ...
  it("liquidate(): updates the snapshots of total stakes and total collateral", async () => {
    ...
    const totalStakesSnapshot_Before = (
      await troveManager.totalStakesSnapshot(weth.address)
    ).toString();
    const totalCollateralSnapshot_Before = (
      await troveManager.totalCollateralSnapshot(weth.address)
    ).toString();
    ...
    // close Bob's Trove.  His ether*0.995 and STAR should be added to the DefaultPool.
    await troveManager.liquidate(bob, { from: owner });
  ...
  it("liquidate(): updates the L_ETH and L_STARDebt reward-per-unit-staked totals", async () => {
    // --- SETUP ---
    const {
      collateral: A_collateral,
      totalDebt: A_totalDebt,
    ...
    // price drops to 1ETH:100STAR, reducing Carols's ICR below MCR
    await priceFeed.setPrice("100000000000000000000");
    ...
    // Carol's ether*0.995 and STAR should be added to the DefaultPool.
    const L_ETH_AfterCarolLiquidated = await troveManager.L_Coll(weth.address);
    const L_STARDebt_AfterCarolLiquidated = await troveManager.L_STARDebt(
      weth.address
    );
    ...
    const L_STARDebt_expected_1 = C_totalDebt.mul(mv._1e18BN).div(
    ...
    assert.isAtMost(
      th.getDifference(L_STARDebt_AfterCarolLiquidated, L_STARDebt_expected_1),
      100
    );

    console.log("about to wthdraw for bob");
    // Bob now withdraws STAR, bringing his ICR to 1.11
    const { increasedTotalDebt: B_increasedTotalDebt } = await withdrawSTAR({
      ICR: toBN(dec(111, 16)),
    ...
    // price drops to 1ETH:50STAR, reducing Bob's ICR below MCR
   ...
   L_STARDebt = (180 / 20) + (890 / 10) = 98 STAR */
    const L_ETH_AfterBobLiquidated = await troveManager.L_Coll(weth.address);
    const L_STARDebt_AfterBobLiquidated = await troveManager.L_STARDebt(
      weth.address
    );
    ...
    const L_STARDebt_expected_2 = L_STARDebt_expected_1.add(
      B_totalDebt.add(B_increasedTotalDebt)
        .add(B_collateral.mul(L_STARDebt_expected_1).div(mv._1e18BN))
        .mul(mv._1e18BN)
        .div(A_collateral)
    );
    console.log("a", L_STARDebt_AfterBobLiquidated.toString());
    console.log("b", L_STARDebt_expected_2.toString());
    assert.isAtMost(
      th.getDifference(L_ETH_AfterBobLiquidated, L_ETH_expected_2),
      100
    );
    assert.isAtMost(
      th.getDifference(L_STARDebt_AfterBobLiquidated, L_STARDebt_expected_2),
      100
    );
  });
  ...
  xit("liquidate(): Liquidates undercollateralized trove if there are two troves in the system", async () => {
    ...
    // Alice creates a single trove with 0.7 ETH and a debt of 70 STAR, and provides 10 STAR to SP
    const {
      collateral: A_collateral,
      totalDebt: A_totalDebt,
    } = await openTrove({
  ...
  it("liquidate(): Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves", async () => {
    // Whale provides STAR to SP
    const spDeposit = toBN(dec(100, 24));
    await openTrove({
      ICR: toBN(dec(4, 18)),
      extraSTARAmount: spDeposit,
  ...
  it("liquidate(): a pure redistribution reduces the TCR only as a result of compensation", async () => {
    ...
    const gasComp_1 = th.getEmittedLiquidationValuesMulti(
      liquidationTx_1
    )[5][0];
    assert.isFalse(await sortedTroves.contains(defaulter_1));
    const TCR_1 = await th.getTCR(contracts);
    ...
    const gasComp_2 = th.getEmittedLiquidationValuesMulti(
      liquidationTx_2
    )[5][0];
    assert.isFalse(await sortedTroves.contains(defaulter_2));
    ...
    const gasComp_3 = th.getEmittedLiquidationValuesMulti(
      liquidationTx_3
    )[5][0];
    assert.isFalse(await sortedTroves.contains(defaulter_3));
    ...
    const gasComp_4 = th.getEmittedLiquidationValuesMulti(
      liquidationTx_4
    )[5][0];
    assert.isFalse(await sortedTroves.contains(defaulter_4));
  ...
  it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await starToken.transfer(dennis, spDeposit, { from: bob });

    //Dennis provides STAR to SP
    ...
    const liquidatedDebt = th.getEmittedLiquidationValuesMulti(
      liquidationTX_C
    )[0];
    const liquidatedColl = th.getEmittedLiquidationValuesMulti(
      liquidationTX_C
    )[3][0];
    assert.isFalse(await sortedTroves.contains(carol));
    // Check Dennis' SP deposit has absorbed Carol's debt, and he has received her liquidated ETH
    const dennis_Deposit_Before = (
      await stabilityPool.getCompoundedSTARDeposit(dennis)
    ).toString();
  ...
  it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and ETH gain", async () => {
    ...
    const bob_ETHGain_Before = (await stabilityPool.getDepositorGains(bob))[1][
      wethIDX
    ];
    assert.isAtMost(
    ...
    const bob_Trove_Status = (
      await troveManager.getTroveStatus(bob)
    ).toString();
    assert.equal(bob_Trove_Status, 3); // check closed by liquidation

    /* Alice's STAR Loss = (300 / 400) * 200 = 150 STAR
       Alice's ETH gain = (300 / 400) * 2*0.995 = 1.4925 ETH

       Bob's STARLoss = (100 / 400) * 200 = 50 STAR
       Bob's ETH gain = (100 / 400) * 2*0.995 = 0.4975 ETH

     Check Bob' SP deposit has been reduced to 50 STAR, and his ETH gain has increased to 1.5 ETH. */
    ...
    const bob_Deposit_After = await stabilityPool.getCompoundedSTARDeposit(bob);
    const bob_ETHGain_After = (await stabilityPool.getDepositorGains(bob))[1][
      wethIDX
    ];
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });
    const { starAmount: A_starAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraSTARAmount: toBN(dec(300, 18)),
      extraParams: { from: alice },
    });
    const { starAmount: B_starAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraSTARAmount: toBN(dec(200, 18)),
      extraParams: { from: bob },
    });
    const { starAmount: C_starAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraSTARAmount: toBN(dec(100, 18)),
      extraParams: { from: carol },
    });
    ...
    // Liquidate A, B and C
    const activeSTARDebt_0 = await activePool.getSTARDebt();
    const defaultSTARDebt_0 = await defaultPool.getSTARDebt();

    await troveManager.liquidate(alice);
    const activeSTARDebt_A = await activePool.getSTARDebt();
    const defaultSTARDebt_A = await defaultPool.getSTARDebt();

    await troveManager.liquidate(bob);
    const activeSTARDebt_B = await activePool.getSTARDebt();
    const defaultSTARDebt_B = await defaultPool.getSTARDebt();
    ...
    assert.equal((await starToken.balanceOf(alice)).toString(), A_starAmount);
    assert.equal((await starToken.balanceOf(bob)).toString(), B_starAmount);
    assert.equal((await starToken.balanceOf(carol)).toString(), C_starAmount);
  });
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Defaulter opens with 60 STAR, 0.6 ETH
    await openTrove({
      ICR: toBN(dec(2, 18)),
      extraParams: { from: defaulter_1 },
    });
    ...
    /* Liquidate defaulter. 30 STAR and 0.3 ETH is distributed between A, B and C.

    A receives (30 * 2/4) = 15 STAR, and (0.3*2/4) = 0.15 ETH
    B receives (30 * 1/4) = 7.5 STAR, and (0.3*1/4) = 0.075 ETH
    C receives (30 * 1/4) = 7.5 STAR, and (0.3*1/4) = 0.075 ETH
    */
    await troveManager.liquidate(defaulter_1);
  ...
  it("liquidate(): when SP > 0, triggers PREON reward event - increases the sum G", async () => {
    ...
    assert.equal(await stabilityPool.getTotalSTARDeposits(), dec(100, 18));
    ...
    // Price drops to 1ETH:100STAR, reducing defaulters to below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice();
    assert.isFalse(await th.checkRecoveryMode(contracts));
  ...
  it("batchLiquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards", async () => {
    ...
    // A adds 10 STAR to the SP, but less than C's debt
    ...
    await borrowerOperations.repaySTAR(dec(1, 18), D, D, { from: D });
    await borrowerOperations.repaySTAR(dec(1, 18), E, E, { from: E });
    ...
    const pendingETH_C = (await troveManager.getPendingCollRewards(C))[1][
      wethIDX
    ];
    const pendingSTARDebt_C = await troveManager.getPendingSTARDebtReward(C);
    const defaultPoolETH = await defaultPool.getCollateral(weth.address);
    const defaultPoolSTARDebt = await defaultPool.getSTARDebt();
    assert.isTrue(pendingETH_C.lte(defaultPoolETH));
    assert.isTrue(pendingSTARDebt_C.lte(defaultPoolSTARDebt));
    //Check only difference is dust
    assert.isAtMost(th.getDifference(pendingETH_C, defaultPoolETH), 1000);
    assert.isAtMost(
      th.getDifference(pendingSTARDebt_C, defaultPoolSTARDebt),
      1000
    );
  ...
  it("batchLiquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves", async () => {
    ...
    // Price drops to 1ETH:100STAR, reducing Bob and Carol's ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice();
  ...
  it("batchLiquidateTroves(): does nothing if all troves have ICR > 110%", async () => {
    ...
    await assertRevert(
      troveManager.batchLiquidateTroves([whale, alice, bob, carol], owner),
      "TroveManager: nothing to liquidate"
    );
  ...
  it("batchLiquidateTroves(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Liquidate defaulter. 30 STAR and 0.3 ETH is distributed uniformly between A, B and C. Each receive 10 STAR, 0.1 ETH
    await troveManager.liquidate(defaulter_1);
  ...
  it("batchLiquidateTroves(): reverts if _troveArray empty", async () => {
    ...
    await assertRevert(
      troveManager.batchLiquidateTroves([], owner),
      "TroveManager: nothing to liquidate"
    );
  ...
  it("batchLiquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const D_balanceBefore = await starToken.balanceOf(dennis);
    const E_balanceBefore = await starToken.balanceOf(erin);
    const F_balanceBefore = await starToken.balanceOf(flyn);
    ...
    assert.equal(
      (await starToken.balanceOf(dennis)).toString(),
      D_balanceBefore
    );
    assert.equal((await starToken.balanceOf(erin)).toString(), E_balanceBefore);
    assert.equal((await starToken.balanceOf(flyn)).toString(), F_balanceBefore);
  });

  it("batchLiquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR", async () => {
    // Whale provides 500 STAR to SP
    await openTrove({
      ICR: toBN(dec(100, 18)),
      extraSTARAmount: toBN(dec(500, 18)),
      extraParams: { from: whale },
    });
    ...
    // Check pool has 500 STAR
    assert.equal(
      (await stabilityPool.getTotalSTARDeposits()).toString(),
      dec(500, 18)
    );
    ...
    assert.equal((await stabilityPool.getTotalSTARDeposits()).toString(), "0");
  ...
  it("batchLiquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
    // Whale provides 400 STAR to the SP
    ...
    const { collateral: A_coll, totalDebt: A_debt } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraSTARAmount: A_deposit,
      extraParams: { from: alice },
    });
    const { collateral: B_coll, totalDebt: B_debt } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraSTARAmount: B_deposit,
      extraParams: { from: bob },
    });
    const { collateral: C_coll, totalDebt: C_debt } = await openTrove({
      ICR: toBN(dec(2, 18)),
    ...
    Total STAR in Pool: 800 STAR

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 150 + 350 + 150 = 650 STAR
    Total liquidated ETH = 1.1 + 3.1 + 1.1 = 5.3 ETH

    whale star loss: 650 * (400/800) = 325 star
    alice star loss:  650 *(100/800) = 81.25 star
    bob star loss: 650 * (300/800) = 243.75 star

    whale remaining deposit: (400 - 325) = 75 star
    alice remaining deposit: (100 - 81.25) = 18.75 star
    bob remaining deposit: (300 - 243.75) = 56.25 star
    ...
    Total remaining deposits: 150 STAR
    Total ETH gain: 4.975 ETH */

    // Check remaining STAR Deposits and ETH gain, for whale and depositors whose troves were liquidated
    const whale_Deposit_After = await stabilityPool.getCompoundedSTARDeposit(
      whale
    );
    const alice_Deposit_After = await stabilityPool.getCompoundedSTARDeposit(
      alice
    );
    const bob_Deposit_After = await stabilityPool.getCompoundedSTARDeposit(bob);

    const whale_ETHGain = (await stabilityPool.getDepositorGains(whale))[1][
      wethIDX
    ];
    const alice_ETHGain = (await stabilityPool.getDepositorGains(alice))[1][
      wethIDX
    ];
    const bob_ETHGain = (await stabilityPool.getDepositorGains(bob))[1][
      wethIDX
    ];
    ...
    assert.isAtMost(
      th.getDifference(total_STARinSP, totalDeposits.sub(liquidatedDebt)),
      1000
    );
    assert.isAtMost(
  ...
  it("batchLiquidateTroves(): when SP > 0, triggers PREON reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } });
  ...
  it("batchLiquidateTroves(): closes every trove with ICR < MCR in the given array", async () => {
    ...
    // Price drops to 1ETH:100STAR, reducing A, B, C ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice();
  ...
  it("batchLiquidateTroves(): skips if a trove has been closed", async () => {
    ...
    await starToken.transfer(carol, dec(100, 18), { from: whale });
  ...
  it("getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption", async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(310, 16)),
      extraSTARAmount: partialRedemptionAmount,
      extraParams: { from: alice },
    });
    ...
    await troveManager.updateTroves(
      [alice, bob, carol, dennis],
      [alice, alice, alice, alice],
      [carol, carol, carol, dennis]
    );
    // --- TEST ---
  ...
  it("getRedemptionHints(): returns 0 as partialRedemptionHintICR when reaching _maxIterations", async () => {
    ...
    // Get hints for a redemption of 3600 + some extra STAR. At least 3 iterations are needed
    // for total redemption of the given amount.
    const { partialRedemptionHintICR } = await hintHelpers.getRedemptionHints(
      toBN(dec(4100, 18)),
      2
  ...
  it("redeemCollateral(): Sanity test for doing one redemption, checks correct amounts.", async () => {
    ...
    await borrowerOperations.openTrove(
      th._100pct,
      await getOpenTroveSTARAmount(dec(6000, 18)),
      A,
      A,
    ...
    await borrowerOperations.openTrove(
      th._100pct,
      await getOpenTroveSTARAmount(dec(20000, 18)),
      B,
      B,
      [contracts.weth.address],
      [dec(1000, 18)],
    ...
    const A_STAR_Balance_Before = await contracts.starToken.balanceOf(A);
    const B_debt_before = await troveManager.getTroveDebt(B);

    const A_WETH_Balance_Before = await contracts.weth.balanceOf(A);
    const B_WETH_Collateral_Before = (
      await th.getTroveEntireColl(contracts, B)
    )[0];

    const active_pool_before = await activePool.getCollateral(weth.address);

    // STAR redemption is 5000 STAR
    const STARRedemption = dec(5000, 18);
    const finalSTARAmount = await th.estimateSTAREligible(
      contracts,
      STARRedemption
    );
    const finalSTARFeeAmount = th.toBN(STARRedemption).sub(finalSTARAmount);
    ...
    const actualSTARAmount = th.getEmittedRedemptionValues(tx1)[1];
    const actualSTARFee = th.getEmittedRedemptionValues(tx1)[2];
    const actualETHAmount = th.getEmittedRedemptionValues(tx1)[4][0];

    assert.isTrue(actualSTARAmount.eq(finalSTARAmount));
    assert.isTrue(actualETHAmount.eq(finalSTARAmount.div(toBN(200))));
    assert.isTrue(actualSTARFee.lt(finalSTARFeeAmount));

    const A_STAR_Balance_After = await contracts.starToken.balanceOf(A);
    const B_debt_after = await troveManager.getTroveDebt(B);

    const A_WETH_Balance_After = await contracts.weth.balanceOf(A);
    const B_WETH_Collateral_After = (
      await th.getTroveEntireColl(contracts, B)
    )[0];

    const active_pool_after = await activePool.getCollateral(weth.address);

    // Assert that A lost the correct amount of STAR. Approximate because fee amount not correct
    await th.assertIsApproximatelyEqual(
      A_STAR_Balance_After,
      A_STAR_Balance_Before.sub(finalSTARAmount).sub(finalSTARFeeAmount),
      100000
    );

    // Assert that B's debt has decreased by the correct amount of STAR
    assert.isTrue(B_debt_after.eq(B_debt_before.sub(finalSTARAmount)));

    // Make sure that A gained an appropriate amount of collateral
    assert.isTrue(
      A_WETH_Balance_After.eq(
        A_WETH_Balance_Before.add(finalSTARAmount.div(th.toBN(200)))
      )
    );

    // Make sure that B lost that amount of collateral
    assert.isTrue(
      B_WETH_Collateral_After.eq(
        B_WETH_Collateral_Before.sub(finalSTARAmount.div(th.toBN(200)))
      )
    );

    // Make sure active pool has decreased by the correct amount of WETH
    assert.isTrue(
      active_pool_after.eq(
        active_pool_before.sub(finalSTARAmount.div(th.toBN(200)))
      )
    );
  });
  ...
  it("redeemCollateral(): Stops when amount fulfilled. Ensure that amounts are correct and that first trove closes", async () => {
    ...
    await borrowerOperations.openTrove(
      th._100pct,
      await getOpenTroveSTARAmount(dec(2000, 18)),
      C,
      C,
      [contracts.weth.address],
      [dec(50, 18)],
    ...
    // STAR redemption is 6000 STAR. Should close C, D and take some of B.
    const STARRedemption = dec(6000, 18);
    const finalSTARAmount = await th.estimateSTAREligible(
      contracts,
      STARRedemption
    );
    ...
    const collSurplusPoolAfter = await contracts.collSurplusPool.getCollateral(
      weth.address
    );
    ...
    // Assert that B's debt has decreased by the correct amount of STAR. 1800 * 2 offset from other troves.
    assert.isTrue(
      B_debt_after.eq(
        B_debt_before.sub(finalSTARAmount).add(toBN(dec(3600, 18)))
      )
    );
    ...
    assert.isTrue(
      B_WETH_Collateral_After.eq(
        B_WETH_Collateral_Before.sub(finalSTARAmount.div(th.toBN(200))).add(
          toBN(dec(18, 18))
        )
      )
    );
    ...
    assert.isTrue(
      active_pool_after.eq(
        active_pool_before
          .sub(finalSTARAmount.div(th.toBN(200)))
          .sub(toBN(dec(82, 18)))
      )
    );
  });
  ...
  it("redeemCollateral(): with invalid first hint, zero address", async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(310, 16)),
      extraSTARAmount: dec(10, 18),
      extraParams: { from: alice },
    });
    const { netDebt: B_netDebt } = await openTrove({
      ICR: toBN(dec(290, 16)),
      extraSTARAmount: dec(8, 18),
      extraParams: { from: bob },
    });
    ...
    await openTrove({
      ICR: toBN(dec(100, 18)),
      extraSTARAmount: redemptionAmount,
      extraParams: { from: dennis },
    });

    const dennis_ETHBalance_Before = toBN(await weth.balanceOf(dennis));

    const dennis_STARBalance_Before = await starToken.balanceOf(dennis);
    ...
    // Find hints for redeeming 20 STAR
    const {
      firstRedemptionHint,
      partialRedemptionHintICR,
    } = await hintHelpers.getRedemptionHints(redemptionAmount, 0);
    ...
    // Dennis redeems 20 STAR
    // Don't pay for gas, as it makes it easier to calculate the received Ether
    const toApprove = redemptionAmount.add(redemptionAmount);
    await starToken.approve(troveManagerRedemptions.address, toApprove, {
      from: dennis,
    });
    ...
    const STARFee = th.getEmittedRedemptionValues(redemptionTx)[2];
    ...
    /* check that Dennis' redeemed 20 STAR has been cancelled with debt from Bobs's Trove (8) and Carol's Trove (10).
    The remaining lot (2) is sent to Alice's Trove, who had the best ICR.
    It leaves her with (3) STAR debt + 50 for gas compensation. */
    th.assertIsApproximatelyEqual(
      alice_debt_After,
    ...
    const expectedTotalETHDrawn = redemptionAmount.div(toBN(200)); // convert redemptionAmount STAR to ETH, at ETH:USD price 200
    const expectedReceivedETH = expectedTotalETHDrawn;

    th.assertIsApproximatelyEqual(expectedReceivedETH, receivedETH);

    const dennis_STARBalance_After = (
      await starToken.balanceOf(dennis)
    ).toString();
    assert.equal(
      dennis_STARBalance_After,
      dennis_STARBalance_Before.sub(redemptionAmount).sub(STARFee).toString()
    );
  });
  ...
  it("redeemCollateral(): with invalid first hint, trove below MCR", async () => {
    ...
    const dennis_STARBalance_After = await starToken.balanceOf(dennis);
    th.assertIsApproximatelyEqual(
      dennis_STARBalance_After,
      dennis_STARBalance_Before.sub(redemptionAmount).sub(STARFee)
    );
  });
  ...
  it("redeemCollateral(): Stops when max iterations reached. Ensure that it does the correct amount after doing max iterations", async () => {
    ...
    // STAR redemption is 10000. Should stop after redeeming against trove D
    const STARRedemption = dec(10000, 18);
    const finalSTARAmount = toBN(dec(3600, 18)); //await th.estimateSTAREligible(contracts, STARRedemption)
    // const finalSTARFeeAmount = th.toBN(STARRedemption).sub(finalSTARAmount);
    // console.log("FINAL STAR FEE AMOUNT " , finalSTARFeeAmount.toString())
    const estimatedSTARFee = await th.estimateRedemptionFee(
      contracts,
      finalSTARAmount
    );

    // const tx1 = await th.redeemCollateralAndGetTxObject(A, contracts, STARRedemption, th._100pct)
    const tx1 = await th.performRedemptionTx(
      A,
      0,
      contracts,
      STARRedemption,
    ...
    assert.isTrue(actualSTARAmount.eq(finalSTARAmount));
    assert.isTrue(actualETHAmount.eq(finalSTARAmount.div(toBN(200))));
    assert.isTrue(actualSTARFee.lt(estimatedSTARFee));
    ...
    await th.assertIsApproximatelyEqual(
      A_STAR_Balance_After,
      A_STAR_Balance_Before.sub(finalSTARAmount).sub(estimatedSTARFee),
      100000
    );
  ...
  it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
    ...
    // STAR redemption is 3000. Should stop after redeeming against trove D
    const STARRedemption = dec(3000, 18);
    const finalSTARAmount = await th.estimateSTAREligible(
      contracts,
      STARRedemption
    );
    const finalSTARFeeAmount = th.toBN(STARRedemption).sub(finalSTARAmount);
    const estimatedSTARAmount = toBN(dec(1800, 18));
    const estimatedSTARFee = await th.estimateRedemptionFee(
      contracts,
      estimatedSTARAmount
    );
    ...
    // const tx1 = await th.performRedemptionTx(A, 0, contracts, STARRedemption, 0, th._100pct, 2)
    assert.isTrue(tx1.receipt.status);
    ...
    assert.isTrue(actualSTARAmount.eq(estimatedSTARAmount));
    assert.isTrue(actualETHAmount.eq(estimatedSTARAmount.div(toBN(200))));
    console.log("Actual fee", actualSTARFee.toString());
    console.log("Actual fee estimation", estimatedSTARFee.toString());
    assert.isTrue(actualSTARFee.lt(estimatedSTARFee));
    ...
    await th.assertIsApproximatelyEqual(
      A_STAR_Balance_After,
      A_STAR_Balance_Before.sub(estimatedSTARAmount).sub(estimatedSTARFee),
      100000
    );
    ...
    assert.isTrue(
      B_debt_after.eq(
        B_debt_before.sub(estimatedSTARAmount).add(toBN(dec(1800, 18)))
      )
    );
    ...
    assert.isTrue(
      A_WETH_Balance_After.eq(
        A_WETH_Balance_Before.add(estimatedSTARAmount.div(th.toBN(200)))
      )
    );
    ...
    assert.isTrue(
      B_WETH_Collateral_After.eq(
        B_WETH_Collateral_Before.sub(estimatedSTARAmount.div(th.toBN(200))).add(
          toBN(dec(9, 18))
        )
      )
    );
    ...
    assert.isTrue(
      active_pool_after.eq(
        active_pool_before
          .sub(estimatedSTARAmount.div(th.toBN(200)))
          .sub(toBN(dec(41, 18)))
      )
    );
  });
  ...
  xit("redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date", async () => {
    ...
    // const { netDebt: W_netDebt } = await openTrove({ ICR: toBN(dec(350, 16)), extraSTARAmount: dec(10000, 18), extraParams: { from: whale } })
    ...
    // Oops, another transaction gets in the way
    {
      ...
      // Alice redeems 1 STAR from Carol's Trove
    ...
    // Dennis tries to redeem 20 STAR
    const redemptionTx = await troveManager.redeemCollateral(
      redemptionAmount,
      redemptionAmount,
      firstRedemptionHint,
    ...
    // Since Alice already redeemed 1 STAR from Carol's Trove, Dennis was  able to redeem:
    //  - 9 STAR from Carol's
    //  - 8 STAR from Bob's
    // for a total of 17 STAR.

    // Dennis calculated his hint for redeeming 2 STAR from Alice's Trove, but after Alice's transaction
    // got in the way, he would have needed to redeem 3 STAR to fully complete his redemption of 20 STAR.
    ...
    const expectedTotalETHDrawn = fullfilledRedemptionAmount
      .sub(frontRunRedemption)
      .div(toBN(200)); // redempted STAR converted to ETH, at ETH:USD price 200
    ...
    console.log(
      "ALICE DEBT AFTER ",
      (await th.getTroveEntireDebt(contracts, alice)).toString()
    );
    ...
    console.log(
      "dennis YUDS Balance before ",
      dennis_STARBalance_Before
        .sub(STARFee)
        .sub(fullfilledRedemptionAmount)
        .sub(frontRunRedemption)
        .toString()
    );
    console.log(
      "dennis YUDS Balance after ",
      dennis_STARBalance_After.toString()
    );
    th.assertIsApproximatelyEqual(
      dennis_STARBalance_After,
      dennis_STARBalance_Before
  ...
  it.skip("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
    ...
    const amount = await getOpenTroveSTARAmount(dec(110, 18));
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: alice } });
    await openTrove({
      ICR: toBN(dec(133, 16)),
      extraSTARAmount: amount,
      extraParams: { from: bob },
    });

    await starToken.transfer(carol, amount, { from: bob });
    ...
    const expectedTotalETHDrawn = toBN(amount).div(toBN(100)); // convert 100 STAR to ETH at ETH:USD price of 100
    const expectedReceivedETH = expectedTotalETHDrawn.sub(ETHFee);

    const receivedETH = carol_ETHBalance_After.sub(carol_ETHBalance_Before);
    assert.isTrue(expectedReceivedETH.eq(receivedETH));

    const carol_STARBalance_After = (
      await starToken.balanceOf(carol)
    ).toString();
    assert.equal(carol_STARBalance_After, "0");
  });
  ...
  it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    // STAR redemption is 3000. Should do against B.
    const STARRedemption = dec(3000, 18);
    const finalSTARAmount = await th.estimateSTAREligible(
      contracts,
      STARRedemption
    ...
    console.log(`STAR Amount: ${actualSTARAmount}`);
    console.log(`STAR amount estimated: ${finalSTARAmount}`);
    assert.isTrue(actualSTARAmount.eq(finalSTARAmount));
    assert.isTrue(actualETHAmount.eq(finalSTARAmount.div(toBN(100))));
    assert.isTrue(actualSTARFee.lt(finalSTARFeeAmount));
    ...
    assert.isTrue(
      A_WETH_Balance_After.eq(
        A_WETH_Balance_Before.add(finalSTARAmount.div(th.toBN(100)))
      )
    );
    ...
    await th.assertIsApproximatelyEqual(
      ...
      B_WETH_Collateral_Before.sub(finalSTARAmount.div(th.toBN(100)))
    );

    // Make sure active pool has decreased by the correct amount of WETH
    assert.isTrue(
      active_pool_after.eq(
        active_pool_before.sub(finalSTARAmount.div(th.toBN(100)))
      )
    );
  });
  ...
  xit("redeemCollateral(): finds the last Trove with ICR == 110% even if there is more than one", async () => {
    ...
    const { totalDebt: whale_totalDebt } = await openTrove({
      ICR: toBN(dec(100, 18)),
      extraSTARAmount: dec(10, 18),
      extraParams: { from: whale },
    });
  ...
  it("redeemCollateral(): reverts when argument _amount is 0", async () => {
    ...
    // Alice opens trove and transfers 500STAR to Erin, the would-be redeemer
    await openTrove({
      ICR: toBN(dec(200, 16)),
      extraSTARAmount: dec(500, 18),
      extraParams: { from: alice },
    });
    await starToken.transfer(erin, dec(500, 18), { from: alice });
  ...
  it("redeemCollateral(): reverts if max fee > 100%", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(400, 16)),
      extraSTARAmount: dec(20, 18),
      extraParams: { from: B },
    });
    await openTrove({
      ICR: toBN(dec(400, 16)),
      extraSTARAmount: dec(30, 18),
      extraParams: { from: C },
    });
    await openTrove({
      ICR: toBN(dec(400, 16)),
      extraSTARAmount: dec(40, 18),
      extraParams: { from: D },
    });
    ...
    await assertRevert(
      th.performRedemptionWithMaxFeeAmount(
        A,
        contracts,
        dec(10, 18),
        dec(11, 18)
  ...
  it("redeemCollateral(): reverts if fee exceeds max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraSTARAmount: dec(8000, 18),
      extraParams: { from: A },
    });
    const { totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraSTARAmount: dec(9000, 18),
      extraParams: { from: B },
    });
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraSTARAmount: dec(10000, 18),
      extraParams: { from: C },
    });
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt);

    // Check total STAR supply
    ...
    // STAR redemption is 27 USD: a redemption that incurs a fee of 27/(270 * 2) = 5%
    const attemptedSTARRedemption = expectedTotalSupply.div(toBN(10));

    // Max fee is <5%
    const lessThan5pct = "49999999999999999";
    console.log(
      "This is attempted STAR Amount ",
      attemptedSTARRedemption.toString()
    );
    console.log(
      "Max fee is supposed to be ",
      toBN(lessThan5pct)
        .mul(attemptedSTARRedemption)
        .div(toBN(dec(1, 18)))
        .toString()
    );
    // await assertRevert(
    ...
    // , "Fee exceeded provided maximum")
    // await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedSTARRedemption, lessThan5pct), "Fee exceeded provided maximum")
    ...
    // th.redeemCollateralAndGetTxObject(A, contracts, attemptedSTARRedemption, dec(1, 16)), "Fee exceeded provided maximum")
    ...
    // await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedSTARRedemption, dec(3754, 13)), "Fee exceeded provided maximum")
    ...
    // await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedSTARRedemption, dec(5, 15)), "Fee exceeded provided maximum")
  });
  ...
  it("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
    ...
    // STAR redemption fee with 10% of the supply will be 0.5% + 1/(10*2)
    const attemptedSTARRedemption = expectedTotalSupply.div(toBN(10));

    // Attempt with maxFee > 5.5%
    const price = await priceFeed.getPrice();
    const ETHDrawn = attemptedSTARRedemption.mul(mv._1e18BN).div(price);
    const slightlyMoreThanFee = await troveManager.getRedemptionFeeWithDecay(
      ETHDrawn
    );
    console.log("attempted STAR  DRAWN ", attemptedSTARRedemption.toString());
    console.log("SLIGHTLY MORE THAN FEE ", slightlyMoreThanFee.toString());
    const tx1 = await th.performRedemptionWithMaxFeeAmount(
      A,
      contracts,
      attemptedSTARRedemption,
      slightlyMoreThanFee.mul(attemptedSTARRedemption).div(toBN(dec(1, 18)))
    );
    // const tx1 = await th.redeemCollateralAndGetTxObject(A, contracts, attemptedSTARRedemption, slightlyMoreThanFee)
    assert.isTrue(tx1.receipt.status);
    ...
    const tx2 = await th.performRedemptionWithMaxFeeAmount(
      C,
      contracts,
      attemptedSTARRedemption,
      exactSameFee.mul(attemptedSTARRedemption).div(toBN(dec(1, 18)))
    );
    // const tx2 = await th.redeemCollateralAndGetTxObject(C, contracts, attemptedSTARRedemption, exactSameFee)
  ...
  it("redeemCollateral(): doesn't affect the Stability Pool deposits or ETH gain of redeemed-from troves", async () => {
    ...
    const { totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraSTARAmount: dec(100, 18),
      extraParams: { from: bob },
    });
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(195, 16)),
      extraSTARAmount: dec(200, 18),
      extraParams: { from: carol },
    });
    const { totalDebt: D_totalDebt } = await openTrove({
      ICR: toBN(dec(190, 16)),
      extraSTARAmount: dec(400, 18),
      extraParams: { from: dennis },
    });
    ...
    const redemptionAmount = B_totalDebt.add(C_totalDebt)
      .add(D_totalDebt)
      .add(F_totalDebt);
    // Alice opens trove and transfers STAR to Erin, the would-be redeemer
    await openTrove({
      ICR: toBN(dec(300, 16)),
      extraSTARAmount: redemptionAmount,
      extraParams: { from: alice },
    });
    await starToken.transfer(erin, redemptionAmount, { from: alice });
    ...
    // Check the remaining STAR and ETH in Stability Pool after liquidation is non-zero
    ...
    assert.isTrue(STARinSP.gte(mv._zeroBN));
  ...
  it("redeemCollateral(): caller can redeem their entire STARToken balance", async () => {
    await th.addERC20(
      contracts.weth,
      bob,
      contracts.borrowerOperations.address,
    ...
    // Alice opens trove and transfers 400 STAR to Erin, the would-be redeemer
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraSTARAmount: dec(400, 18),
      extraParams: { from: alice },
    });
    await starToken.transfer(erin, dec(400, 18), { from: alice });

    // Check Erin's balance before
    const erin_balance_before = await starToken.balanceOf(erin);
    assert.equal(erin_balance_before, dec(400, 18));

    // B, C, D open trove
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraSTARAmount: dec(590, 18),
      extraParams: { from: bob },
    });
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraSTARAmount: dec(1990, 18),
      extraParams: { from: carol },
    });
    ...
    // Erin attempts to redeem 400 STAR
    const finalSTARAmount = await th.estimateSTAREligible(
      contracts,
      dec(400, 18)
    );
    const finalSTARFeeAmount = toBN(toBN(dec(400, 18))).sub(finalSTARAmount);
    await th.performRedemptionTx(erin, price, contracts, dec(400, 18));
    const activePool_debt_after = await activePool.getSTARDebt();
    assert.equal(
      activePool_debt_before.sub(activePool_debt_after).toString(),
      finalSTARAmount.toString()
    );
    ...
    assert.equal(
      ...
      activePool_coll_before.sub(finalSTARAmount.div(toBN(200))).toString()
    );

    // Check Erin's balance after
    const erin_balance_after = (await starToken.balanceOf(erin)).toString();

    await th.assertIsApproximatelyEqual(erin_balance_after, 0);
  });

  it("redeemCollateral(): reverts when requested redemption amount exceeds caller's STAR token balance", async () => {
    await th.addERC20(
      contracts.weth,
      bob,
      contracts.borrowerOperations.address,
    ...
    // Erin tries to redeem 1000 STAR
    try {
      const redemptionTx = await th.performRedemptionTx(
        erin,
        0,
        contracts,
        dec(1000, 18)
      ...
      assert.include(
        error.message,
        "Requested redemption amount must be <= user's STAR token balance"
      );
    }

    // Erin tries to redeem 401 STAR
    try {
      const redemptionTx = await th.performRedemptionTx(
        erin,
        0,
    ...
    // Erin tries to redeem 239482309 STAR
    try {
      const redemptionTx = await th.performRedemptionWithMaxFeeAmount(
        erin,
        contracts,
        toBN("239482309000000000000000000"),
        toBN("23948230900000000000000000")
      );
      assert.isFalse(redemptionTx.receipt.status);
    } catch (error) {
    ...
    // Erin tries to redeem 2^256 - 1 STAR
  ...
  it("redeemCollateral(): value of issued ETH == face value of redeemed STAR (assuming 1 STAR has value of $1)", async () => {
    await th.addERC20(
      contracts.weth,
      bob,
      contracts.borrowerOperations.address,
    ...
    // Alice opens trove and transfers 1000 STAR each to Erin, Flyn, Graham
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraSTARAmount: dec(4990, 18),
      extraParams: { from: alice },
    });
    await starToken.transfer(erin, dec(1000, 18), { from: alice });
    await starToken.transfer(flyn, dec(1000, 18), { from: alice });
    await starToken.transfer(graham, dec(1000, 18), { from: alice });

    // B, C, D open trove
    const { collateral: B_coll } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraSTARAmount: dec(1590, 18),
      extraParams: { from: bob },
    });
    const { collateral: C_coll } = await openTrove({
      ICR: toBN(dec(600, 16)),
      extraSTARAmount: dec(1090, 18),
      extraParams: { from: carol },
    });
    ...
    const _120_STAR = "120000000000000000000";
    const _373_STAR = "373000000000000000000";
    const _950_STAR = "950000000000000000000";
    ...
    // Erin redeems 120 STAR
    ({
      firstRedemptionHint,
      partialRedemptionHintICR,
    } = await hintHelpers.getRedemptionHints(_120_STAR, 0));
    ...
    const toApprove = toBN(1000).mul(th.toBN(_120_STAR));
    await starToken.approve(troveManagerRedemptions.address, toApprove, {
      from: erin,
    });
    ...
    /* 120 STAR redeemed.  Expect $120 worth of ETH removed. At ETH:USD price of $200, 
    ...
    assert.equal(
      ...
      activeETH_0.sub(toBN(_120_STAR).mul(mv._1e18BN).div(price))
    );

    // Flyn redeems 373 STAR
    ({
      firstRedemptionHint,
      partialRedemptionHintICR,
    } = await hintHelpers.getRedemptionHints(_373_STAR, 0));
    ...
    /* 373 STAR redeemed.  Expect $373 worth of ETH removed. At ETH:USD price of $200, 
    ...
    assert.equal(
      ...
      activeETH_1.sub(toBN(_373_STAR).mul(mv._1e18BN).div(price))
    );

    // Graham redeems 950 STAR
    ({
      firstRedemptionHint,
      partialRedemptionHintICR,
    } = await hintHelpers.getRedemptionHints(_950_STAR, 0));
    ...
    /* 950 STAR redeemed.  Expect $950 worth of ETH removed. At ETH:USD price of $200, 
    ...
    assert.equal(
      ...
      activeETH_2.sub(toBN(_950_STAR).mul(mv._1e18BN).div(price))
    );
  });
  ...
  it.skip("redeemCollateral(): reverts if there is zero outstanding system debt", async () => {
    // --- SETUP --- illegally mint STAR to Bob
    await starToken.unprotectedMint(bob, dec(100, 18));

    assert.equal(await starToken.balanceOf(bob), dec(100, 18));
    ...
    // Bob tries to redeem his illegally obtained STAR
    try {
      const redemptionTx = await troveManager.redeemCollateral(
        dec(100, 18),
        firstRedemptionHint,
  ...
  it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
    ...
    // --- SETUP --- illegally mint STAR to Bob
    await starToken.unprotectedMint(bob, "101000000000000000000");

    assert.equal(await starToken.balanceOf(bob), "101000000000000000000");
    ...
    th.assertIsApproximatelyEqual(
      (await activePool.getSTARDebt()).toString(),
      totalDebt
    );
    ...
    // Bob attempts to redeem his ill-gotten 101 STAR, from a system that has 100 STAR outstanding debt
    try {
      const redemptionTx = await troveManager.redeemCollateral(
  ...
  it("redeemCollateral(): a redemption made when base rate is zero increases the base rate", async () => {
    ...
    const A_balanceBefore = await starToken.balanceOf(A);

    await th.performRedemptionTx(A, 0, contracts, dec(10, 18));

    // Check A's balance has decreased by 10 STAR
    await th.assertIsApproximatelyEqual(
      await starToken.balanceOf(A),
      A_balanceBefore.sub(toBN(dec(10, 18)))
    );
  ...
  it("redeemCollateral(): a redemption made when base rate is non-zero increases the base rate, for negligible time passed", async () => {
    ...
    await preonToken.approve(sPREON.address, dec(1, 18), { from: multisig });
    await sPREON.mint(dec(1, 18), { from: multisig });
    ...
    const A_balanceBefore = await starToken.balanceOf(A);
    const B_balanceBefore = await starToken.balanceOf(B);
    ...
    // Check B's balance has decreased by 10 STAR
    await th.assertIsApproximatelyEqual(
      await starToken.balanceOf(B),
      B_balanceBefore.sub(toBN(dec(10, 18)))
    );
  ...
  it("redeemCollateral(): a redemption made at zero base rate send a non-zero ETHFee to PREON staking contract", async () => {
    await th.addERC20(
      contracts.weth,
      A,
      contracts.borrowerOperations.address,
    ...
    // Get sPREON balance before redemption
    const sPREONBalance_Before = await starToken.balanceOf(sPREON.address);
    ...
    // Make sure STAR Staking contract balance has increased STAR Balance
    const sPREONBalance_After = toBN(await starToken.balanceOf(sPREON.address));
    assert.isTrue(sPREONBalance_After.gt(sPREONBalance_Before));
  });

  it("redeemCollateral(): a redemption made at zero base increases the ETH-fees-per-PREON-staked in sPREON contract", async () => {
    await th.addERC20(
      contracts.weth,
      A,
      contracts.borrowerOperations.address,
    ...
    // Check PREON Staking ETH-fees-per-PREON-staked before
    ...
    // Check PREON Staking STAR-fees higher after redemption
    const F_STAR_After = await await starToken.balanceOf(sPREON.address);
    assert.isTrue(F_STAR_After.gt(F_STAR_Before));
  });

  it("redeemCollateral(): a redemption made at a non-zero base rate send a non-zero ETHFee to PREON staking contract", async () => {
    await th.addERC20(
      contracts.weth,
      A,
      contracts.borrowerOperations.address,
    ...
    const sPREONBalance_Before = toBN(
      await starToken.balanceOf(sPREON.address)
    );
    ...
    // check PREON Staking balance has increased
    assert.isTrue(sPREONBalance_After.gt(sPREONBalance_Before));
  });

  it("redeemCollateral(): a redemption made at a non-zero base rate increases ETH-per-PREON-staked in the staking contract", async () => {
    await th.addERC20(
      contracts.weth,
      A,
      contracts.borrowerOperations.address,
    ...
    // Check PREON Staking STAR fees before is zero
    const F_STAR_Before = await starToken.balanceOf(sPREON.address);
    ...
    // check STAR balance of sPREON has increased
    assert.isTrue(F_STAR_After.gt(F_STAR_Before));
  });
  ...
  it("redeemCollateral(): a redemption sends the ETH remainder (ETHDrawn - ETHFee) to the redeemer", async () => {
    ...
    // Check total STAR supply
    const activeSTAR = await activePool.getSTARDebt();
    const defaultSTAR = await defaultPool.getSTARDebt();

    const totalSTARSupply = activeSTAR.add(defaultSTAR);
    th.assertIsApproximatelyEqual(totalSTARSupply, totalDebt);

    // A redeems 9 STAR
    const redemptionAmount = toBN(dec(9, 18));
    await th.performRedemptionWithMaxFeeAmount(
      A,
      contracts,
      redemptionAmount,
      redemptionAmount
  ...
  it("redeemCollateral(): a full redemption (leaving trove with 0 debt), closes the trove", async () => {
    ...
    // whale redeems 360 STAR.  Expect this to fully redeem A, B, C, and partially redeem D.
    await th.performRedemptionWithMaxFeeAmount(
      whale,
      contracts,
      redemptionAmount,
  ...
  const redeemCollateral3Full1Partial = async () => {
    ...
    // whale redeems STAR.  Expect this to fully redeem A, B, C, and partially redeem D.
    ...
    */
    ...
    await assertRevert(
      ...
      // openTrove({ ICR: toBN(dec(200, 16)), extraSTARAmount: dec(100, 18), extraParams: { from: D } }),
  ...
  it("redeemCollateral(): emits correct debt and coll values in each redeemed trove's TroveUpdated event", async () => {
    ...
    // whale redeems STAR.  Expect this to fully redeem A, B, C, and partially redeem 15 STAR from D.
  ...
  // This test is not necessary because the fee has to be paid in STAR, so you will only get a corresponding
  ...
  it.skip("redeemCollateral(): reverts if fee eats up all returned collateral", async () => {
    ...
    // --- SETUP ---
    const { starAmount } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraSTARAmount: dec(1, 24),
      extraParams: { from: alice },
    });
    ...
    const finalSTARFeeAmount = toBN(starAmount).sub(finalSTARAmount);

    const {
      firstRedemptionHint,
      partialRedemptionHintICR,
    } = await hintHelpers.getRedemptionHints(finalSTARAmount, 0);
  ...
  it("getPendingSTARDebtReward(): Returns 0 if there is no pending STARDebt reward", async () => {
    // Make some troves
    ...
    const current_L_STARDebt = await troveManager.L_STARDebt(weth.address);
    assert.equal(current_L_STARDebt, 0);

    const carolSnapshot_L_Coll = await troveManager.getRewardSnapshotColl(
      carol,
      contracts.weth.address
    );
    assert.equal(carolSnapshot_L_Coll, 0);

    const carolSnapshot_L_STARDebt = await troveManager.getRewardSnapshotSTAR(
      carol,
      contracts.weth.address
    );
    assert.equal(carolSnapshot_L_STARDebt, 0);

    const carol_PendingSTARDebtReward = await troveManager.getPendingSTARDebtReward(
      carol
    );
    assert.equal(carol_PendingSTARDebtReward, 0);
  });
  ...
  it("getPendingETHReward(): Returns 0 if there is no pending Coll reward", async () => {
    ...
    const carol_PendingCollReward = (
      await troveManager.getPendingCollRewards(carol)
    )[1];
  ...
  it("computeICR(): Returns 0 if trove's coll is worth 0", async () => {
    ...
    const ICR = (
      await troveManager.computeICR([contracts.weth.address], [coll], debt)
    ).toString();

    assert.equal(ICR, 0);
  });

  it("computeICR(): Returns 2^256-1 for ETH:USD = 100, coll = 1 ETH, debt = 100 STAR", async () => {
    const price = dec(100, 18);
    const coll = dec(1, "ether");
    const debt = dec(100, 18);
    await priceFeed.setPrice(price);
  ...
  it("computeICR(): returns correct ICR for ETH:USD = 100, coll = 200 ETH, debt = 30 STAR", async () => {
    const price = dec(100, 18);
  ...
  it("computeICR(): returns correct ICR for ETH:USD = 250, coll = 1350 ETH, debt = 127 STAR", async () => {
  ...
  it("computeICR(): returns correct ICR for ETH:USD = 100, coll = 1 ETH, debt = 54321 STAR", async () => {
    const price = dec(100, 18);
    const coll = dec(1, "ether");
  ...
  it("getTroveColl(): Returns coll", async () => {
    ...
    assert.equal(
      (await troveManager.getTroveColls(A))[1][0],
      A_coll.toString()
    );
    assert.equal(
      (await troveManager.getTroveColls(A))[1][0],
  ...
  it("getTroveStatus(): Returns status", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(200, 16)),
      extraSTARAmount: B_totalDebt,
      extraParams: { from: A },
    });

    // to be able to repay:
    await starToken.transfer(B, B_totalDebt, { from: A });
  ...
  it("Single Collateral low ratio-partial to SP partial redistribution", async () => {
    ...
    console.log(
      "collVC in active pool: ",
      (await activePool.getVC()).toString()
    );
    console.log(
      "total amount weth in active pool",
      (await weth.balanceOf(activePool.address)).toString()
    );
    ...
    console.log(
      "collVC in default pool: ",
      (await defaultPool.getVC()).toString()
    );
    console.log(
      "total amount weth in default pool",
      (await weth.balanceOf(defaultPool.address)).toString()
    );

    console.log(
      "debt in default pool: ",
      (await defaultPool.getSTARDebt()).toString()
    );
    console.log("TCR: ", (await contracts.troveManager.getTCR()).toString());
    console.log(
      "entire system VC: ",
      (await contracts.borrowerOperations.getEntireSystemColl()).toString()
    );
    console.log(
      "entire system eth amount: ",
      (await weth.balanceOf(activePool.address))
        .add(await weth.balanceOf(defaultPool.address))
        .toString()
    );

    console.log(
      "entire system debt: ",
      (await contracts.borrowerOperations.getEntireSystemDebt()).toString()
    );
    ...
    */
    const A_collAfterL1 = A_coll.add(th.applyLiquidationFee(B_coll)).sub(
      toBN(dec(1000, 18))
    );
    assert.isAtMost(
      th.getDifference(
    ...
    const entireSystemColl = (await activePool.getCollateral(weth.address))
      .add(await defaultPool.getCollateral(weth.address))
      .add(await stabilityPool.getCollateral(weth.address))
      .toString();
    assert.equal(
      entireSystemColl,
    ...
    // check STAR gas compensation
    assert.equal((await starToken.balanceOf(owner)).toString(), dec(400, 18));
  });
});
