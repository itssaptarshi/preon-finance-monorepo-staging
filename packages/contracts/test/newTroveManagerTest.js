...
contract("newTroveManager", async (accounts) => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    it("basicliquidation(), basic sanity", async () => {
      ...
      let wethMinted = await th.addERC20(
        contracts.weth,
        whale,
      ...
      let wavaxMinted = await th.addERC20(
        contracts.wavax,
        whale,
        borrowerOperations.address,
        wavaxToMint,
      ...
      const activePoolWavax = await contracts.wavax.balanceOf(
        activePool.address
      );
      ...
      const aliceSTAR = await starToken.balanceOf(alice);
      // console.log("star MINTED:", (aliceSTAR.div(toBN(10 ** 18))).toNumber());
    ...
    it("redistribution: A, B Open. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards", async () => {
      ...
      const test = (await troveManager.getPendingCollRewards(alice))[1][
        wethIDX
      ];
      ...
      */
      ...
      const entireSystemColl = (
        await activePool.getCollateral(contracts.weth.address)
      )
        .add(await defaultPool.getCollateral(contracts.weth.address))
        .toString();
      assert.equal(
        entireSystemColl,
    ...
    it("redeemCollateral() - MultiCollateral: Sanity test for doing one redemption, checks correct amounts.", async () => {
      ...
      // mint weth for Alice and approve borrowerOperations to use it
      let wethMintedAlice = await th.addERC20(
        contracts.weth,
        A,
        borrowerOperations.address,
        wethToMintAlice,
      ...
      // mint wavax for Alice and approve borrowerOperations to use it
      let wavaxMintedAlice = await th.addERC20(
        contracts.wavax,
        A,
        borrowerOperations.address,
        wavaxToMintAlice,
      ...
      await th.openTroveWithColls(contracts, {
        ...
        extraSTARAmount: await getOpenTroveSTARAmount(toBN(dec(6000, 18))),
        oracles: priceFeeds,
      });
      ...
      let wethMintedBob = await th.addERC20(
        contracts.weth,
        B,
        borrowerOperations.address,
        wethToMintBob,
      ...
      let wavaxMintedBob = await th.addERC20(
        contracts.wavax,
        B,
        borrowerOperations.address,
        wavaxToMintBob,
      ...
      await th.openTroveWithColls(contracts, {
        ...
        colls: colls,
        amounts: amountsBob,
        extraSTARAmount: await getOpenTroveSTARAmount(toBN(dec(20000, 18))),
        oracles: priceFeeds,
      });

      //await borrowerOperations.openTrove(th._100pct, await getOpenTroveSTARAmount(dec(6000, 18)), A, A, [contracts.weth.address], [dec(1000, 18)], { from: A })
      //await borrowerOperations.openTrove(th._100pct, await getOpenTroveSTARAmount(dec(20000, 18)), B, B, [contracts.weth.address], [dec(1000, 18)], { from: B })
      ...
      const A_Coll_Balance_Before = A_WETH_Balance_Before.add(
        A_WAVAX_Balance_Before
      );
      const B_WETH_Collateral_Before = (
        await th.getTroveEntireColl(contracts, B)
      )[0];
      const B_WAVAX_Collateral_Before = (
        await th.getTroveEntireColl(contracts, B)
      )[1];
      const B_Coll_Before = B_WETH_Collateral_Before.add(
        B_WAVAX_Collateral_Before
      );

      const active_pool_before = (
        await activePool.getCollateral(contracts.weth.address)
      ).add(await activePool.getCollateral(contracts.wavax.address));

      // STAR redemption is 5000 STAR
      const STARRedemption = dec(1000, 18);
      const finalSTARAmount = await th.estimateSTAREligible(
        contracts,
        STARRedemption
      );
      // console.log("ESTIMATED STAR", finalSTARAmount.toString())
      ...
      const actualCollAmount = th
        .getEmittedRedemptionValues(tx1)[4][0]
        .add(th.getEmittedRedemptionValues(tx1)[4][1]);

      assert.isTrue(actualSTARAmount.eq(finalSTARAmount));
      await th.assertIsApproximatelyEqual(
        actualCollAmount,
        finalSTARAmount.div(toBN(200)),
        100000
      );
      ...
      const A_Coll_Balance_After = A_WETH_Balance_After.add(
        A_WAVAX_Balance_After
      );
      const B_WETH_Collateral_After = (
        await th.getTroveEntireColl(contracts, B)
      )[0];
      const B_WAVAX_Collateral_After = (
        await th.getTroveEntireColl(contracts, B)
      )[1];
      const B_Coll_After = B_WETH_Collateral_After.add(
        B_WAVAX_Collateral_After
      );
      const active_pool_after = (
        await activePool.getCollateral(contracts.weth.address)
      ).add(await activePool.getCollateral(contracts.wavax.address));
      ...
      await th.assertIsApproximatelyEqual(
        ...
        A_Coll_Balance_Before.add(finalSTARAmount.div(th.toBN(200))),
        100000
      );
      //assert.isTrue(A_Coll_Balance_After.eq(A_Coll_Balance_Before.add(finalSTARAmount.div(th.toBN(200)))))

      // Make sure that B lost that amount of collateral
      await th.assertIsApproximatelyEqual(
        B_Coll_After,
        B_Coll_Before.sub(finalSTARAmount.div(th.toBN(200))),
        100000
      );
      //assert.isTrue(B_Coll_After.eq(B_Coll_Before.sub(finalSTARAmount.div(th.toBN(200)))))
      ...
      //assert.isTrue(active_pool_after.eq(active_pool_before.sub(finalSTARAmount.div(th.toBN(200)))))
    });
    ...
    it("redeemCollateral() - MultiCollateral: redeems trove with one collateral first, a trove with multiple collaterals second, and partially redeems third trove with one collateral", async () => {
      ...
      let wethMintedDennis = await th.addERC20(
        contracts.weth,
        D,
        borrowerOperations.address,
        wethToMintDennis,
      ...
      let wavaxMintedDennis = await th.addERC20(
        contracts.wavax,
        D,
        borrowerOperations.address,
        wavaxToMintDennis,
      ...
      // console.log('staramount', (finalSTARAmount.div(toBN(200))).toString())
      //await th.assertIsApproximatelyEqual(actualCollAmount, (finalSTARAmount.div(toBN(200))))
      assert.isTrue(actualCollAmount.eq(finalSTARAmount.div(toBN(200))));
      assert.isTrue(actualSTARFee.lt(finalSTARFeeAmount));
      ...
      const collSurplusPoolAfter = (
        await contracts.collSurplusPool.getCollateral(contracts.weth.address)
      ).add(
        await contracts.collSurplusPool.getCollateral(contracts.wavax.address)
      );
      ...
      const active_pool_after = await activePool.getCollateral(
        contracts.weth.address
      );
      ...
      // console.log('B debt before', (B_debt_before.sub(finalSTARAmount).add(toBN(dec(3600, 18)))).toString())
      //assert.isTrue(B_debt_after.eq(B_debt_before.sub(finalSTARAmount).add(toBN(dec(3600, 18)))))
      await th.assertIsApproximatelyEqual(
        B_debt_after,
      ...
      // console.log('A coll before', (A_Coll_Balance_Before.add(finalSTARAmount.div(th.toBN(200)))).toString())
      //assert.isTrue(A_Coll_Balance_After.eq(A_Coll_Balance_Before.add(finalSTARAmount.div(th.toBN(200)))))
      await th.assertIsApproximatelyEqual(
        A_Coll_Balance_After,
        A_Coll_Balance_Before.add(finalSTARAmount.div(th.toBN(200))),
      ...
      //assert.isTrue(B_WETH_Collateral_After.eq(B_WETH_Collateral_Before.sub(finalSTARAmount.div(th.toBN(200))).add(toBN(dec(18, 18)))))
      await th.assertIsApproximatelyEqual(
      ...
      // Make sure active pool has decreased by the correct amount of WETH
      //assert.isTrue(active_pool_after.eq(active_pool_before.sub(finalSTARAmount.div(th.toBN(200))).sub(toBN(dec(82, 18)))))
      await th.assertIsApproximatelyEqual(
        active_pool_after,
        active_pool_before
    ...
    it("redeemCollateral() - MultiCollateral: different weth to wavax ratio for trove D", async () => {
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveSTARAmount(dec(4000, 18)),
        C,
        C,
        [contracts.weth.address],
        [dec(50, 18)],
      ...
      // STAR redemption is 8000 STAR. Should close D, C and take some of B.
      const STARRedemption = dec(8000, 18);
      const finalSTARAmount = await th.estimateSTAREligible(
        contracts,
        STARRedemption
      );
      // console.log("finalSTARAMOUNT", finalSTARAmount.toString())
      const finalSTARFeeAmount = th.toBN(STARRedemption).sub(finalSTARAmount);
      ...
      // Assert that B's debt has decreased by the correct amount of STAR. 1800 + 3800 offset from other troves.
      // console.log('B debt after', B_debt_after.toString())
      // console.log('B debt before', (B_debt_before.sub(finalSTARAmount).add(toBN(dec(5600, 18)))).toString())
      //assert.isTrue(B_debt_after.eq(B_debt_before.sub(finalSTARAmount).add(toBN(dec(3600, 18)))))
      await th.assertIsApproximatelyEqual(
        B_debt_after,
        B_debt_before.sub(finalSTARAmount).add(toBN(dec(5600, 18))),
        100000
      );
      ...
      //Trove A reedems 1800 STAR from trove D, which has 10 WAVAX and 40 ETH. Trove A's WAVAX balance = 1.8 WAVAX = (1800 STAR x (10 WAVAX / 50 WAVAX/WETH))/200
    ...
    it("redeemCollateral() - MultiCollateral: perform two separate redemptions, check if two troves positions update, check if final ICR's are correct", async () => {
      ...
      let wethMintedCarol = await th.addERC20(
        contracts.weth,
        C,
        borrowerOperations.address,
        wethToMintCarol,
      ...
      let wavaxMintedCarol = await th.addERC20(
        contracts.wavax,
        C,
        borrowerOperations.address,
        wavaxToMintCarol,
      ...
      await th.openTroveWithColls(contracts, {
        from: C,
        colls: colls,
        STARAmount: await getOpenTroveSTARAmount(dec(3000, 18)),
      ...
      await th.openTroveWithColls(contracts, {
        from: D,
        colls: colls,
        STARAmount: await getOpenTroveSTARAmount(dec(4000, 18)),
        amounts: amountsDennis,
        priceFeeds: priceFeeds,
      });
      ...
      // STAR redemption is 2000 STAR. Shouldn't fully redeem any troves.
      const STARRedemption = dec(2000, 18);
      const finalSTARAmount = await th.estimateSTAREligible(
        contracts,
        STARRedemption
      );
      ...
      const D_totalColl = (await th.getTroveEntireColl(contracts, D))[0]
        .add((await th.getTroveEntireColl(contracts, D))[1])
        .mul(toBN(200));
      const D_totalDebt = await th.getTroveEntireDebt(contracts, D);
      assert(D_totalDebt.eq(toBN(dec(4000, 18)).sub(finalSTARAmount)));
      await th.assertIsApproximatelyEqual(
        D_totalColl,
        toBN(dec(10000, 18)).sub(finalSTARAmount),
        100000
      );

      const STARRedemption2 = dec(1000, 18);
      const finalSTARAmount2 = await th.estimateSTAREligible(
        contracts,
        STARRedemption2
      );
      ...
      const C_totalColl = (await th.getTroveEntireColl(contracts, C))[0]
        .add((await th.getTroveEntireColl(contracts, C))[1])
        .mul(toBN(200));
      const C_totalDebt = await th.getTroveEntireDebt(contracts, C);
      assert(C_totalDebt.eq(toBN(dec(3000, 18)).sub(finalSTARAmount2)));
      await th.assertIsApproximatelyEqual(
        C_totalColl,
        toBN(dec(10000, 18)).sub(finalSTARAmount2),
        100000
      );
    ...
    it("redeemCollateral() - MultiCollateral: fully redeem trove C with riskytoken and weth, partially redeem trove B", async () => {
      ...
      let tokenRiskyMinted = await th.addERC20(
        tokenSuperRisky,
        C,
        borrowerOperations.address,
        tokenRiskyMintCarol,
      ...
      const active_pool_before = (
        await activePool.getCollateral(contracts.weth.address)
      ).add(await activePool.getCollateral(tokenSuperRisky.address));

      // STAR redemption is 3000 STAR.
      ...
      // 3000 STAR - gascomp = 2800 STAR -> (2800 STAR debt / 5600 STAR collat) x Amount of Risky tokens = 8
      // Trove A redeems 2888.538 STAR. Fully redeems Trove C. Partially redeems Trove B's ETH (~88.538 STAR in ETH)
      assert.isTrue(actualRISKYAmount.eq(toBN(dec(8, 18))));
      const ETHinSTAR = actualETHAmount.mul(toBN(200));
      const RISKYinSTAR = actualRISKYAmount.mul(toBN(100));
      // console.log(ETHinSTAR.toString())
      // console.log(finalSTARAmount.sub(RISKYinSTAR).toString())

      //check if ETH redeemed equals total STAR redeemed - risky token redeemed
      await th.assertIsApproximatelyEqual(
        ETHinSTAR,
        finalSTARAmount.sub(RISKYinSTAR),
        100000
      );
      const collSurplusPoolAfter = (
        await contracts.collSurplusPool.getCollateral(contracts.weth.address)
      ).add(
        await contracts.collSurplusPool.getCollateral(tokenSuperRisky.address)
      );
      // console.log('eth surplus after', (await contracts.collSurplusPool.getCollateral(contracts.weth.address)).toString())
    ...
    it("redeemCollateral() - MultiCollateral: fully redeem trove C with stablecoin and weth, partially redeem trove B", async () => {
      ...
      let tokenStableMinted = await th.addERC20(
        stableCoin,
        C,
        borrowerOperations.address,
        stableToMintCarol,
      ...
      const active_pool_before = (
        await activePool.getCollateral(contracts.weth.address)
      ).add(await activePool.getCollateral(stableCoin.address));
      ...
      const ETHinSTAR = actualETHAmount.mul(toBN(200));
      // console.log(ETHinSTAR.toString())
      // console.log(finalSTARAmount.sub(actualSTABLEAmount).toString())

      //check if ETH redeemed equals total STAR redeemed - stable coin redeemed
      await th.assertIsApproximatelyEqual(
        ETHinSTAR,
        finalSTARAmount.sub(actualSTABLEAmount),
        100000
      );
      const collSurplusPoolAfter = (
        await contracts.collSurplusPool.getCollateral(contracts.weth.address)
      ).add(await contracts.collSurplusPool.getCollateral(stableCoin.address));
      // console.log('eth surplus after', (await contracts.collSurplusPool.getCollateral(contracts.weth.address)).toString())
    ...
    it("redeemCollateral() - MultiCollateral: partially redeem trove C with riskytoken stablecoin and weth", async () => {
      ...
      let stableCoinMinted = await th.addERC20(
        stableCoin,
        C,
        borrowerOperations.address,
        stableToMintCarol,
      ...
      await th.openTroveWithColls(contracts, {
        from: C,
        colls: colls,
        STARAmount: await getOpenTroveSTARAmount(dec(5150, 18)),
        amounts: amountsCarol,
        priceFeeds: priceFeeds,
      });
      ...
      const A_Coll_Balance_Before = A_WETH_Balance_Before.add(
        A_RISKY_Balance_Before
      ).add(A_STABLE_Balance_Before);

      // STAR redemption is 1320 STAR
      const STARRedemption = toBN(dec(1320, 18));
      ...
      // (1320 STAR debt / 6600 STAR collat) = 0.2
    ...
    it("redeemCollateral() - MultiCollateral: redeems trove C with riskytoken stablecoin and weth TWICE", async () => {
      ...
      const STARRedemption2 = toBN(dec(1320, 18));
    ...
    it("updateTroves(): When only one trove is updated it keeps the old icr values correctly. Multi Collateral. Other operations also work with trove manager update function", async function () {
      ...
      await assertSortedListIsOrdered(contracts);
      assert.isTrue(
        (await sortedTroves.getOldICR(B)).eq(
          await troveManager.getCurrentICR(B)
        )
      );
      ...
      await borrowerOperations.adjustTrove(
        [contracts.weth.address, stableCoin.address],
        [toBN(dec(1, 18)), toBN(dec(1000, 18))],
        [],
        [],
        0,
        false,
      ...
      assert.isTrue(
        (await sortedTroves.getOldICR(C)).eq(
          await troveManager.getCurrentICR(C)
        )
      );
      await assertSortedListIsOrdered(contracts);
      ...
      await th.openTroveWithColls(contracts, {
        ICR: toBN(dec(300, 16)),
        colls: [contracts.weth, stableCoin],
        amounts: [toBN(dec(2000, 18)), toBN(dec(500, 18))],
        from: F,
      });
      assert.isTrue(
        (await sortedTroves.getOldICR(F)).eq(
          await troveManager.getCurrentICR(F)
        )
      );
      await assertSortedListIsOrdered(contracts);
      ...
      assert.isTrue(
        (await sortedTroves.getOldICR(bottomBeforeRedemption)).eq(
          await troveManager.getCurrentICR(bottomBeforeRedemption)
        )
      );
      assert.isTrue(
        ICRBefore.lt(await troveManager.getCurrentICR(bottomBeforeRedemption))
      );
      await assertSortedListIsOrdered(contracts);
    });
  };
  ...
  // Sequentially add coll and withdraw STAR, 1 account at a time
  const makeTrovesInSequence = async () => {
    ...
    const allAmounts = [
      ...
      toBN(dec(200000, 18))
        .mul(toBN(dec(1, 18)))
        .div(toBN(dec(105, 16))), // price = 1. Ratio = 1.05. Collateral amount = 200000 / 1.05
      toBN(dec(200000, 18))
        .mul(toBN(dec(1, 36)))
        .div(toBN(dec(75, 16)))
        .div(toBN(dec(200, 18))),
    ]; // price = 200. Ratio = 0.75. Collateral amount = 200000 / 200 / 0.75 = 100 / 0.75
