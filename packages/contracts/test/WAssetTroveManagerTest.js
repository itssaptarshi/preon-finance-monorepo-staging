...
const {
  ...
  getJOEContracts,
  wrapAVAX,
  zapForWAVAX_WETH_JLP,
  approveJLP,
} = require("../utils/joeHelper.js");
...
contract("WAssetTroveManagerTests", async (accounts) => {
  ...
  afterEach(async () => {
    ...
    await jlp
      .connect(await hre.ethers.getSigner(alice))
      .transfer(zeroAddress, aliceJLPBalance);
    await jlp
      .connect(await hre.ethers.getSigner(harry))
      .transfer(zeroAddress, harryJLPBalance);
  ...
  it("Wrapping for someone else reverts when not called by borrower operations", async () => {
    ...
    await assertRevert(
      WJLP.wrap(amount, harry, alice, alice, { from: alice }),
  ...
  it("open trove and liquidate-standard liquidation one Trove liquidated and one SP depositor", async () => {
    ...
    const STARBalance = await starToken.balanceOf(alice);
    await stabilityPool.provideToSP(STARBalance, zeroAddress, { from: alice });
    ...
    assert.equal(
      treasuryRewardsMid["0"].toString(),
      expectedWJLP_inSP.toString()
    );
    ...
    await stabilityPool.withdrawFromSP(STARBalance, { from: alice });
  ...
  it("Partial Redemption using wasset, check balances. Partial redemption", async () => {
    ...
    const STARRedemption = toBN(dec(2000, 18));
    ...
    const ActivePoolWJLPAfter = await WJLP.balanceOf(
    ...
    assert.isTrue(
      ActivePoolWJLPAfter.toString() == harryRewardsFinal[0].toString()
    );
  ...
  it("Full Redemption using wasset, check balances", async () => {
    ...
    const STARRedemption = toBN(dec(4000, 18));
    const tx1 = await th.performRedemptionWithMaxFeeAmount(
      alice,
      contracts,
      STARRedemption,
    ...
    const collSurplusPoolWJLPAfter = await WJLP.balanceOf(
    ...
    assert.isTrue(
      collSurplusPoolWJLPAfter.toString() == harryRewardsFinal[0].toString()
    );
  ...
  it.only("Tests multiple borrowers interacting with WJLP and adjusting trove with wjlp", async () => {
    ...
    await jlp
      .connect(await hre.ethers.getSigner(harry))
      .transfer(alice, harryJLPBalance);
    ...
    await contracts.borrowerOperations.openTrove(
      ...
      th.ZERO_ADDRESS,
      th.ZERO_ADDRESS,
      [WJLP.address, weth.address],
      [amount.toString(), toBN(dec(20, 18))],
      { from: alice }
    );
    ...
    await contracts.borrowerOperations.withdrawColl(
      [WJLP.address],
      [dec(1, 18)],
      harry,
      harry,
    ...
    await contracts.starToken.transfer(harry, toBN(dec(1000, 18)), {
      from: alice,
    });
    await contracts.borrowerOperations.closeTrove({ from: harry });
    const harryBalanceFinal = await jlp.balanceOf(harry);
    assert.isTrue(
      harryBalanceFinal.toString() == amount3.add(amount2).toString()
    );
  });
  ...
  //   await th.openTrove(contracts, { ICR: toBN(dec(2, 18)), extraSTARAmount: toBN(dec(2, 25)), extraParams: { from: alice } })
  //
  ...
  //     //
  //     // th.redeemCollateralAndGetTxObject(alice, contracts, starToken.balanceOf(alice))
  //     //
  //     // // fastforward time
  //     // await ethers.provider.send('evm_increaseTime', [2592000]);
  //     // await ethers.provider.send('evm_mine');
  ...
  //
  //     await stabilityPool.provideToSP(starToken.balanceOf(alice), ZERO_ADDRESS, { from: alice })
  //
  //     // fastforward time
  //     await ethers.provider.send('evm_increaseTime', [2592000]);
  //     await ethers.provider.send('evm_mine');
