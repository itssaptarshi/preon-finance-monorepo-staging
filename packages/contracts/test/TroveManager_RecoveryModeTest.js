...
contract("TroveManager - in Recovery Mode", async (accounts) => {
  ...
  it("checkRecoveryMode(): Returns true if TCR falls below CCR", async () => {
    ...
    // price drops to 1ETH:150STAR, reducing TCR below 150%.  setPrice() calls checkTCRAndSetRecoveryMode() internally.
  ...
  it("liquidate(), with ICR < 100%: removes stake and updates totalStakes", async () => {
    ...
    const bob_Stake_Before = await troveManager.getTroveStake(
      bob,
      weth.address
    );
  ...
  it("liquidate(), with ICR < 100%: updates system snapshots correctly", async () => {
    ...
    // --- TEST ---
    // price drops to 1ETH:100STAR, reducing TCR below 150%, and all Troves below 100% ICR
    await priceFeed.setPrice("100000000000000000000");
    ...
    const totalCollateralSnapshot_before000 = (
  ...
  it("liquidate(), with 100 < ICR < 110%: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    //  Bob withdraws up to 2000 STAR of debt, bringing his ICR to 210%
    ...
    // --- TEST ---
    // price drops to 1ETH:100STAR, reducing TCR to 117%
    await priceFeed.setPrice("100000000000000000000");
  ...
  it("liquidate(), with 100% < ICR < 110%: updates system snapshots correctly", async () => {
    ...
    //  Bob withdraws up to 20000 STAR of debt, bringing his ICR to 210%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(150, 16)),
      extraParams: { from: alice },
    });
  ...
  it("liquidate(), with 100% < ICR < 110%: offsets as much debt as possible with the Stability Pool, then redistributes the remainder coll and debt", async () => {
    ...
    // Alice deposits 390STAR to the Stability Pool
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: alice });
    ...
    // check pool STAR before liquidation
    const stabilityPoolSTAR_Before = (
      await stabilityPool.getTotalSTARDeposits()
    ).toString();
    assert.equal(stabilityPoolSTAR_Before, "390000000000000000000");
    ...
    /* Now, liquidate Bob. Liquidated coll is 21 ether, and liquidated debt is 2000 STAR.
    
    With 390 STAR in the StabilityPool, 390 STAR should be offset with the pool, leaving 0 in the pool.
  
    Stability Pool rewards for alice should be:
    STARLoss: 390STAR
    ETHGain: (390 / 2000) * 21*0.995 = 4.074525 ether

    After offsetting 390 STAR and 4.074525 ether, the remainders - 1610 STAR and 16.820475 ether - should be redistributed to all active Troves.
   */
    // Liquidate Bob
    await troveManager.liquidate(bob, { from: owner });

    const aliceDeposit = await stabilityPool.getCompoundedSTARDeposit(alice);
    ...
    /* Now, check redistribution to active Troves. Remainders of 1610 STAR and 16.82 ether are distributed.
    ...
    L_STARDebt = 1610 / 6 = 268.333 STAR
    L_ETH = 16.820475 /6 =  2.8034125 ether
    */
    const L_STARDebt = (await troveManager.getL_STAR(weth.address)).toString();
  ...
  it("liquidate(), with ICR > 110%, trove has lowest ICR, and StabilityPool is empty: does nothing", async () => {
    ...
    const bob_Stake = (
      await troveManager.getTroveStake(bob, weth.address)
    ).toString();
  ...
  // --- liquidate(), applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool STAR is GREATER THAN liquidated debt ---

  it("liquidate(), with 110% < ICR < TCR, and StabilityPool STAR > debt to liquidate: offsets the trove entirely with the pool", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 STAR of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 STAR of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(240, 16)),
      extraSTARAmount: dec(250, 18),
      extraParams: { from: bob },
    });
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
    ...
    // Alice deposits STAR in the Stability Pool
    ...
    /* Check accrued Stability Pool rewards after. Total Pool deposits was 1490 STAR, Alice sole depositor.
    As liquidated debt (250 STAR) was completely offset

    Alice's expected compounded deposit: (1490 - 250) = 1240STAR
    Alice's expected ETH gain:  Bob's liquidated capped coll (minus gas comp), 2.75*0.995 ether
  
    */
    const aliceExpectedDeposit = await stabilityPool.getCompoundedSTARDeposit(
      alice
    );
    const aliceExpectedETHGain = (
      await stabilityPool.getDepositorGains(alice)
    )[1][0].toString();
    console.log("----------");
    console.log("aliceExpectedETHGain", aliceExpectedETHGain);

    const testGain = await th.applyLiquidationFee(
    ...
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getCollateral(contracts.weth.address),
  ...
  it("liquidate(), with ICR% = 110 < TCR, and StabilityPool STAR > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 STAR of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 STAR of debt, resulting in ICR of 220%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(220, 16)),
      extraSTARAmount: dec(250, 18),
      extraParams: { from: bob },
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool STAR > debt to liquidate: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 STAR of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 STAR of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool STAR > debt to liquidate: updates system snapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 STAR of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 STAR of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool STAR > debt to liquidate: closes the Trove", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 STAR of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 STAR of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool STAR > debt to liquidate: can liquidate troves out of order", async () => {
    // taking out 1000 STAR, CR of 200%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: alice },
    });
    ...
    await openTrove({
      ICR: toBN(dec(200, 16)),
      extraSTARAmount: totalLiquidatedDebt,
    ...
    th.assertIsApproximatelyEqual(
      ...
      carol_remainingCollateral
        .add(bob_remainingCollateral)
        .add(dennis_remainingCollateral)
    );
  ...
  STAR is LESS THAN the liquidated debt: a non fullfilled liquidation --- */

  it("liquidate(), with ICR > 110%, and StabilityPool STAR < liquidated debt: Trove remains active", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 STAR of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 STAR of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
      extraSTARAmount: dec(1500, 18),
      extraParams: { from: alice },
    });
    const { collateral: B_coll } = await openTrove({
      ICR: toBN(dec(240, 16)),
    ...
    // Alice deposits 1490 STAR in the Stability Pool
    ...
    /* Since the pool only contains 100 STAR, and Bob's pre-liquidation debt was 250 STAR,
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool STAR < liquidated debt: Trove remains in TroveOwners array", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 STAR of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 STAR of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
    ...
    // Alice deposits 100 STAR in the Stability Pool
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool STAR < liquidated debt: nothing happens", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 STAR of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 STAR of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
    ...
    /*  Since Bob's debt (250 STAR) is larger than all STAR in the Stability Pool, Liquidation won’t happen
    ...
    */
    ...
    const bob_CollAfter = (
      await troveManager.getTroveColls(bob)
    )[1][0].toString();
    const bob_StakeAfter = (
      await troveManager.getTroveStake(bob, weth.address)
    ).toString();
    ...
    const totalStakes_After = (
      await troveManager.totalStakes(weth.address)
    ).toString();
    assert.equal(totalStakes_After.toString(), A_coll.add(B_coll).add(D_coll));
  });

  it("liquidate(), with ICR > 110%, and StabilityPool STAR < liquidated debt: updates system shapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 STAR of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 STAR of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool STAR < liquidated debt: causes correct Pool offset and ETH gain, and doesn't redistribute to active troves", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 STAR of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 STAR of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
    ...
    const L_STARDebt_After = (
      await troveManager.getL_STAR(weth.address)
    ).toString();
    const L_ETH_After = (await troveManager.getL_Coll(weth.address)).toString();

    assert.equal(L_STARDebt_After, "0");
    assert.equal(L_ETH_After, "0");
  });

  it("liquidate(), with ICR > 110%, and StabilityPool STAR < liquidated debt: ICR of non liquidated trove does not change", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 STAR of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 STAR of debt, resulting in ICR of 240%. Bob has lowest ICR.
    // Carol withdraws up to debt of 240 STAR, -> ICR of 250%.
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
      extraSTARAmount: dec(1500, 18),
      extraParams: { from: alice },
    ...
    //Check SP STAR has been completely emptied
    assert.equal(
    ...
    await starToken.transfer(bob, dec(100, 18), { from: alice });
    ...
    // Alice provides another 50 STAR to pool
  ...
  it("liquidate() with ICR > 110%, and StabilityPool STAR < liquidated debt: total liquidated coll and debt is correct", async () => {
    // Whale provides 50 STAR to the SP
    await openTrove({
      ICR: toBN(dec(300, 16)),
      extraSTARAmount: dec(50, 18),
      extraParams: { from: whale },
    });
  ...
  it("liquidate(): Doesn't liquidate undercollateralized trove if it is the only trove in the system", async () => {
    // Alice creates a single trove with 0.62 ETH and a debt of 62 STAR, and provides 10 STAR to SP
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    /*  Since there is 0 STAR in the stability Pool, A, with ICR >110%, should stay active.
  ...
  it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
    ...
    //Dennis provides 200 STAR to SP
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({
      ICR: toBN(dec(220, 16)),
      extraSTARAmount: dec(1000, 18),
      extraParams: { from: whale },
    });

    const { starAmount: A_starAmount } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraSTARAmount: dec(300, 18),
      extraParams: { from: alice },
    });
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, re-open, be reedemed and claim again", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 STAR of debt, resulting in ICRs of 266%.
    // Bob withdraws up to 480 STAR of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(240, 16)),
      extraSTARAmount: dec(480, 18),
      extraParams: { from: bob },
    });
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
    ...
    // Bob re-opens the trove, price 200, total debt 80 STAR, ICR = 120% (lowest one)
    ...
    const { collateral: D_coll, netDebt: D_netDebt_2 } = await openTrove({
      ICR: toBN(dec(266, 16)),
      extraSTARAmount: B_netDebt_2,
      extraParams: { from: dennis },
    });
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, after another claim from a redemption", async () => {
    // --- SETUP ---
    // Bob withdraws up to 90 STAR of debt, resulting in ICR of 222%
    const { collateral: B_coll, netDebt: B_netDebt } = await openTrove({
      ICR: toBN(dec(222, 16)),
      extraSTARAmount: dec(90, 18),
      extraParams: { from: bob },
    });
    // Dennis withdraws to 150 STAR of debt, resulting in ICRs of 266%.
    const { collateral: D_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
      extraSTARAmount: B_netDebt,
      extraParams: { from: dennis },
    });
    ...
    // Bob re-opens the trove, price 200, total debt 250 STAR, ICR = 240% (lowest one)
    ...
    th.assertIsApproximatelyEqual(
      (await collSurplusPool.getCollateral(contracts.weth.address)).toString(),
  ...
  xit("liquidateTroves(): With all ICRs > 110%, Liquidates Troves until system leaves recovery mode", async () => {
    ...
    // Everyone withdraws some STAR from their Trove, resulting in different ICRs
    ...
    await openTrove({
      ICR: toBN(dec(400, 16)),
      extraSTARAmount: liquidationAmount,
      extraParams: { from: alice },
    });

    // Alice deposits STAR to Stability Pool
    await stabilityPool.provideToSP(liquidationAmount, ZERO_ADDRESS, {
      from: alice,
    });

    // price drops
    // price drops to 1ETH:90STAR, reducing TCR below 150%
  ...
  xit("liquidateTroves(): Liquidates Troves until 1) system has left recovery mode AND 2) it reaches a Trove with ICR >= 110%", async () => {
    ...
    // price drops to 1ETH:85STAR, reducing TCR below 150%
  ...
  xit("liquidateTroves(): a liquidation sequence containing Pool offsets increases the TCR", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(198, 16)),
      extraSTARAmount: dec(101, 18),
      extraParams: { from: defaulter_1 },
    });
    await openTrove({
      ICR: toBN(dec(184, 16)),
      extraSTARAmount: dec(217, 18),
      extraParams: { from: defaulter_2 },
    });
    await openTrove({
      ICR: toBN(dec(183, 16)),
      extraSTARAmount: dec(328, 18),
      extraParams: { from: defaulter_3 },
    });
    await openTrove({
      ICR: toBN(dec(186, 16)),
      extraSTARAmount: dec(431, 18),
    ...
    // Check Stability Pool has 500 STAR
    assert.equal(
      (await stabilityPool.getTotalSTARDeposits()).toString(),
      dec(500, 18)
    );
  ...
  xit("liquidateTroves(): does nothing if all troves have ICR > 110% and Stability Pool is empty", async () => {
    ...
    // Confirm 0 STAR in Stability Pool
    assert.equal((await stabilityPool.getTotalSTARDeposits()).toString(), "0");
  ...
  xit("liquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves", async () => {
    ...
    // Confirm STAR in Stability Pool
    assert.equal(
      (await stabilityPool.getTotalSTARDeposits()).toString(),
      spDeposit.toString()
    );
  ...
  xit("liquidateTroves():  emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial", async () => {
    ...
    const entireColl_A = (await troveManager.getTroveColls(alice))[1][0].add(
      await troveManager.getPendingETHReward(alice)
    );
    const entireDebt_A = (await troveManager.getTroveDebt(alice)).add(
      await troveManager.getPendingSTARDebtReward(alice)
    );
  ...
  xit("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const { starAmount: starAmountD } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: dennis },
    });
    const { starAmount: starAmountE } = await openTrove({
      ICR: toBN(dec(133, 16)),
      extraParams: { from: erin },
    });
    const { starAmount: starAmountF } = await openTrove({
      ICR: toBN(dec(111, 16)),
      extraParams: { from: freddy },
    });
    ...
    assert.equal((await starToken.balanceOf(dennis)).toString(), starAmountD);
    assert.equal((await starToken.balanceOf(erin)).toString(), starAmountE);
    assert.equal((await starToken.balanceOf(freddy)).toString(), starAmountF);
  ...
  xit("liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
    // Whale provides STAR to the SP
    const { starAmount: W_starAmount } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraSTARAmount: dec(4000, 18),
      extraParams: { from: whale },
    });
    await stabilityPool.provideToSP(W_starAmount, ZERO_ADDRESS, {
      from: whale,
    });

    const {
      starAmount: A_starAmount,
    ...
    await stabilityPool.provideToSP(A_starAmount, ZERO_ADDRESS, {
      from: alice,
    });
    await stabilityPool.provideToSP(B_starAmount, ZERO_ADDRESS, { from: bob });

    const totalDeposit = W_starAmount.add(A_starAmount).add(B_starAmount);
    ...
    Total STAR in Pool: 680 STAR

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 100 + 300 + 100 = 500 STAR
    Total liquidated ETH = 1 + 3 + 1 = 5 ETH

    Whale STAR Loss: 500 * (400/680) = 294.12 STAR
    Alice STAR Loss:  500 *(40/680) = 29.41 STAR
    Bob STAR Loss: 500 * (240/680) = 176.47 STAR

    Whale remaining deposit: (400 - 294.12) = 105.88 STAR
    Alice remaining deposit: (40 - 29.41) = 10.59 STAR
    Bob remaining deposit: (240 - 176.47) = 63.53 STAR

    Whale ETH Gain: 5*0.995 * (400/680) = 2.93 ETH
    Alice ETH Gain: 5*0.995 *(40/680) = 0.293 ETH
    Bob ETH Gain: 5*0.995 * (240/680) = 1.76 ETH

    Total remaining deposits: 180 STAR
    ...
    const whale_Deposit_After = (
      await stabilityPool.getCompoundedSTARDeposit(whale)
    ).toString();
    ...
    assert.isAtMost(
      th.getDifference(
        whale_Deposit_After,
        W_starAmount.sub(liquidatedDebt.mul(W_starAmount).div(totalDeposit))
      ),
      100000
    );
    assert.isAtMost(
      th.getDifference(
        alice_Deposit_After,
        A_starAmount.sub(liquidatedDebt.mul(A_starAmount).div(totalDeposit))
      ),
      100000
    );
    assert.isAtMost(
      th.getDifference(
        bob_Deposit_After,
        B_starAmount.sub(liquidatedDebt.mul(B_starAmount).div(totalDeposit))
      ),
      100000
    );
    ...
    assert.isAtMost(
      th.getDifference(total_STARinSP, totalDeposit.sub(liquidatedDebt)),
      1000
    );
    assert.isAtMost(
  ...
  xit("liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or ETH gain", async () => {
    ...
    // Check STAR and ETH in Pool after
  ...
  xit("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains active", async () => {
    ...
    That leaves 50 STAR in the Pool to absorb exactly half of Carol's debt (100) */
    await troveManager.liquidateTroves(10);
  ...
  xit("liquidateTroves() with a non fullfilled liquidation: still can liquidate further troves after the non-liquidated, emptied pool", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
     ...
     That leaves 97 STAR in the Pool that won’t be enough to absorb Carol,
  ...
  xit("liquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 STAR in the Pool that won’t be enough to absorb any other trove */
    const tx = await troveManager.liquidateTroves(10);

    // Expect system debt reduced by 203 STAR and system coll 2.3 ETH
    const entireSystemCollAfter = await troveManager.getEntireSystemColl();
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt();
  ...
  xit("liquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 STAR in the Pool which won’t be enough for any other liquidation */
    const liquidationTx = await troveManager.liquidateTroves(10);
    ...
    assert.equal(starGasComp.toString(), dec(400, 18));
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Recovery -> Normal Mode", async () => {
    ...
    */
    ...
    // call batchLiquidateTroves
    await troveManager.batchLiquidateTroves(
      [bob, carol, dennis, erin, freddy, alice],
      owner
    );
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    await starToken.transfer(alice, A_totalDebt, { from: whale });
    // Alice closes trove
    ...
    */
    ...
    // call batchLiquidateTroves
    await troveManager.batchLiquidateTroves(
      [alice, bob, bob, carol, dennis, erin, freddy],
      owner
    );
  ...
  it("batchLiquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    // Expect system debt reduced by 203 STAR and system coll by 2 ETH
    const entireSystemCollAfter = await troveManager.getEntireSystemColl();
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt();
    ...
    th.assertIsApproximatelyEqual(
      changeInEntireSystemDebt.toString(),
      A_totalDebt.add(B_totalDebt).toString()
    );
    assert.equal(
      changeInEntireSystemColl.div(toBN(120)).toString(),
      A_coll.add(B_coll).toString()
    );
  });
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool STAR > debt to liquidate: can liquidate troves out of order", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: alice },
    });
    ...
    // Whale provides 1000 STAR to the SP
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool empty: doesn't liquidate any troves", async () => {
    ...
    const trovesToLiquidate = [dennis, bob, carol];
    await assertRevert(
      troveManager.batchLiquidateTroves(trovesToLiquidate, owner),
      "TroveManager: nothing to liquidate"
    );
    ...
    const dennisDebt_After = (await troveManager.getTroveDebt(dennis)).add(
      await troveManager.getPendingSTARDebtReward(dennis)
    );
    const bobDebt_After = (await troveManager.getTroveDebt(bob)).add(
      await troveManager.getPendingSTARDebtReward(bob)
    );
    const carolDebt_After = (await troveManager.getTroveDebt(carol)).add(
      await troveManager.getPendingSTARDebtReward(carol)
    );

    const dennisColl_After = (
      await troveManager.getTroveColls(dennis)
    )[1][0].add((await troveManager.getPendingCollRewards(dennis))[1][0]);
    const bobColl_After = (await troveManager.getTroveColls(bob))[1][0].add(
      (await troveManager.getPendingCollRewards(bob))[1][0]
    );
    const carolColl_After = (await troveManager.getTroveColls(carol))[1][0].add(
      (await troveManager.getPendingCollRewards(carol))[1][0]
    );
  ...
  it("batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size", async () => {
    ...
    await assertRevert(
      troveManager.batchLiquidateTroves([G, H, I], owner),
      "TroveManager: nothing to liquidate"
    );
    ...
    assert.equal(
      G_collBefore.toString(),
      (await troveManager.getTroveColls(G))[1][0].toString()
    );
    assert.equal(
      G_debtBefore.toString(),
      (await troveManager.getTroveDebt(G)).toString()
    );
    assert.equal(
      H_collBefore.toString(),
      (await troveManager.getTroveColls(H))[1][0].toString()
    );
    assert.equal(
      H_debtBefore.toString(),
      (await troveManager.getTroveDebt(H)).toString()
    );
    assert.equal(
      I_collBefore.toString(),
      (await troveManager.getTroveColls(I))[1][0].toString()
    );
    assert.equal(
      I_debtBefore.toString(),
      (await troveManager.getTroveDebt(I)).toString()
    );
    ...
    // Whale withdraws entire deposit, and re-deposits 132 STAR
    ...
    // Expected Stability Pool to fully absorb B (92 STAR + 10 virtual debt),
    // but not E as there are not enough funds in Stability Pool

    const stabilityBefore = await stabilityPool.getTotalSTARDeposits();
    ...
    const stabilityAfter = await stabilityPool.getTotalSTARDeposits();
  ...
  it("batchLiquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves", async () => {
    ...
    const [
      liquidatedDebt,
      starGasComp,
      liquidatedCollTokens,
      liquidatedCollAmounts,
      totalCollGasCompTokens,
    ...
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getCollateral(contracts.weth.address),
      freddy_remainingCollateral.add(greta_remainingCollateral)
    );
  ...
  it("batchLiquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial", async () => {
    ...
    // Whale opens trove and adds 220 STAR to SP
    const spDeposit = F_totalDebt.add(G_totalDebt).add(
      A_totalDebt.div(toBN(2))
    );
    await openTrove({
    ...
    const entireColl_A = (await troveManager.getTroveColls(alice))[1][0].add(
      (await troveManager.getPendingCollRewards(alice))[1][0]
    );
    const entireDebt_A = (await troveManager.getTroveDebt(alice)).add(
      await troveManager.getPendingSTARDebtReward(alice)
    );
