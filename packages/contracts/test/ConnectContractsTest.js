...
contract(
  ...
  async (accounts) => {
    ...
    before(async () => {
      ...
      starToken = coreContracts.starToken;
      ...
      await deploymentHelper.connectCoreContracts(
        coreContracts,
        PREONContracts
      );
    ...
    it("Sets the correct STARToken address in TroveManager", async () => {
      const starTokenAddress = starToken.address;

      const recordedClvTokenAddress = await troveManager.starToken();

      assert.equal(starTokenAddress, recordedClvTokenAddress);
    });
    ...
    it("Sets the correct STARToken address in StabilityPool", async () => {
      const starTokenAddress = starToken.address;

      const recordedClvTokenAddress = await stabilityPool.starToken();
    ...
    // Sets STARToken in SPREON
    it("Sets the correct STAR Token address in SPREON", async () => {
      const starTokenAddress = starToken.address;

      const recordedSTARTokenAddress = await sPREON.starToken();
      assert.equal(starTokenAddress, recordedSTARTokenAddress);
    });
