...
contract("newBorrowerOperations", async (accounts) => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    before(async () => {
      ...
      await liquityBaseTester.setAddresses(
    ...
    describe("Left Sum Colls Tests", async () => {
      it("leftSumColls(): empty second token/array ", async () => {
        ...
        const amounts1 = Array.from(
          { length: whitelistedAddresses.length },
          () => Math.floor(Math.random() * 120125888)
        );
        const tokens2 = [];
        const amounts2 = [];

        sum = await liquityBaseTester.leftSumColls(
          tokens1,
          amounts1,
          tokens2,
          amounts2
    ...
    describe("Normal Sum Colls Tests", async () => {
      it("Sum arrays Same Length and Tokens ", async () => {
        ...
        sum = await liquityBaseTester.sumCollsTwoColls(
          tokens1,
          amounts1,
          tokens2,
          amounts2
        ...
        sum = await liquityBaseTester.sumCollsOneCollsOneSplit(
          tokens1,
          amounts1,
          tokens2,
          amounts2
        ...
        sum = await liquityBaseTester.sumCollsTwoSplit(
          tokens1,
          amounts1,
          tokens2,
          amounts2
      ...
      it("Sum arrays-same length and 50 collaterals", async () => {
        ...
        const amounts1 = Array.from({ length: tokens1.length }, () =>
          Math.floor(Math.random() * 120125888)
        );

        const tokens2 = whitelistedAddresses;
        // randomly generated N = tokens1.length length array 0 <= A[N] <= 39
        const amounts2 = Array.from({ length: tokens2.length }, () =>
          Math.floor(Math.random() * 1500009)
        );
    ...
    describe("Left Sub Colls Tests", async () => {
      it("leftSubColls(): empty second token/array ", async () => {
        ...
        sub = await liquityBaseTester.leftSubColls(
          tokens1,
          amounts1,
          tokens2,
          amounts2
      ...
      it("leftSubColls(): non-empty second token/array with tokens = all whitelisted tokens", async () => {
        ...
        const amounts1 = Array.from(
          { length: whitelistedAddresses.length },
          () => 130125888 + Math.floor(Math.random() * 120125888)
        );
        const tokens2 = whitelistedAddresses;
        const amounts2 = Array.from(
          { length: whitelistedAddresses.length },
      ...
      it("leftSubColls(): non-empty second token/array in order with tokens != all whitelisted tokens", async () => {
        ...
        const amounts1 = Array.from(
          { length: whitelistedAddresses.length },
          () => 300000 + Math.floor(Math.random() * 120125888)
        );
        const tokens2 = [a2, a4];
        const amounts2 = [1502, 12490];
        let amounts3 = [...amounts1];
    ...
    describe("Normal Sub Colls Tests", async () => {
      it("subColls(): Same Length and Tokens ", async () => {
        ...
        sub = await liquityBaseTester.subColls(
          tokens1,
          amounts1,
          tokens2,
          amounts2
      ...
      it("subColls(): Different Length and Different Tokens Not of Order", async () => {
        ...
        sum = await liquityBaseTester.subColls(
          tokens1,
          amounts1,
          tokens2,
          amounts2
      ...
      it("subColls()-same length and 50 collaterals", async () => {
        ...
        const amounts1 = Array.from(
          ...
          () => 100000000 + Math.floor(Math.random() * 120125888)
        );
