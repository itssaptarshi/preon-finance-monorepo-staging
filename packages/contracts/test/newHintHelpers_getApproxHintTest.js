...
contract("HintHelpers", async (accounts) => {
  ...
  /* Open a Trove for each account. STAR debt is 200 STAR each, with collateral beginning at
  1.5 ether, and rising by 0.01 ether per Trove.  Hence, the ICR of account (i + 1) is always 1% greater than the ICR of account i. 
 */

  // Open Troves in parallel, then withdraw STAR in parallel
  const makeTrovesInParallel = async (accounts, n) => {
    ...
    const withdrawSTARpromises = activeAccounts.map((account) =>
      withdrawSTARfromTrove(account)
    );
    await Promise.all(withdrawSTARpromises);
  ...
  const withdrawSTARfromTrove = async (account) => {
  ...
  const makeTrovesInSequence = async (accounts, n) => {
    ...
    for (const account of activeAccounts) {
      ...
      //   await th.openTrove(contracts, { extraSTARAmount: toBN(dec(10000, 18)), ICR: ICR_BN, extraParams: { from: account } })
  ...
  it("getApproxHint(): for 100 random CRs, returns the address of a Trove within sqrt(length) positions of the correct insert position", async () => {
    ...
    for (i = 0; i < 100; i++) {
      ...
      const ICRPercent_hintAddress =
        Number(web3.utils.fromWei(ICR_hintAddress, "ether")) * 100;
