  ...
  async (accounts) => {
    ...
    context("Batch liquidations", () => {
      const setup = async () => {
        ...
        const totalLiquidatedDebt = A_totalDebt.add(B_totalDebt).add(
          C_totalDebt
        );
      ...
      it("Two troves over MCR are liquidated", async () => {
        ...
        // @KingPreon: Our contracts do not throw events
        ...
        // @King Preon: Edited this test case because beleived Liquity's to be incorrect
      ...
      it("Stability Pool profit matches", async () => {
        ...
        const spstarBefore = await stabilityPool.getTotalSTARDeposits();
        ...
        const spstarAfter = await stabilityPool.getTotalSTARDeposits();
        ...
        const expectedGainInSTAR = expectedCollateralLiquidatedA
          .mul(price)
          .div(mv._1e18BN)
          .sub(A_totalDebt);
        const realGainInSTAR = spEthAfter
          .sub(spEthBefore)
          .mul(price)
          .div(mv._1e18BN)
          .sub(spstarBefore.sub(spstarAfter));
        ...
        assert.equal(
          spstarBefore.sub(spstarAfter).toString(),
          A_totalDebt.toString(),
          "Stability Pool STAR doesn’t match"
        );
        assert.equal(
          realGainInSTAR.toString(),
          expectedGainInSTAR.toString(),
