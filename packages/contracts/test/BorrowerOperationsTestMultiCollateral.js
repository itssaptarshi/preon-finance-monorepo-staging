...
const LinearPriceCurve = artifacts.require(
  "./PriceCurves/ThreePieceWiseLinearPriceCurve.sol"
);
...
contract("BorrowerOperations", async (accounts) => {
  ...
  const getTroveEntireTokens = async (trove) =>
    th.getTroveEntireTokens(contracts, trove);
  ...
  const STARMinAmount = toBN("1791044776119402985075");
  ...
  const testCorpus = ({ withProxy = false }) => {
    beforeEach(async () => {
      ...
      let result = await deploymentHelper.deployExtraCollateral(
        contracts,
        paramsA
      );
      tokenA = result.token;
    ...
    describe("openTrove() multi collateral", async () => {
      it("Open a trove with multiple collateral types, check if amounts added are correct", async () => {
        ...
        const activePoolRawBalanceBefore_A = await tokenA.balanceOf(
          activePool.address
        );
        const activePoolRawBalanceBefore_B = await tokenB.balanceOf(
          activePool.address
        );
        const activePoolRawBalanceBefore_C = await tokenC.balanceOf(
          activePool.address
        );
        ...
        const activePoolRawBalanceAfter_A = await tokenA.balanceOf(
          activePool.address
        );
        const activePoolRawBalanceAfter_B = await tokenB.balanceOf(
          activePool.address
        );
        const activePoolRawBalanceAfter_C = await tokenC.balanceOf(
          activePool.address
        );
        ...
        assert.isTrue(
          await th.assertCollateralsEqual(
            aliceTokens,
            aliceColls,
            [tokenA.address, tokenB.address, tokenC.address],
            [dec(5, 18), dec(10, 18), dec(15, 18)]
    ...
    describe("adjustTrove() multi collateral", async () => {
      it("Open a trove with multiple collateral types, then adjust", async () => {
        await th.addMultipleERC20(
          ...
          [dec(500, 18), dec(500, 18), dec(500, 18), dec(500, 18)],
          { from: alice }
        );
        await th.addMultipleERC20(
          bob,
          borrowerOperations.address,
          [tokenA, tokenB, tokenC],
          [dec(500, 18), dec(500, 18), dec(500, 18)],
          { from: bob }
        );
        await borrowerOperations.openTrove(
          th._100pct,
        ...
        assert.isTrue(
          await th.assertCollateralsEqual(
            ...
            [dec(8, 18), dec(9, 18), dec(13, 18), dec(1, 18)]
          )
        );
        assert.isTrue(aliceDebt.eq(toBN(dec(2000, 18))));
        ...
        assert.isTrue(
          await th.assertCollateralsEqual(
            ...
            aliceColls2,
            [tokenA.address, tokenB.address, tokenC.address, tokenD.address],
            [dec(11, 18), dec(8, 18), dec(11, 18), dec(2, 18)]
          )
        );
      ...
      it("Adjusting or opening trove with duplicate collat does not work", async () => {
        ...
        await assertRevert(
          borrowerOperations.openTrove(
            ...
            alice,
            alice,
            [tokenA.address, tokenA.address, tokenC.address],
            [dec(5, 18), dec(10, 18), dec(15, 18)],
            { from: alice }
          ),
        ...
        await assertRevert(
          borrowerOperations.adjustTrove(
            [],
            [],
            [tokenA.address, tokenA.address, tokenB.address],
            [toBN(dec(1, 18)), toBN(dec(1, 18))],
            0,
            false,
            alice,
      ...
      it("Adjusting a trove with collateral after price drops calculates VC correctly", async () => {
        await th.addMultipleERC20(
          alice,
          borrowerOperations.address,
          [tokenSuperRisky, tokenC, tokenLowDecimal],
          [dec(500, 18), dec(500, 18), dec(500, 18)],
          { from: alice }
        );
        await th.addMultipleERC20(
        ...
        await borrowerOperations.openTrove(
          ...
          alice,
          alice,
          [tokenSuperRisky.address, tokenC.address, tokenLowDecimal.address],
          [dec(20, 18), dec(5, 18), dec(5, 6)],
          { from: alice }
        );
        await borrowerOperations.openTrove(
          th._100pct,
          ...
          alice,
          [tokenSuperRisky.address, tokenC.address, tokenLowDecimal.address],
          [dec(450, 18), dec(450, 18), dec(450, 6)],
          { from: whale }
        );
        ...
        const debtAddition = toBN(dec(1000, 18))
          .mul(toBN(1000))
          .div(toBN(1005)); // 1000 adjusted for fees
        await borrowerOperations.adjustTrove(
          [tokenLowDecimal.address, tokenC.address],
          [toBN(dec(5, 18)), toBN(dec(7, 18))],
    ...
    describe("check VC, TCR, balances multi collateral", async () => {
      ...
      it("Open two multi collateral trove, check raw balances of contracts are correct", async () => {
        ...
        const activePool_Coll_Before = [
          await activePool.getCollateral(tokenA.address),
          await activePool.getCollateral(tokenB.address),
          await activePool.getCollateral(tokenC.address),
        ];
        const activePool_RawColl_Before = [
          toBN(await tokenA.balanceOf(activePool.address)),
          toBN(await tokenB.balanceOf(activePool.address)),
          toBN(await tokenC.balanceOf(activePool.address)),
        ];
        ...
        const activePool_Coll_After = [
          await activePool.getCollateral(tokenA.address),
          await activePool.getCollateral(tokenB.address),
          await activePool.getCollateral(tokenC.address),
        ];
        const activePool_RawColl_After = [
          toBN(await tokenA.balanceOf(activePool.address)),
          toBN(await tokenB.balanceOf(activePool.address)),
          toBN(await tokenC.balanceOf(activePool.address)),
        ];

        const sumResult = await contracts.borrowerOperations.sumColls(
          aliceTokens,
          aliceColls,
          bobTokens,
          bobColls
      ...
      it("Open multi collateral trove, adjust prices individually, check if VC and TCR change accordingly. Ratio 1 tokens", async () => {
        ...
        await borrowerOperations.openTrove(
          ...
          alice,
          alice,
          [tokenA.address, tokenC.address, tokenB.address],
          [dec(10, 18), dec(5, 18), dec(5, 18)],
          { from: alice }
        );
      ...
      it("Ratio < 1, Open multi collateral trove, adjust prices individually, check if VC and TCR change accordingly", async () => {
        ...
        await borrowerOperations.openTrove(
          ...
          alice,
          alice,
          [tokenSuperRisky.address, tokenC.address, tokenB.address],
          [dec(20, 18), dec(5, 18), dec(5, 18)],
          { from: alice }
        );
      ...
      it("Ratio > 1, Open multi collateral trove, adjust prices, VC and TCR should change accordingly. ", async () => {
        await th.addMultipleERC20(
          ...
          borrowerOperations.address,
          [stableCoin, tokenC],
          [dec(500000, 18), dec(500, 18)],
          { from: alice }
        );

        priceFeedStableCoin.setPrice(toBN(dec(1, 18)));
        const amountStableCoin = toBN(dec(2000, 18))
          .mul(toBN(dec(1, 18)))
          .div(toBN(dec(105, 16))); // 2000 / 105% to get the amount to take out in stablecoin
        await borrowerOperations.openTrove(
          th._100pct,
          STARMinAmount,
          alice,
          alice,
          [tokenC.address, stableCoin.address],
          [dec(10, 18), amountStableCoin],
          { from: alice }
        );
        ...
        const VCStableAfter = toBN(dec(2000, 18))
          .mul(toBN(dec(95, 16)))
          .div(toBN(dec(1, 18)));

        const expectedTCR = VCStableAfter.add(toBN(dec(2000, 18)))
          .mul(toBN(dec(1, 18)))
        ...
        const VCStableAfter2 = toBN(dec(2000, 18))
          .mul(toBN(dec(105, 16)))
          .div(toBN(dec(1, 18)));
        const expectedTCR2 = VCStableAfter2.add(toBN(dec(2000, 18)))
          .mul(toBN(dec(1, 18)))
          .div(toBN(dec(2000, 18)));
        const TCRAfter2 = await th.getTCR(contracts);
    ...
    describe("Various Test multi collateral borrow ops", async () => {
      it("Try various operations with lots of accounts and tokens.", async () => {
        ...
        await openTrove({
          extraSTARAmount: toBN(dec(100000, 30)),
          ICR: toBN(dec(2, 18)),
          extraParams: { from: whale },
        });
        await th.addTokensToAccountsAndOpenTroveWithICR(
          contracts,
          toBN(dec(2, 18)),
          accounts,
          tokens
        ...
        for (let i = 0; i < accounts.length - 1; i++) {
          ...
          const transfer = await starToken.transfer(
            accounts[i],
            toBN(dec(1000, 30)),
            { from: whale }
          ); // for borrow fee.
            ...
            if (balance.gt(toBN(dec(1, 30)))) {
              if (tokens[j].address === tokenLowDecimal.address) {
                await th.assertIsApproximatelyEqual(
                  balance,
                  toBN(dec(1, 30)).add(toBN(dec(1, 10)))
                );
              } else {
                await th.assertIsApproximatelyEqual(
                  balance,
                  toBN(dec(1, 30)).add(toBN(dec(1, 22)))
                );
              }
            } else {
    ...
    describe("Various Test fees borrow ops", async () => {
      it("Basic get fee flat open trove before and after", async () => {
        ...
        await th.addMultipleERC20(
          alice,
          borrowerOperations.address,
          [contracts.weth, contracts.wavax],
          [dec(500, 18), dec(500, 18), dec(500, 18)],
          { from: alice }
        );
        await th.addMultipleERC20(
        ...
        await newPriceCurve.adjustParams(
          "WAVAX Price curve",
          "0",
          dec(5, 15),
          "0",
        ...
        await contracts.whitelist.changePriceCurve(
        ...
        const sPREONBalanceBefore = await contracts.starToken.balanceOf(
        ...
        await newPriceCurve2.adjustParams(
          "Token A Price curve",
          "0",
          dec(1, 16),
          "0",
        ...
        await borrowerOperations.openTrove(
          ...
          carol,
          carol,
          [contracts.wavax.address, tokenA.address],
          [dec(20, 18), dec(20, 18)],
          { from: carol }
        );
        ...
        // Check that sPREON got the correct amount of tokens. + 10 + the normal 0.005 fee.
        const sPREONBalanceAfter = await contracts.starToken.balanceOf(
          PREONContracts.sPREON.address
        );
        await th.assertIsApproximatelyEqual(
          sPREONBalanceAfter,
          sPREONBalanceBefore
            .add(toBN(dec(60, 18)))
            .add(toBN(dec(1800, 18)).sub(STARMinAmount))
        );
      });
      ...
      it("Basic get fee sloped open and adjust trove before and after", async () => {
        ...
        await th.addMultipleERC20(
          bob,
          borrowerOperations.address,
          [contracts.weth, contracts.wavax, tokenA],
          [dec(500, 18), dec(500, 18), dec(500, 18)],
          { from: bob }
        );
        ...
        // Check that sPREON got the correct amount of tokens. + 110
        const sPREONBalanceAfter = await contracts.starToken.balanceOf(
          PREONContracts.sPREON.address
        );
        ...
        await th.assertIsApproximatelyEqual(
          sPREONBalanceAfter,
          sPREONBalanceBefore.add(toBN(dec(220, 18)))
        );
      });
      ...
      it("Test fee cap passed in to open trove", async () => {
        ...
        const justBelowFeeCap = toBN(dec(30, 18))
          .add(toBN(dec(1800, 18)).sub(STARMinAmount))
          .mul(toBN(dec(1, 18)))
          .div(toBN(dec(4000, 18)))
          .sub(toBN(dec(1, 1)));
        ...
        await borrowerOperations.openTrove(
          justBelowFeeCap.add(toBN(dec(1, 1))),
          STARMinAmount,
          bob,
          bob,
          [contracts.wavax.address],
      ...
      it("Test fee cap passed in to adjustTrove", async () => {
        ...
        const justBelowFeeCap = toBN(dec(220, 18))
          .mul(toBN(dec(1, 18)))
          .div(toBN(dec(24000, 18)))
          .sub(toBN(dec(1, 1)));
      ...
      it("During bootstrapping period, max fee is 1%", async () => {
        ...
        await borrowerOperations.openTrove(
          ...
          alice,
          alice,
          [contracts.weth.address, contracts.wavax.address],
          [dec(20, 18), dec(20, 18)],
          { from: alice }
        );
        ...
        const bobDebt = await troveManager.getTroveDebt(bob);
        assert.isTrue(
          bobDebt.sub(toBN(dec(2000, 18))).eq(toBN(dec(40, 18))),
          "maxFee is 1% "
        );

        await starToken.transfer(bob, dec(1500, 18), { from: alice });
        ...
        assert.isTrue(
          bobDebt2.sub(toBN(dec(2000, 18))).gt(toBN(dec(40, 18))),
          "maxFee no longer applies at 1% "
        );
      });
      ...
      it("Test fee decay without much change in time.", async () => {
        ...
        const carolDebt = await troveManager.getTroveDebt(carol);
        assert.isTrue(
          carolDebt.sub(toBN(dec(2000, 18))).lt(toBN(dec(35, 18))),
          "slightly decayed"
        );
        assert.isTrue(
          carolDebt.sub(toBN(dec(2000, 18))).gt(toBN(dec(349, 17))),
          "slightly decayed"
        );
      });
      ...
      it("Test that fee decays after time.", async () => {
        ...
        assert.isTrue(
          carolDebt.sub(toBN(dec(2000, 18))).gt(toBN(dec(30, 18))),
          "but still not full fee (> 29.28"
        );
      });
      ...
      it("Variable fee adjust trove, with collateral out and collateral in", async () => {
        ...
        // tokenA is 40 / 160 = 25% of total. The fee should be (0% + 25%) / 2 => (0.005 + 0.0075) = 0.00625%
        await borrowerOperations.adjustTrove(
          [contracts.weth.address, tokenA.address],
          [dec(100, 18), dec(40, 18)],
          [contracts.wavax.address],
          [dec(20, 18)],
          0,
          false,
        ...
        // Check that sPREON got the correct amount of tokens. + 50
        const sPREONBalanceAfter = await contracts.starToken.balanceOf(
          PREONContracts.sPREON.address
        );
        ...
        await th.assertIsApproximatelyEqual(
          sPREONBalanceAfter,
          sPREONBalanceBefore.add(toBN(dec(50, 18)))
        );
      });
      ...
      it("Variable fee adjust trove, with extra debt and collateral in. Applies stacking fee correctly", async () => {
        ...
        // Check that sPREON got the correct amount of tokens. + 50 + .5 from flat fee.
        const sPREONBalanceAfter = await contracts.starToken.balanceOf(
          PREONContracts.sPREON.address
        );
        ...
        await th.assertIsApproximatelyEqual(
          sPREONBalanceAfter,
          sPREONBalanceBefore.add(toBN(dec(505, 17)))
        );
      });
    ...
    describe.only("Lever up", async () => {
      beforeEach(async () => {
        joeRouter = new ethers.Contract(
          ...
          (abi = routerABI),
          (signer = await hre.ethers.getSigner(H))
        );
        await starToken.unprotectedMint(H, dec(100000000, 18));
        await contracts.weth.mint(H, dec(100000000, 18));
        await starToken.approve(joeRouter.address, dec(100000000, 18), {
          from: H,
        });
        await contracts.weth.approve(joeRouter.address, dec(100000000, 18), {
          from: H,
        });
        ...
        await contracts.wavax.approve(joeRouter.address, dec(100000000, 18), {
          from: H,
        });
        await joeRouter.addLiquidity(
          starToken.address,
        ...
        await deploymentHelper.deployNewRouter(contracts, {
          name: "Joe router weth",
          joeRouter: joeRouter,
          whitelistCollateral: contracts.weth,
        });
        await deploymentHelper.deployNewRouter(contracts, {
          name: "Joe router wavax",
          joeRouter: joeRouter,
          whitelistCollateral: contracts.wavax,
        });
      });
      ...
      it("Basic open trove lever up with one collateral type", async () => {
        ...
        await borrowerOperations.openTroveLeverUp(
          th._100pct,
          0,
          alice,
          alice,
        ...
        const activePoolRawWethBalance = await contracts.weth.balanceOf(
          activePool.address
        );
        ...
        assert.isTrue(
          activePoolRawWethBalance.toString() == aliceWeth.toString()
        );
      });
      ...
      it("Open trove lever up with multiple collateral types, multiple lever at once", async () => {
        ...
        const activePoolRawWavaxBalance = await contracts.wavax.balanceOf(
          activePool.address
        );
        ...
        assert.isTrue(
          activePoolRawWavaxBalance.sub(toBN(dec(500, 18))).toString() ==
            aliceWavax.toString()
        );
      });
      ...
      it("Adjust trove lever up on same collateral", async () => {
        ...
        await borrowerOperations.addCollLeverUp(
          [contracts.weth.address],
          [dec(20, 18)],
          [dec(3, 18)],
          [dec(1, 17)],
      ...
      it("Withdraw coll unlever up one asset levered", async () => {
        ...
        const aliceWeth = (await troveManager.getTroveColls(alice))[1][0];
        const aliceWavax = (await troveManager.getTroveColls(alice))[1][1];
        const aliceRawBalance = await starToken.balanceOf(alice);
        assert.isTrue(aliceDebt.eq(toBN(dec(8240, 18))));
        contracts.weth.approve(
          await contracts.whitelist.getDefaultRouterAddress(
            contracts.weth.address
          ),
          dec(20, 18),
          { from: alice }
        );
        await borrowerOperations.withdrawCollUnleverUp(
          [contracts.weth.address],
          [dec(20, 18)],
          [dec(1, 17)],
          dec(3000, 18),
        ...
        const aliceRawBalanceAfter = await starToken.balanceOf(alice);
        ...
        assert.isTrue(
          aliceRawBalanceAfter.sub(aliceRawBalance).gt(toBN(dec(900, 18)))
        );
      });
      ...
      it("Withdraw coll unlever up, covers extra balance if not enough sold", async () => {
        ...
        // should revert if she does not have enough STAR
        await th.assertRevert(
          borrowerOperations.withdrawCollUnleverUp(
            [contracts.weth.address],
            [dec(5, 18)],
            [dec(1, 17)],
            dec(3000, 18),
        ...
        // give alice some STAR first from bob
        await th.addMultipleERC20(
          bob,
          borrowerOperations.address,
          [contracts.weth, contracts.wavax],
          [dec(500, 25), dec(500, 25), dec(500, 25)],
          { from: bob }
        );
        await borrowerOperations.openTrove(
          th._100pct,
        ...
        await contracts.starToken.transfer(alice, dec(10000, 18), {
          from: bob,
        });
        const aliceRawBalance = await starToken.balanceOf(alice);
        const bobDebt = await troveManager.getTroveDebt(bob);
        ...
        var activePoolRawWethBalance = await contracts.weth.balanceOf(
          contracts.activePool.address
        );
        var activePoolRawWavaxBalance = await contracts.wavax.balanceOf(
          contracts.activePool.address
        );
        var activePoolVirtualWethBalance = (
          await contracts.activePool.getCollateralVC(contracts.weth.address)
        ).div(toBN(200));
        var activePoolVirtualSTARDebt = await contracts.activePool.getSTARDebt();
        var activePoolVirtualWavaxBalance = (
          await contracts.activePool.getCollateralVC(contracts.wavax.address)
        ).div(toBN(200));
        assert.isTrue(
          activePoolRawWethBalance.eq(activePoolVirtualWethBalance)
        );
        assert.isTrue(
          activePoolRawWavaxBalance.eq(activePoolVirtualWavaxBalance)
        );
        assert.isTrue(activePoolVirtualSTARDebt.eq(bobDebt.add(aliceDebt)));
        ...
        activePoolRawWethBalance = await contracts.weth.balanceOf(
          contracts.activePool.address
        );
        activePoolRawWavaxBalance = await contracts.wavax.balanceOf(
          contracts.activePool.address
        );
        activePoolVirtualWethBalance = (
          await contracts.activePool.getCollateralVC(contracts.weth.address)
        ).div(toBN(200));
        activePoolVirtualSTARDebt = await contracts.activePool.getSTARDebt();
        activePoolVirtualWavaxBalance = (
          await contracts.activePool.getCollateralVC(contracts.wavax.address)
        ).div(toBN(200));
        assert.isTrue(
          activePoolRawWethBalance.eq(activePoolVirtualWethBalance)
        ...
        assert.isTrue(
          activePoolVirtualSTARDebt.eq(bobDebt.add(aliceDebtAfter))
        );
      });
      ...
      it("Full close trove unlever up one asset", async () => {
        ...
        await th.assertRevert(
          borrowerOperations.closeTroveUnlever(
            [contracts.weth.address],
            [dec(20, 18)],
            [dec(1, 17)],
            { from: alice }
          ),
          "should revert if not enough STAR to cover remaining balance. "
        );

        await contracts.starToken.transfer(alice, dec(10000, 18), {
          from: bob,
        });
        const aliceSTARBalanceBefore = await starToken.balanceOf(alice);

        await borrowerOperations.closeTroveUnlever(
          [contracts.weth.address],
          [dec(20, 18)],
          [dec(1, 17)],
          { from: alice }
        ...
        const activePoolRawWethBalanceAfter = await contracts.weth.balanceOf(
          activePool.address
        );
        const activePoolRawWavaxBalanceAfter = await contracts.wavax.balanceOf(
          activePool.address
        );
        const activePoolVirtualWethBalance = (
          await contracts.activePool.getCollateralVC(contracts.weth.address)
        ).div(toBN(200));
        const activePoolVirtualWavaxBalance = (
          await contracts.activePool.getCollateralVC(contracts.wavax.address)
        ).div(toBN(200));
        const activePoolVirtualSTARDebt = await contracts.activePool.getSTARDebt();
        assert.isTrue(
          activePoolRawWethBalanceAfter.eq(activePoolVirtualWethBalance)
        );
        assert.isTrue(
          activePoolRawWavaxBalanceAfter.eq(activePoolVirtualWavaxBalance)
        );
        assert.isTrue(activePoolVirtualSTARDebt.eq(bobDebt));
        ...
        // Her STAR balance should have decreased by the corresponding amount as well.
        const aliceRawWethAfter = await contracts.weth.balanceOf(alice);
        const aliceRawWavax = await contracts.wavax.balanceOf(alice);
        const aliceSTARAfter = await contracts.starToken.balanceOf(alice);

        assert.isTrue(
          aliceRawWethAfter
            .sub(aliceRawWethBefore)
            .eq(aliceWeth.sub(toBN(dec(20, 18))))
        );
        assert.isTrue(aliceRawWavax.eq(toBN(dec(500, 18))));

        // star balance should be less than before. debt = toBN(dec(10250, 18))
        // by selling 20 weth = 20 * ~200 = 4000, she should have paid back about 6000 STAR from her own balance
        await th.assertIsApproximatelyEqual(
          aliceSTARBalanceBefore.sub(aliceSTARAfter).div(toBN(dec(1, 18))),
          toBN(dec(6000, 0)),
          50
        );
      });
      ...
      it("open trove reverts when max slippage is overtaken", async () => {
        ...
        await th.assertRevert(
          borrowerOperations.openTroveLeverUp(
            th._100pct,
            dec(2000, 18),
            alice,
            alice,
      ...
      it("add coll reverts when max slippage is overtaken", async () => {
        ...
        await joeRouter.swapExactTokensForTokens(
          dec(40, 18),
          1,
          [contracts.weth.address, contracts.starToken.address],
          H,
          dec(1, 50),
          { from: H }
        );
        ...
        await th.assertRevert(
          borrowerOperations.addCollLeverUp(
            [contracts.weth.address],
            [dec(20, 18)],
            [dec(3, 18)],
            [dec(1, 1)],
