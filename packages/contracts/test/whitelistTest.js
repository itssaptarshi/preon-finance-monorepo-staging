const {
  ETHERSCAN_BASE_URL,
} = require("../mainnetDeployment/deploymentParams.avalanche.js");
  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    it("deprecateCollateral(), loop through valid collateral and delete", async () => {
      ...
      for (let i = 0; i < validCollateral.length; i++) {
        assert.isTrue(
          await whitelist.getIsActive(validCollateral[i]),
          "Collateral shouldn't be deprecated"
        );
        ...
        assert.isFalse(
          await whitelist.getIsActive(validCollateral[i]),
          "Collateral should be deprecated"
        );
      }
    ...
    it("changeRatio(), loop through valid collateral and change ratio", async () => {
      ...
      for (let i = 0; i < validCollateral.length; i++) {
        ...
        await whitelist.changeRatio(
          validCollateral[i],
          dec(parseInt(Math.random() * 110), 16)
        );
      ...
      for (let i = 0; i < validCollateral.length; i++) {
        ...
        assertRevert(
          whitelist.changeRatio(
            validCollateral[i],
            dec(110 + parseInt(Math.random() * 100), 16)
          )
        );
      }
    });
    ...
    it("changePriceCurve(), loop through valid collateral and change PriceCurve", async () => {
      ...
      for (let i = 0; i < validCollateral.length; i++) {
        let curve = await whitelist.getPriceCurve(validCollateral[i]);
        await whitelist.changePriceCurve(
          validCollateral[i],
          validCollateral[i]
        );
        assert.notEqual(
          curve,
          await whitelist.getPriceCurve(validCollateral[i])
        );
      }
    });
    ...
    it("getPrice(), loop through valid collateral and getPrice. Randomly set price", async () => {
      ...
      for (let k = 0; k < 10000; k += Math.random() * 100) {
        for (let i = 0; i < validCollateral.length; i++) {
          await tokensToOracles
            .get(validCollateral[i])
            .setPrice(dec(parseInt(k), parseInt(Math.random() * 19))); //Randomly set price and decimals
          ...
          assert(
            price.eq(await tokensToOracles.get(validCollateral[i]).getPrice())
          );
        }
      }
    });
        ...
        for (let i = 0; i < validCollateral.length; i++) {
          ...
          await tokensToOracles
            ...
            .setPrice(
              dec(
                parseInt(k),
                decimals.add(web3.utils.toBN(parseInt(Math.random() * 4) - 2))
              )
            ); //Randomly set price and decimals
          let price = await whitelist.getPrice(validCollateral[i]);
    ...
    it("getValueUSD(), loop through valid collateral and getValueUSD", async () => {
      ...
      for (let i = 0; i < validCollateral.length; i++) {
        ...
        assert.equal(
          value.toString(),
          ((price * amount) / dec(1, 18)).toString()
        );
        // console.log(ethers.BigNumber.isBN(price));
        // console.log(price.toNumber(), await tokensToOracles.get(validCollateral[i]).getPrice().toNumber()/ 1E18);
    ...
    it("getValueVC(), loop through valid collateral and getValueUSD. Randomly set price", async () => {
      ...
      for (let k = 1; k < 10000; k += Math.random() * 100) {
        for (let i = 0; i < validCollateral.length; i++) {
          let decimals = await whitelist.getDecimals(validCollateral[i]);
          let ratio = await whitelist.getRatio(validCollateral[i]);
          let decimalOffset = web3.utils
            .toBN(10)
            .pow(decimals.add(web3.utils.toBN(18)));
    ...
    it("Check after deprecation of collateral that people cannot add new of that collateral, but it still works inside the system.", async () => {
      ...
      const { amounts: A_amounts } = await th.openTroveWithColls(contracts, {
        ICR: toBN(dec(400, 16)),
        colls: a_colls,
        amounts: a_amounts,
      ...
      const { amounts: B_amounts } = await th.openTroveWithColls(contracts, {
        ICR: toBN(dec(400, 16)),
        colls: b_colls,
        amounts: b_amounts,
      ...
      const { amounts: C_amounts } = await th.openTroveWithColls(contracts, {
        ICR: toBN(dec(210, 16)),
        colls: c_colls,
        amounts: c_amounts,
      ...
      assertRevert(
        th.openTroveWithColls(contracts, {
          ICR: toBN(dec(400, 16)),
          colls: d_colls,
          amounts: d_amounts,
          oracles: [contracts.priceFeedETH],
      ...
      await weth.approve(borrowerOperations.address, addedColl1, {
        from: alice,
      });
      assertRevert(
      ...
      await wavax.approve(borrowerOperations.address, addedColl1, {
        from: carol,
      });
      ...
      const A_ETHAfterL1 = A_amounts[0].add(
        th.applyLiquidationFee(C_amounts[0])
      );
      assert.isAtMost(
      ...
      assert.equal(
        entireSystemETH,
        A_amounts[0].add(th.applyLiquidationFee(C_amounts[0]))
      );
      ...
      console.log((await starToken.balanceOf(alice)).toString());
      await starToken.unprotectedMint(alice, toBN(dec(1300, 18)));
      await starToken.approve(borrowerOperations.address, toBN(dec(1300, 18)), {
        from: alice,
      });
      console.log((await starToken.balanceOf(alice)).toString());
      ...
      assert.equal(
        entireSystemAVAX,
        A_amounts[0].add(th.applyLiquidationFee(C_amounts[1]))
      );
    ...
    it("VC and PriceFeed still works after deprecate", async () => {
      ...
      const vc = (
        await whitelist.getValueVC(weth.address, dec(1, 18))
      ).toString();
