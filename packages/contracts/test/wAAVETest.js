...
const masterChefABI = [
  {
    inputs: [
      { internalType: "contract JoeToken", name: "_joe", type: "address" },
      { internalType: "address", name: "_devAddr", type: "address" },
      { internalType: "address", name: "_treasuryAddr", type: "address" },
      { internalType: "address", name: "_investorAddr", type: "address" },
      { internalType: "uint256", name: "_joePerSec", type: "uint256" },
  ...
  },
  {
    inputs: [],
    name: "joe",
    outputs: [{ internalType: "contract JoeToken", name: "", type: "address" }],
    stateMutability: "view",
    type: "function",
  },
  {
  ...
  {
    inputs: [
      { internalType: "uint256", name: "_newDevPercent", type: "uint256" },
    ],
    name: "setDevPercent",
    outputs: [],
    stateMutability: "nonpayable",
  ...
  {
    inputs: [{ internalType: "uint256", name: "_joePerSec", type: "uint256" }],
    name: "updateEmissionRate",
    outputs: [],
    stateMutability: "nonpayable",
    type: "function",
...
contract("wAAVE Test", async (accounts) => {
  const [
    owner,
    alice,
    bob,
  ...
  beforeEach(async () => {
    ...
    joeMasterChef = new ethers.Contract(
  ...
  async function makeTOWRAP(accountSigner) {
    ...
    await WAVAX.approve(lendingPool.address, MAXINT);
    ...
    // await TOWRAP.mint({value: dec(1000, 18)})
    // console.log("Approving");
    await TOWRAP.approve(WASSET.address, MAXINT);
    console.log(
      "TOWRAP Balance",
      (await TOWRAP.balanceOf(accountSigner)).toString()
    );
  ...
  it("Testing wrapper wraps and unwraps properly multiple users", async () => {
    ...
    assert(aftertoWrapBalanceA.gt(toWrapBalanceA));
    ...
    assert(aftertoWrapBalanceB.gt(toWrapBalanceB));
    ...
    assert(aftertoWrapBalanceH.gt(toWrapBalanceH));
  ...
  //     if (rewardTokens[i]==ZERO_ADDRESS) {
  //       preRewardBalances[i]=await ethers.provider.getBalance(harry);
  //     } else {
  //       tokenContract=new ethers.Contract(rewardTokens[i], abi = ERC20ABI, signer = await hre.ethers.getSigner(harry))
  //       preRewardBalances[i]=await tokenContract.balanceOf(harry)
  //     }
  ...
  //   for (let i = 0; i < rewardTokens.length; i++) {
  //     if (rewardTokens[i]==ZERO_ADDRESS) {
  //       postRewardBalances[i]=await ethers.provider.getBalance(harry);
  //     } else {
  //       tokenContract=new ethers.Contract(rewardTokens[i], abi = ERC20ABI, signer = await hre.ethers.getSigner(harry))
  //       postRewardBalances[i]=await tokenContract.balanceOf(harry)
  //     }
  ...
  //   }
  //   // QI=new ethers.Contract("0x8729438EB15e2C8B576fCc6AeCdA6A148776C0F5", abi = ERC20ABI, signer = await hre.ethers.getSigner(harry))
  //   // console.log(await QI.balanceOf(harry));
  // })
});
