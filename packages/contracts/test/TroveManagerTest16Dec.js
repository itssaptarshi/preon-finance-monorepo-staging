...
contract("TroveManager", async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    const LQTYContracts = await deploymentHelper.deployPREONContracts(
      bountyAddress,
      lpRewardsAddress,
      multisig
    );
    ...
    await deploymentHelper.connectPREONContracts(LQTYContracts);
    ...
    await contracts.whitelist.addCollateral(
      contracts.dec16.address,
      "1000000000000000000",
      contracts.priceFeedDEC16.address,
      20,
      contracts.PriceCurveDEC16.address,
      false,
  ...
  it("liquidate(): updates the L_ETH and L_STARDebt reward-per-unit-staked totals", async () => {
    ...
    console.log(
      "bobICR: ",
      (await contracts.troveManager.getCurrentICR(bob)).toString()
    );
    console.log(
      "bobVC: ",
      (await contracts.troveManager.getVC(bobCTS[0], bobCTS[1])).toString()
    );
    ...
    console.log(
      "aliceICR: ",
      (await contracts.troveManager.getCurrentICR(alice)).toString()
    );
    console.log(
      "aliceVC: ",
      (await contracts.troveManager.getVC(aliceCTS[0], aliceCTS[1])).toString()
    );
    ...
    console.log(
      "carolICR: ",
      (await contracts.troveManager.getCurrentICR(carol)).toString()
    );
    console.log(
      "carolVC: ",
      (await contracts.troveManager.getVC(carolCTS[0], carolCTS[1])).toString()
    );
    ...
    const L_STARDebt_expected_1 = C_totalDebt.mul(mv._1e18BN)
      .div(A_collateral.add(B_collateral))
      .mul(toBN(dec(1, 2)));

    console.log("a", L_STARDebt_AfterCarolLiquidated.toString());
    console.log("b", L_STARDebt_expected_1.toString());
    assert.isAtMost(
   ...
   L_STARDebt = (180 / 20) + (890 / 10) = 98 STAR */
    ...
    const L_ETH_expected_2 = L_ETH_expected_1.add(
      th
        .applyLiquidationFee(
          B_collateral.add(
            B_collateral.mul(L_ETH_expected_1).div(toBN(dec(1, 20)))
          )
        )
        .mul(toBN(dec(1, 20)))
        .div(A_collateral)
  ...
  it("batchLiquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards", async () => {
    ...
    const pendingETH_C = (await troveManager.getPendingCollReward(C))[1][
      wethIDX
    ];
  ...
  it("getRedemptionHints(): returns 0 as partialRedemptionHintNICR when reaching _maxIterations", async () => {
    ...
    // Get hints for a redemption of 170 + 30 + some extra STAR. At least 3 iterations are needed
  ...
  it("redeemCollateral(): cancels the provided STAR with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether", async () => {
    // --- SETUP ---
    ...
    const STARAmount = th
      .getEmittedRedemptionValues(redemptionTx)[0]
      .toString();
    ...
    const expectedTotalETHDrawn = redemptionAmount; // convert redemptionAmount STAR to ETH, at ETH:USD price 200
    ...
    console.log("star after", dennis_STARBalance_After.toString());
    console.log("star before", dennis_STARBalance_Before.toString());
    assert.equal(
      dennis_STARBalance_After,
      dennis_STARBalance_Before.sub(redemptionAmount).toString()
    );
  });
  ...
  it("redeemCollateral(): with invalid first hint, zero address", async () => {
    ...
    assert.equal(
      dennis_STARBalance_After,
      dennis_STARBalance_Before.sub(redemptionAmount)
    );
  });
  ...
  it("redeemCollateral(): ends the redemption sequence when the token redemption request has been filled", async () => {
    ...
    // open trove from redeemer.  Redeemer has highest ICR (100ETH, 100 STAR), 20000%
    const { starAmount: F_starAmount } = await openTrove({
      ICR: toBN(dec(200, 18)),
      extraSTARAmount: redemptionAmount.mul(toBN(2)),
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-60) = 40 STAR
    const flynBalance = await starToken.balanceOf(flyn);

    const STARFee = (await troveManager.getRedemptionRate())
      .mul(redemptionAmount)
      .div(toBN(dec(1, 18)));

    // console.log("redemption amount " + redemptionAmount.toString())
    // console.log("FLYN BALANCE : " + flynBalance.toString())
    // console.log("FLYN STAR AMOUNT BEFORE " + F_starAmount.toString())
    // console.log("F STAR AMOUNT MINUS REDEMPTION AMOUNT: " + (F_starAmount.sub(redemptionAmount).toString()))
    // console.log("STAR FEE CALCULATION: " + (baseRate.mul(redemptionAmount).div(toBN(dec(1,18))).toString()))
    th.assertIsApproximatelyEqual(
      flynBalance,
      F_starAmount.sub(redemptionAmount).sub(STARFee)
    );
  ...
  it("redeemCollateral(): ends the redemption sequence when max iterations have been reached", async () => {
    ...
    const STARFeeAttempted = (await troveManager.getRedemptionRate())
      .mul(attemptedRedemptionAmount)
      .div(toBN(dec(1, 18)));
    const STARFee = (await troveManager.getRedemptionRate())
      .mul(redemptionAmount)
      .div(toBN(dec(1, 18)));
    console.log("STAR FEE ", STARFee.toString());
    console.log("STAR FEE ATTEMPTED ", STARFeeAttempted.toString());
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-40) = 60 STAR
    const flynBalance = (await starToken.balanceOf(flyn)).toString();

    console.log("redemption amount " + redemptionAmount.toString());
    console.log(
      "Attempted redemption amount " + attemptedRedemptionAmount.toString()
    );
    console.log("FLYN BALANCE : " + flynBalance.toString());
    console.log("FLYN STAR AMOUNT BEFORE " + F_starAmount.toString());
    console.log(
      "F STAR AMOUNT MINUS REDEMPTION AMOUNT: " +
        F_starAmount.sub(redemptionAmount).toString()
    );

    const STARFee2 = (await troveManager.getRedemptionRate())
      .mul(redemptionAmount)
      .div(toBN(dec(1, 18)));
    console.log("STAR FEE ", STARFee2.toString());

    console.log(
      "NEW BASE RATE ",
      (await troveManager.getRedemptionRate()).toString()
    );
  ...
  it("redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt", async () => {
    ...
    // await borrowerOperations.openTrove(th._100pct, await getOpenTroveSTARAmount(dec(10000, 18)), A, A, { from: A, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(
      th._100pct,
    ...
    // await borrowerOperations.openTrove(th._100pct, await getOpenTroveSTARAmount(dec(20000, 18)), B, B, { from: B, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(
      th._100pct,
      await getOpenTroveSTARAmount(dec(30000, 18)),
      C,
      C,
      [contracts.weth.address],
      [dec(1000, 18)],
    ...
    // await borrowerOperations.openTrove(th._100pct, await getOpenTroveSTARAmount(dec(30000, 18)), C, C, { from: C, value: dec(1000, 'ether') })

    // A and C send all their tokens to B
    await starToken.transfer(B, await starToken.balanceOf(A), { from: A });
    await starToken.transfer(B, await starToken.balanceOf(C), { from: C });
    ...
    // STAR redemption is 55000 US
    const STARRedemption = dec(55000, 18);
  ...
  it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
    ...
    // await borrowerOperations.openTrove(th._100pct, await getOpenTroveSTARAmount(dec(6000, 18)), A, A, { from: A, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(
      th._100pct,
      await getOpenTroveSTARAmount(dec(20000, 18)),
      B,
  ...
  it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    const {
      starAmount: B_starAmount,
      totalDebt: B_totalDebt,
    } = await openTrove({
      ICR: toBN(dec(133, 16)),
      extraSTARAmount: A_debt,
      extraParams: { from: bob },
    });

    await starToken.transfer(carol, B_starAmount, { from: bob });
  ...
  it("redeemCollateral(): reverts if fee exceeds max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraSTARAmount: dec(80, 18),
      extraParams: { from: A },
    });
    const { totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraSTARAmount: dec(90, 18),
      extraParams: { from: B },
    });
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
  ...
  it("redeemCollateral(): caller can redeem their entire STARToken balance", async () => {
    ...
    // Check activePool debt reduced by  400 STAR
  ...
  it("redeemCollateral(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation [ @skip-on-coverage ]", async () => {
    ...
    // Check A's balance has decreased by 10 STAR
    assert.equal(
      A_balanceBefore.sub(await starToken.balanceOf(A)),
      dec(10, 18)
    );
  ...
  it("redeemCollateral(): reverts if fee eats up all returned collateral", async () => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const {
        firstRedemptionHint,
        partialRedemptionHintNICR,
      } = await hintHelpers.getRedemptionHints(starAmount, 0);
      ...
      await borrowerOperations.adjustTrove(
        ...
        alice,
        alice,
        { from: alice, value: starAmount.mul(mv._1e18BN).div(price) }
      );
    }
  ...
  it("getPendingSTARDebtReward(): Returns 0 if there is no pending STARDebt reward", async () => {
    ...
    const carolSnapshot_L_STARDebt = (
