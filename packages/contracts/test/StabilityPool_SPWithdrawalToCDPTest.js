  ...
  async (accounts) => {
    ...
    describe("Stability Pool Withdrawal", async () => {
      ...
      it("withdrawETHGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
        ...
        assert.isAtMost(
          th.getDifference(
            (await stabilityPool.getCompoundedSTARDeposit(alice)).toString(),
        ...
        assert.isAtMost(
          th.getDifference(
            (await stabilityPool.getCompoundedSTARDeposit(carol)).toString(),
            "6666666666666666666666"
          ),
          10000
        );
      ...
      it("withdrawETHGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing STAR", async () => {
        // Whale opens Trove with 100k ETH
        await borrowerOperations.openTrove(
          th._100pct,
          await getOpenTroveSTARAmount(dec(100000, 18)),
      ...
      it("withdrawETHGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing STAR", async () => {
        // Whale opens Trove with 100k ETH
        await borrowerOperations.openTrove(
          th._100pct,
          await getOpenTroveSTARAmount(dec(100000, 18)),
      ...
      */
      ...
      it("withdrawETHGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 STAR.  A, B, C, D withdraw correct STAR deposit and ETH Gain", async () => {
        // Whale opens Trove with 100k ETH
        await borrowerOperations.openTrove(
          th._100pct,
          await getOpenTroveSTARAmount(dec(100000, 18)),
      ...
      it("withdrawETHGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 STAR.  A, B, C, D withdraw correct STAR deposit and ETH Gain", async () => {
        // Whale opens Trove with 100k ETH
        await borrowerOperations.openTrove(
          th._100pct,
          await getOpenTroveSTARAmount(dec(100000, 18)),
      ...
      it("withdrawETHGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct STAR deposit and ETH Gain", async () => {
        // Whale opens Trove with 1m ETH
        await borrowerOperations.openTrove(
          th._100pct,
          await getOpenTroveSTARAmount(dec(1000000, 18)),
      ...
      */
      ...
      it("withdrawETHGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 STAR.  A, B, C, D withdraw correct STAR deposit and ETH Gain", async () => {
        // Whale opens Trove with 100k ETH
        await borrowerOperations.openTrove(
          th._100pct,
          await getOpenTroveSTARAmount(dec(100000, 18)),
      ...
      it("withdrawETHGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct STAR deposit and ETH Gain", async () => {
        // Whale opens Trove with 100k ETH
        await borrowerOperations.openTrove(
          th._100pct,
          await getOpenTroveSTARAmount(dec(100000, 18)),
      ...
      */
      ...
      it("withdrawETHGainToTrove(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 STAR. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct STAR deposit and ETH Gain", async () => {
        // Whale opens Trove with 100k ETH
        await borrowerOperations.openTrove(
          th._100pct,
          await getOpenTroveSTARAmount(dec(100000, 18)),
      ...
      it("withdrawETHGainToTrove(): Depositors withdraw correct compounded deposit after liquidation empties the pool", async () => {
        ...
        assert.isAtMost(
          th.getDifference(
            (await stabilityPool.getCompoundedSTARDeposit(erin)).toString(),
      ...
      it("withdrawETHGainToTrove(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
        ...
        assert.isAtMost(
          th.getDifference(
            (await stabilityPool.getCompoundedSTARDeposit(flyn)).toString(),
            "0"
          ),
          100000
        );
        assert.isAtMost(
          th.getDifference(
            (await stabilityPool.getCompoundedSTARDeposit(graham)).toString(),
            "0"
          ),
          100000
        );
        assert.isAtMost(
          th.getDifference(
            (await stabilityPool.getCompoundedSTARDeposit(harriet)).toString(),
            "0"
          ),
          100000
        );
      ...
      it("withdrawETHGainToTrove(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
        ...
        // Expect Bob to retain 1% of initial deposit (100 STAR) and all the liquidated ETH (60 ether)
        assert.isAtMost(
          th.getDifference(
            (await stabilityPool.getCompoundedSTARDeposit(bob)).toString(),
    ...
    */
      it("withdrawETHGainToTrove(): Several deposits of 10000 STAR span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
        // Whale opens Trove with 100k ETH
        await borrowerOperations.openTrove(
          th._100pct,
          await getOpenTroveSTARAmount(dec(100000, 18)),
        ...
        assert.equal(await stabilityPool.getCompoundedSTARDeposit(alice), "0");
        assert.equal(await stabilityPool.getCompoundedSTARDeposit(alice), "0");
        assert.equal(await stabilityPool.getCompoundedSTARDeposit(alice), "0");
        // D should retain around 0.9999 STAR, since his deposit of 9999.9 was reduced by a factor of 1e-5
        assert.isAtMost(
          th.getDifference(
            (await stabilityPool.getCompoundedSTARDeposit(dennis)).toString(),
            dec(99999, 12)
      ...
      it("withdrawETHGainToTrove(): Large liquidated coll/debt, deposits and ETH price", async () => {
        ...
        // Check STAR balances
        const aliceSTARBalance = await stabilityPool.getCompoundedSTARDeposit(
          alice
        );
        ...
        const bobSTARBalance = await stabilityPool.getCompoundedSTARDeposit(
          bob
        );
