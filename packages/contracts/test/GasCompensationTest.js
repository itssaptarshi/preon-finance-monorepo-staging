...
contract("Gas compensation tests", async (accounts) => {
  ...
  xit("_getCollGasCompensation(): returns the 0.5% of collaterall if it is < $10 in value", async () => {
    ...
    const gasCompensation_1 = await troveManagerTester.getCollGasCompensation(
      weth.address,
      dec(1, "ether")
    );
    ...
    const gasCompensation_2 = await troveManagerTester.getCollGasCompensation(
      weth.address,
      dec(100, "finney")
    );
  ...
  it("_getCollGasCompensation(): returns 0.5% of collaterall when 0.5% of collateral < $10 in value", async () => {
    ...
    -> Expect 0.5% of collaterall as gas compensation */
    const gasCompensation_3 = await troveManagerTester.getCollGasCompensation(
      weth.address,
      "6092324088087235800"
    );
  ...
  it("getCollGasCompensation(): returns 0.5% of collaterall when 0.5% of collateral = $10 in value", async () => {
    ...
    -> Expect 0.5% of collaterall as gas compensation */
    const gasCompensation = await troveManagerTester.getCollGasCompensation(
      weth.address,
      dec(10, "ether")
    );
  ...
  it("getCollGasCompensation(): returns 0.5% of collaterall when 0.5% of collateral = $10 in value", async () => {
    ...
    const price_2 = await priceFeed.getPrice();
    const gasCompensation_5 = await troveManagerTester.getCollGasCompensation(
      weth.address,
  ...
  it("_getCompositeDebt(): returns (debt + 50) when collateral < $10 in value", async () => {
    ...
    debt = 10 STAR
    0.5% of coll = 0.04995 ETH. USD value: $9.99
    -> Expect composite debt = 10 + 200  = 2100 STAR*/
    ...
    /* ETH:USD price = 200
     ...
     debt = 0 STAR
     0.5% of coll = 0.000275 ETH. USD value: $0.055
     -> Expect composite debt = 0 + 200 = 200 STAR*/
    ...
    // debt = 200 STAR
    // 0.5% of coll = 0.004995 ETH. USD value: $6.09
    // -> Expect  composite debt =  200 + 200 = 400  STAR */
  ...
  it("getCompositeDebt(): returns (debt + 50) collateral = $10 in value", async () => {
    ...
    debt = 123.45 STAR
    0.5% of coll = 0.5 ETH. USD value: $10
    -> Expect composite debt = (123.45 + 200) = 323.45 STAR  */
  ...
  it("getCompositeDebt(): returns (debt + 50) when 0.5% of collateral > $10 in value", async () => {
    ...
    coll = 100 ETH  
    debt = 2000 STAR
    -> Expect composite debt = (2000 + 200) = 2200 STAR  */
    ...
    coll = 37.5 ETH  
    debt = 500 STAR
    -> Expect composite debt = (500 + 200) = 700 STAR  */
    ...
    coll = 94758.230582309850 ETH  
    debt = 1 billion STAR
    -> Expect composite debt = (1000000000 + 200) = 1000000200 STAR  */
    await priceFeed.setPrice("45323545420000000000000");
    const price_2 = await priceFeed.getPrice();
    ...
    ETH:USD price = 1000000 $/E (1 million)
    coll = 300000000 ETH   (300 million)
    debt = 54321.123456789 STAR
   -> Expect composite debt = (54321.123456789 + 200) = 54521.123456789 STAR */
    await priceFeed.setPrice(dec(1, 24));
    const price_3 = await priceFeed.getPrice();
  ...
  it("getCurrentICR(): Incorporates virtual debt, and returns the correct ICR for new troves", async () => {
    ...
    // E opens with 4405.45 ETH, 32598.35 STAR
    await openTrove({
  ...
  xit("Gas compensation from pool-offset liquidations. All collateral paid as compensation", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(2, 18)),
      extraSTARAmount: B_totalDebt.add(C_totalDebt),
      extraParams: { from: erin },
    });

    // D, E each provide STAR to SP
    ...
    const STARinSP_0 = await stabilityPool.getTotalSTARDeposits();
    ...
    const liquidatorBalance_before_A = web3.utils.toBN(
      await weth.balanceOf(liquidator)
    );
    ...
    // Check SP STAR has decreased due to the liquidation
    const STARinSP_A = await stabilityPool.getTotalSTARDeposits();
    assert.isTrue(STARinSP_A.lte(STARinSP_0));
    ...
    // Check SP STAR has decreased due to the liquidation of B
    const STARinSP_B = await stabilityPool.getTotalSTARDeposits();
    assert.isTrue(STARinSP_B.lt(STARinSP_A));
    ...
    // Check SP STAR has decreased due to the liquidation of C
    const STARinSP_C = await stabilityPool.getTotalSTARDeposits();
    assert.isTrue(STARinSP_C.lt(STARinSP_B));
  ...
  it("gas compensation from pool-offset liquidations: 0.5% collateral < $10 in value. Compensates $10 worth of collateral, liquidates the remainder", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(60, 18)),
      extraSTARAmount: dec(600, 18),
      extraParams: { from: carol },
    });
    await openTrove({
      ICR: toBN(dec(80, 18)),
      extraSTARAmount: dec(1, 23),
      extraParams: { from: dennis },
    });
    await openTrove({
      ICR: toBN(dec(80, 18)),
    ...
    // D, E each provide 10000 STAR to SP
    ...
    // Check SP STAR has decreased due to the liquidation of A
    const STARinSP_A = await stabilityPool.getTotalSTARDeposits();
    assert.isTrue(STARinSP_A.lt(STARinSP_0));
  ...
  it("Gas compensation from pool-offset liquidations. Liquidation event emits the correct gas compensation and total liquidated coll and debt", async () => {
    ...
    const [
      ...
      loggedColl_A,
      loggedGasComp_A,
    ] = th.getEmittedLiquidationValues(liquidationTxA, wethIDX);
    ...
    const [
      ...
      loggedColl_B,
      loggedGasComp_B,
    ] = th.getEmittedLiquidationValues(liquidationTxB, wethIDX);
  ...
  it("gas compensation from pool-offset liquidations. Liquidation event emits the correct gas compensation and total liquidated coll and debt", async () => {
    ...
    // D, E each provide 10,000 STAR to SP
    await stabilityPool.provideToSP(dec(1, 23), ZERO_ADDRESS, { from: dennis });
    await stabilityPool.provideToSP(dec(1, 23), ZERO_ADDRESS, { from: erin });
    ...
    const aliceDebt = await troveManager.getTroveDebt(alice);
    const aliceCollValueInUSD = await whitelist.getValueUSD(
      weth.address,
      aliceColl
    );
  ...
  it("liquidateTroves(): full redistribution. Compensates the correct amount, and liquidates the remainder", async () => {
    ...
    const STARinDefaultPool_0 = await defaultPool.getSTARDebt();
    ...
    */
    ...
    // Check STAR in DefaultPool has decreased
    const STARinDefaultPool_1 = await defaultPool.getSTARDebt();
    assert.isTrue(STARinDefaultPool_1.gt(STARinDefaultPool_0));
  ...
  it("liquidateTroves(): full offset. Liquidation event emits the correct gas compensation and total liquidated coll and debt", async () => {
    ...
    */
    ...
    // Expect liquidatedDebt = 51 + 190 + 1025 + 13510 = 14646 STAR
    const expectedLiquidatedDebt = A_totalDebt.add(B_totalDebt)
      .add(C_totalDebt)
      .add(D_totalDebt);

    // Liquidate troves A-D
    const liquidationTxData = await troveManager.batchLiquidateTroves(
      [alice, bob, dennis, carol],
      liquidator,
      { from: liquidator, gasPrice: 0 }
    );
    ...
    const [
      ...
      loggedColl,
      loggedGasComp,
    ] = th.getEmittedLiquidationValues(liquidationTxData, wethIDX);
  ...
  it("Trove ordering: same collateral, decreasing debt. Price successively increases. Troves should maintain ordering by ICR", async () => {
    ...
    const firstColl = (await troveManager.getTroveColls(_10_accounts[0]))[1][
      wethIDX
    ];
    ...
    while (price < 210) {
      ...
      const ICRList = [];
      const coll_firstTrove = (
        await troveManager.getTroveColls(_10_accounts[0])
      )[1][wethIDX];
      const gasComp_firstTrove = await troveManagerTester.getCollGasCompensation(
        weth.address,
        coll_firstTrove
      );
  ...
  it("Trove ordering: increasing collateral, constant debt. Price successively increases. Troves should maintain ordering by ICR", async () => {
    ...
    let coll = 50;
    // create 20 troves, increasing collateral, constant debt = 100STAR
