...
contract("StabilityPool", async (accounts) => {
  ...
  const openTroveToken = async (token, params) =>
    th.openTroveWithToken(contracts, token, params);
  ...
  describe("Stability Pool Mechanisms", async () => {
    ...
    it("MULTICOLLATERAL withdrawFromSP(): partial retrieval - retrieves correct STAR amount and the entire ETH Gain, and updates deposit", async () => {
      // --- SETUP ---
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveSTARAmount(dec(18500, 18)),
        whale,
        whale,
      ...
      // Whale deposits 10000 STAR in StabilityPool
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveSTARAmount(dec(2400, 18)),
        defaulter_1,
        defaulter_1,
        [weth.address],
        [dec(20, "ether")],
      ...
      // await borrowerOperations.openTrove(th._100pct, await getOpenTroveSTARAmount(dec(25000, 18)), bob, bob,
      //       [weth.address, wavax.address],
      //       [dec(100, 'ether'), dec(100, 'ether')],
      //       { from: bob }
      // )
      ...
      assert.equal(
        defaulter1_ICR_pre.toString(),
        "1666666666666666666",
        "Defaulter 1 ICR incorrect"
      );
      assert.equal(
        defaulter2_ICR_pre.toString(),
        "1750000000000000000",
        "Defaulter 2 ICR incorrect"
      );

      // // --- TEST ---

      // Alice makes deposit #1: 2000 STAR
      ...
      const [
        liquidatedDebt_1,
        starGasComp_1,
        liquidatedCollTokens_1,
        liquidatedCollAmounts_1,
        totalCollGasCompTokens_1,
      ...
      const [
        liquidatedDebt_2,
        starGasComp_2,
        liquidatedCollTokens_2,
        liquidatedCollAmounts_2,
        totalCollGasCompTokens_2,
        totalCollGasCompAmounts_2,
      ] = await th.getEmittedLiquidationValuesMulti(liquidationTX_2);

      // Alice STARLoss is ((2000/(10000 + 2000)) * liquidatedDebt), for each liquidation

      const expectedSTARLoss_A = liquidatedDebt_1
        .mul(toBN(aliceInSP))
        .div(toBN(aliceInSP).add(toBN(whaleInSP)))
        .add(
          liquidatedDebt_2
            .mul(toBN(aliceInSP))
            .div(toBN(aliceInSP).add(toBN(whaleInSP)))
      ...
      const expectedCompoundedSTARDeposit_A = toBN(aliceInSP).sub(
        expectedSTARLoss_A
      );
      ...
      // Alice retrieves part of her entitled STAR: 1800 STARer
      ...
      const alice_WETH_gain = alice_post_WETH_balance.sub(
        alice_pre_WETH_balance
      );
      const alice_WAVAX_gain = alice_post_WAVAX_balance.sub(
        alice_pre_WAVAX_balance
      );
      const alice_expected_WETH_gain = toBN(dec(20, 18))
        .mul(aliceInSP)
        .div(aliceInSP.add(whaleInSP))
        .mul(toBN(dec(995, 18)))
        .div(toBN(dec(1000, 18)));
      const alice_expected_WAVAX_gain = toBN(dec(21, 18))
        .mul(aliceInSP)
        .div(aliceInSP.add(whaleInSP))
        .mul(toBN(dec(995, 18)))
        .div(toBN(dec(1000, 18)));
      ...
      assert.isAtMost(
        th.getDifference(alice_WETH_gain, alice_expected_WETH_gain),
        100000
      );

      // gains from WAVAX liquidation
      assert.isAtMost(
        th.getDifference(alice_WAVAX_gain, alice_expected_WAVAX_gain),
        100000
      );
      ...
      const expectedSTARLoss_whale = liquidatedDebt_1
        .mul(whaleInSP)
        .div(aliceInSP.add(whaleInSP))
        .add(liquidatedDebt_2.mul(whaleInSP).div(aliceInSP.add(whaleInSP)));

      const expectedCompoundedSTARDeposit_Whale = toBN(whaleInSP).sub(
        expectedSTARLoss_whale
      );
      const compoundedSTARDeposit_Whale = await stabilityPool.getCompoundedSTARDeposit(
        whale
      );

      assert.isAtMost(
        th.getDifference(
          expectedCompoundedSTARDeposit_Whale,
          compoundedSTARDeposit_Whale
        ),
        100000
      );

      await stabilityPool.withdrawFromSP(dec(1000, 18), { from: whale }); // whale withdraws 1000 STAR from SP

      const whale_post_WETH_balance = await weth.balanceOf(whale);
      const whale_post_WAVAX_balance = await wavax.balanceOf(whale);

      const whale_WETH_gain = whale_post_WETH_balance.sub(
        whale_pre_WETH_balance
      );
      const whale_WAVAX_gain = whale_post_WAVAX_balance.sub(
        whale_pre_WAVAX_balance
      );

      const whale_expected_WETH_gain = toBN(dec(20, 18))
        .mul(whaleInSP)
        .div(aliceInSP.add(whaleInSP))
        .mul(toBN(dec(995, 18)))
        .div(toBN(dec(1000, 18)));
      const whale_expected_WAVAX_gain = toBN(dec(21, 18))
        .mul(whaleInSP)
        .div(aliceInSP.add(whaleInSP))
        .mul(toBN(dec(995, 18)))
        .div(toBN(dec(1000, 18)));
      ...
      // gains from WETH liquidation
      assert.isAtMost(
        th.getDifference(whale_WETH_gain, whale_expected_WETH_gain),
        100000
      );

      // gains from WAVAX liquidation
      assert.isAtMost(
        th.getDifference(whale_WAVAX_gain, whale_expected_WAVAX_gain),
        100000
      );

      const expectedNewDeposit_Whale = compoundedSTARDeposit_Whale.sub(
        toBN(dec(1000, 18))
      );

      // check whale's deposit has been updated to equal her compounded deposit minus her withdrawal */
      const newWhaleDeposit = (
        await stabilityPool.deposits(whale)
      )[0].toString();
      assert.isAtMost(
        th.getDifference(newWhaleDeposit, expectedNewDeposit_Whale),
        100000
      );

      // // Expect whale has withdrawn all ETH gain
      const whale_pendingETHGain = (
        await stabilityPool.getDepositorGains(whale)
      )[1][0];
      assert.equal(whale_pendingETHGain, 0);

      // // Expect whale has withdrawn all WAVAX gain
      const whale_pendingWAVAXGain = (
        await stabilityPool.getDepositorGains(whale)
      )[1][1];
    ...
    it("open, deposit into SP, liquidate, deposit into SP #2, liquidate", async () => {
      ...
      const expectedSTARLoss_A = liquidatedDebt_1
        .mul(toBN(aliceInSP))
        .div(toBN(aliceInSP).add(toBN(whaleInSP).add(toBN(bobInSP))))
        .add(
          liquidatedDebt_2
            .mul(toBN(aliceInSP))
            .div(toBN(aliceInSP).add(toBN(whaleInSP).add(toBN(bobInSP))))
      ...
      const alice_expected_WETH_gain = toBN(dec(20, 18))
        .mul(aliceInSP)
        .div(aliceInSP.add(whaleInSP).add(bobInSP))
        .mul(toBN(dec(995, 18)))
        .div(toBN(dec(1000, 18)));
      const alice_expected_WAVAX_gain = toBN(dec(21, 18))
        .mul(aliceInSP)
      ...
      // carol makes deposit #1: 2000 STAR
    ...
    it("withdrawFromSP(): doesn't impact any troves, including the caller's trove", async () => {
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveSTARAmount(dec(12000, 18)),
        alice,
        alice,
        [weth.address, wavax.address],
        [dec(100, "ether"), dec(100, "ether")],
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveSTARAmount(dec(35000, 18)),
        carol,
        carol,
        [weth.address, wavax.address],
        [dec(100, "ether"), dec(100, "ether")],
      ...
      const wavaxIDX = await contracts.whitelist.getIndex(
        contracts.wavax.address
      );
      const whale_Coll_Before_1 = (await troveManager.getTroveColls(whale))[1][
        wavaxIDX
      ].toString();
      const alice_Coll_Before_1 = (await troveManager.getTroveColls(alice))[1][
        wavaxIDX
      ].toString();
      const bob_Coll_Before_1 = (await troveManager.getTroveColls(bob))[1][
        wavaxIDX
      ].toString();
      const carol_Coll_Before_1 = (await troveManager.getTroveColls(carol))[1][
        wavaxIDX
      ].toString();
      ...
      const whale_Coll_After_1 = (await troveManager.getTroveColls(whale))[1][
        wavaxIDX
      ].toString();
      const alice_Coll_After_1 = (await troveManager.getTroveColls(alice))[1][
        wavaxIDX
      ].toString();
      const bob_Coll_After_1 = (await troveManager.getTroveColls(bob))[1][
        wavaxIDX
      ].toString();
      const carol_Coll_After_1 = (await troveManager.getTroveColls(carol))[1][
        wavaxIDX
      ].toString();
