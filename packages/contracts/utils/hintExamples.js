...
async function main() {
  ...
  const PREONContracts = await dh.deployPREONContractsHardhat(
    ARBITRARY_ADDRESS,
    ARBITRARY_ADDRESS,
    ARBITRARY_ADDRESS
  );
  ...
  const STARAmount = toBN(toWei("2500")); // borrower wants to withdraw 2500 STAR
  ...
  const liquidationReserve = await troveManager.STAR_GAS_COMPENSATION();
  const expectedFee = await troveManager.getBorrowingFeeWithDecay(STARAmount);

  // Total debt of the new trove = STAR amount drawn, plus fee, plus the liquidation reserve
  const expectedDebt = STARAmount.add(expectedFee).add(liquidationReserve);
  ...
  await borrowerOperations.openTrove(maxFee, STARAmount, upperHint, lowerHint, {
    value: ETHColl,
  });
  ...
  const STARRepayment = toBN(toWei("230")); // borrower wants to repay 230 STAR
  ...
  const newDebt = debt.sub(STARRepayment);
  ...
  const redemptionhint = await hintHelpers.getRedemptionHints(STARAmount, 50);

  const {
    0: firstRedemptionHint,
    1: partialRedemptionNewICR,
    2: truncatedSTARAmount,
  } = redemptionhint;
  ...
  /* Finally, perform the on-chain redemption, passing the truncated STAR amount, the correct hints, and the expected
