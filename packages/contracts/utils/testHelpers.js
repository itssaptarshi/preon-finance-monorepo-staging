...
class TestHelper {
  ...
  // Adds the gas compensation (50 STAR) (Why does say 50 here? - from @RoboPreon)
  ...
  static async getTroveEntireDebt(contracts, trove) {
    return this.toBN(
      (await contracts.troveManager.getEntireDebtAndColls(trove))[0]
    );
  }
  ...
  /*
   * given the requested STAR amomunt in openTrove, returns the total debt
   * So, it adds the gas compensation and the borrowing fee
   */
  static async getOpenTroveTotalDebt(contracts, starAmount) {
    const fee = await contracts.troveManager.getBorrowingFee(starAmount);
    // console.log("Fee " + fee)
    const compositeDebt = await this.getCompositeDebt(contracts, starAmount);
    // console.log("Composite debt " + compositeDebt)
    return compositeDebt.add(fee);
  }

  /*
   * given the desired total debt, returns the STAR amount that needs to be requested in openTrove
   * So, it subtracts the gas compensation and then the borrowing fee
   */
  static async getOpenTroveSTARAmount(contracts, totalDebt) {
  ...
  static async getAmountWithBorrowingFee(contracts, starAmount) {
    const fee = await contracts.troveManager.getBorrowingFee(starAmount);
    return starAmount.add(fee);
  }
  ...
  static getEmittedRedemptionValues(redemptionTx) {
    for (let i = 0; i < redemptionTx.logs.length; i++) {
      if (redemptionTx.logs[i].event === "Redemption") {
        const attemptedSTARAmount = redemptionTx.logs[i].args[0];
        const actualSTARAmount = redemptionTx.logs[i].args[1];
        const STARFee = redemptionTx.logs[i].args[2];
  ...
  // @RoboPreon: added new functino to be use dto get multi-collateral liquidation values
  static getEmittedLiquidationValuesMulti(liquidationTx) {
    for (let i = 0; i < liquidationTx.logs.length; i++) {
      if (liquidationTx.logs[i].event === "Liquidation") {
        ...
        const starGasComp = liquidationTx.logs[i].args[1];
  ...
  // @RoboPreon: changed this to fit one collateral-type test cases.
  static getEmittedLiquidationValues(liquidationTx, tokenIDX) {
    for (let i = 0; i < liquidationTx.logs.length; i++) {
      if (liquidationTx.logs[i].event === "Liquidation") {
        ...
        return [
          liquidatedDebt,
          liquidatedCollAmounts[tokenIDX],
          totalCollGasCompAmounts[tokenIDX],
        ];
      }
    }
    throw "The transaction logs do not contain a liquidation event";
  ...
  static getSTARFeeFromSTARBorrowingEvent(tx) {
    for (let i = 0; i < tx.logs.length; i++) {
      if (tx.logs[i].event === "STARBorrowingFeePaid") {
        return tx.logs[i].args[1].toString();
      }
    }
    throw "The transaction logs do not contain an STARBorrowingFeePaid event";
  }
  ...
  static async getEntireCollAndDebt(contracts, account) {
    ...
    const rawColl = (await contracts.troveManager.getTroveColls(account))[1][
      wethIDX
    ];
    const rawDebt = await contracts.troveManager.getTroveDebt(account);
    const pendingETHReward = (
      await contracts.troveManager.getPendingCollRewards(account)
    )[1][wethIDX];
    const pendingSTARDebtReward = await contracts.troveManager.getPendingSTARDebtReward(
      account
    );
    const entireColl = rawColl.add(pendingETHReward);
    const entireDebt = rawDebt.add(pendingSTARDebtReward);
  ...
  static async getCollAndDebtFromWithdrawSTAR(contracts, account, amount) {
  ...
  static async getCollAndDebtFromRepaySTAR(contracts, account, amount) {
    const { entireColl, entireDebt } = await this.getEntireCollAndDebt(
      contracts,
      account
    );
  ...
  static async getCollAndDebtFromAdjustment(
    ...
    const fee = STARChange.gt(this.toBN("0"))
      ? await contracts.troveManager.getBorrowingFee(STARChange)
      : this.toBN("0");
    const newColl = entireColl.add(ETHChange);
    const newDebt = entireDebt.add(STARChange).add(fee);
  ...
  static async openTrove_allAccounts(
    ...
    const gasCostList = [];
    const totalDebt = await this.getOpenTroveTotalDebt(contracts, STARAmount);
  ...
  static async openTrove_allAccounts_randomETH_ProportionalSTAR(
    minETH,
    maxETH,
    accounts,
    contracts,
    ...
    for (const account of accounts) {
      const randCollAmount = this.randAmountInWei(minETH, maxETH);
      const proportionalSTAR = web3.utils
        .toBN(proportion)
  ...
  static async openTrove_allAccounts_randomETH_randomSTAR(
    minETH,
    maxETH,
    accounts,
    contracts,
    ...
    for (const account of accounts) {
      ...
      const randSTARProportion = this.randAmountInWei(
        minSTARProportion,
        maxSTARProportion
      );
      const proportionalSTAR = web3.utils
        .toBN(randSTARProportion)
      ...
      if (logging && tx.receipt.status) {
        i++;
        const ICR = await contracts.troveManager.getCurrentICR(account);
        // console.log(`${i}. Trove opened. addr: ${this.squeezeAddr(account)} coll: ${randCollAmount} debt: ${proportionalSTAR} ICR: ${ICR}`)
      }
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
    }
  ...
  static async openTrove_allAccounts_randomSTAR(
    minSTAR,
    maxSTAR,
    accounts,
    contracts,
    ...
    for (const account of accounts) {
      const randSTARAmount = this.randAmountInWei(minSTAR, maxSTAR);
      const totalDebt = await this.getOpenTroveTotalDebt(
        contracts,
        randSTARAmount
      );
  ...
  static async openTrove_allAccounts_decreasingSTARAmounts(
    accounts,
    contracts,
    ETHAmount,
    maxSTARAmount
    ...
    let i = 0;
    for (const account of accounts) {
      const STARAmount = (maxSTARAmount - i).toString();
      const STARAmountWei = web3.utils.toWei(STARAmount, "ether");
      const totalDebt = await this.getOpenTroveTotalDebt(
        contracts,
        STARAmountWei
      );
  ...
  static async openTrove(
    ...
    if (!extraSTARAmount) extraSTARAmount = this.toBN(0);
    else if (typeof extraSTARAmount == "string")
      extraSTARAmount = this.toBN(extraSTARAmount);
    ...
    if (ICR) {
      const collateralAmount = ICR.mul(totalDebt)
        .div(price)
        .div(this.toBN(this.dec(1, this.toBN(18).sub(await token.decimals()))));
      collateral = collateralAmount;
    } else {
      collateral = extraParams.value;
    }
    const tokenMintedSuccessfully = await this.addERC20(
      token,
      account,
    ...
    extraParams.value = 0;
    // console.log("STAR Amount (minDebt + extra STAR amount)", starAmount.toString())
  ...
  static async openTroveWithJLPManual(
    contracts,
    { Zapper, token, AVAXIn, signer }
  ) {
    ...
    const approval = await WToken.allowance(
      signer,
      contracts.borrowerOperations.address
    );
      ...
      const collateralAmount = ICR.mul(totalDebt)
        ...
        .div(
          this.toBN(
            this.dec(
              1,
              this.toBN(this.toBN(this.dec(18, 0)).sub(await token.decimals()))
            )
          )
        );
      collateral = collateralAmount;
  ...
  static async openTroveWithToken(
    contracts,
    token,
    {
      maxFeePercentage,
    ...
    const collateralVC = await contracts.whitelist.getValueVC(
    ...
    // mint weth for from account (in extra params) and approve borrowerOperations to use it
    const tokenMint = await this.addERC20(
      token,
      account,
      contracts.borrowerOperations.address,
      collateralAmount,
  ...
  static async addColl(contracts, collateralAmount, account) {
    ...
    await contracts.borrowerOperations.addColl(
      [contracts.weth.address],
      [collateralAmount],
      account,
      account,
  ...
  static async leftAssertCollateralsEqual(
    tokens1,
    amounts1,
    tokens2,
    amounts2
  ...
  //   if (!maxFeePercentage) maxFeePercentage = this._100pct
  //   if (!extraSTARAmount) extraSTARAmount = this.toBN(0)
  //   else if (typeof extraSTARAmount == 'string') extraSTARAmount = this.toBN(extraSTARAmount)
  //   if (!upperHint) upperHint = this.ZERO_ADDRESS
  //   if (!lowerHint) lowerHint = this.ZERO_ADDRESS
  ...
  //     extraParams._starAmount, upperHint, lowerHint, extraParams._colls, extraParams._amounts, extraParams)

  //   return {
  //     starAmount: extraParams._starAmount,
  //     netDebt,
  //     totalDebt,
  //     ICR,
  ...
  static async openTrove2(
    contracts,
    { WJLPIn: finalWJLPBalance, signer: harry }
  ) {
    const MIN_DEBT = (
      await this.getNetBorrowingAmount(
        contracts,
  ...
  // debt of extraSTARAmount
  static async openTroveWithColls(
    contracts,
    {
      maxFeePercentage,
      extraSTARAmount,
    ...
    starAmount = MIN_DEBT.add(extraSTARAmount);

    if (!ICR) ICR = this.toBN(this.dec(15, 17));
    // 150%
    else if (typeof ICR == "string") ICR = this.toBN(ICR);
    ...
    for (let i = 0; i < colls.length; i++) {
      const VC = await contracts.whitelist.getValueVC(
        collsAddress[i],
        amounts[i]
      );
    ...
    console.log("YUDS Amount: ", starAmount.toString());
    tx = await contracts.borrowerOperations.openTrove(
      maxFeePercentage,
      starAmount,
      upperHint,
  ...
  static async openTroveWithCollsOld(
    contracts,
    {
      maxFeePercentage,
      extraSTARAmount,
    ...
    if (!upperHint) upperHint = this.ZERO_ADDRESS;
    if (!lowerHint) lowerHint = this.ZERO_ADDRESS;
    if (!STARAmount) STARAmount = this.toBN(0);
    ...
    if (STARAmount.gt(MIN_DEBT)) starAmount = STARAmount;
    else starAmount = MIN_DEBT.add(extraSTARAmount);
    ...
    if (ICR) {
      starAmount = await this.getOpenTroveSTARAmount(
        contracts,
        totalVC.mul(this.toBN(this.dec(1, 18))).div(ICR)
      );
    }
    for (let i = 0; i < colls.length; i++) {
      await this.addERC20(
    ...
    let tx = await contracts.borrowerOperations.openTrove(
      maxFeePercentage,
      starAmount,
      upperHint,
      lowerHint,
  ...
  static async openTroveWithCollsFixedSTAR(
    contracts,
    {
      maxFeePercentage,
      extraSTARAmount,
  ...
  static async addERC20(
    ...
    const tokenApprovedAmount = await token.allowance(
      account,
      addressToApprove
    );
    ...
    return (
      this.toNormalBase(postMintBalance.sub(preMintBalance)) ==
        this.toNormalBase(collateralAmount) &&
      this.toNormalBase(collateralAmount) ==
        this.toNormalBase(tokenApprovedAmount)
    );
  }

  static async addMultipleERC20(
    account,
    addressToApprove,
    tokens,
    amounts,
  ...
  static async mintAndApproveSTARToken(
    contracts,
    account,
    addressToApprove,
    collateralAmount,
    ...
    const preMintBalance = await contracts.starToken.balanceOf(account);
    await contracts.starToken.unprotectedMint(account, collateralAmount);
    const postMintBalance = await contracts.starToken.balanceOf(account);

    await contracts.starToken.callInternalApprove(
      account,
      addressToApprove,
      collateralAmount
    );
    const approvedToken = await contracts.starToken.allowance(
      account,
      addressToApprove
    );
  ...
  static async addTokensToAccountsAndOpenTroveWithICR(
    contracts,
    ICR,
    accounts,
    tokens
    ...
    for (let i = 0; i < accounts.length; i++) {
      ...
      await tokens[index1].approve(
        contracts.borrowerOperations.address,
        this.toBN(this.dec(1, 30)),
        extraParams
      );
      await tokens[index2].approve(
        contracts.borrowerOperations.address,
        this.toBN(this.dec(1, 30)),
        extraParams
      );
      await tokens[index3].approve(
        contracts.borrowerOperations.address,
        this.toBN(this.dec(1, 30)),
        extraParams
      );
  ...
  static async withdrawSTAR(
    contracts,
    { maxFeePercentage, starAmount, ICR, upperHint, lowerHint, extraParams }
  ) {
    if (!maxFeePercentage) maxFeePercentage = this._100pct;
    if (!upperHint) upperHint = this.ZERO_ADDRESS;
    if (!lowerHint) lowerHint = this.ZERO_ADDRESS;
    ...
    assert(
      !(starAmount && ICR) && (starAmount || ICR),
      "Specify either star amount or target ICR, but not both"
    );
    ...
    if (ICR) {
      ...
      starAmount = await this.getNetBorrowingAmount(
        contracts,
        increasedTotalDebt
      );
    } else {
    ...
    await contracts.borrowerOperations.withdrawSTAR(
      maxFeePercentage,
      starAmount,
      upperHint,
      lowerHint,
  ...
  // static async adjustTrove_allAccounts(accounts, contracts, ETHAmount, STARAmount) {
  //   const gasCostList = []
  //
  //   for (const account of accounts) {
  //     let tx;
  ...
  //     let STARChangeBN = this.toBN(STARAmount)
  //
  //     const { newColl, newDebt } = await this.getCollAndDebtFromAdjustment(contracts, account, ETHChangeBN, STARChangeBN)
  //     const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)
  //
  //     const zero = this.toBN('0')
  //
  //     let isDebtIncrease = STARChangeBN.gt(zero)
  //     STARChangeBN = STARChangeBN.abs()
  //
  //     // Add ETH to trove
  //     if (ETHChangeBN.gt(zero)) {
  //       tx = await contracts.borrowerOperations.adjustTrove(this._100pct, 0, STARChangeBN, isDebtIncrease, upperHint, lowerHint, { from: account, value: ETHChangeBN })
  //       // Withdraw ETH from trove
  //     } else if (ETHChangeBN.lt(zero)) {
  //       ETHChangeBN = ETHChangeBN.neg()
  //       tx = await contracts.borrowerOperations.adjustTrove(this._100pct, ETHChangeBN, STARChangeBN, isDebtIncrease, upperHint, lowerHint, { from: account })
  //     }
  //
  //     const gas = this.gasUsed(tx)
  //     gasCostList.push(gas)
  ...
  static async adjustTrove_allAccounts_randomAmount(
    ...
    for (const account of accounts) {
      ...
      let STARChangeBN = this.toBN(this.randAmountInWei(STARMin, STARMax));
  ...
  static async adjustTrove_allAccounts_randomAmount(
    ...
    for (const account of accounts) {
      ...
      const gas = this.gasUsed(tx);
      // console.log(`ETH change: ${ETHChangeBN},  STARChange: ${STARChangeBN}, gas: ${gas} `)
  ...
  static async withdrawSTAR_allAccounts(accounts, contracts, amount) {
    const gasCostList = [];

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromWithdrawSTAR(
        contracts,
        account,
        amount
      );
      ...
      const tx = await contracts.borrowerOperations.withdrawSTAR(
        this._100pct,
        amount,
        upperHint,
        lowerHint,
  ...
  static async withdrawSTAR_allAccounts_randomAmount(
    min,
    max,
    accounts,
    contracts
    ...
    for (const account of accounts) {
      const randSTARAmount = this.randAmountInWei(min, max);
  ...
  static async repaySTAR_allAccounts(accounts, contracts, amount) {
    const gasCostList = [];

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromRepaySTAR(
        contracts,
        account,
        amount
      );
      ...
      const tx = await contracts.borrowerOperations.repaySTAR(
        amount,
        upperHint,
        lowerHint,
        { from: account }
  ...
  static async repaySTAR_allAccounts_randomAmount(
    min,
    max,
    accounts,
    contracts
  ...
  static async performRedemptionTx(
    ...
    var finalSTARAmount = this.toBN(0);
    var finalSTARFeeAmount = this.toBN(0);
    if (STARMaxFee == 0) {
      finalSTARAmount = await this.estimateSTAREligible(contracts, STARAmount);
      finalSTARFeeAmount = this.toBN(STARAmount).sub(finalSTARAmount);
    } else {
      finalSTARAmount = STARAmount;
      finalSTARFeeAmount = await this.estimateRedemptionFee(
        contracts,
        this.toBN(STARAmount)
      );
    }
    // TODO Ensure max fee is not greater than the amount of STAR fee calculated
    ...
    finalSTARAmount = this.toBN(finalSTARAmount);
    const totalSTAR_ToPullIn = this.toBN(1000000).mul(
      finalSTARAmount.add(finalSTARFeeAmount)
    );
    console.log("TO Pull IN", totalSTAR_ToPullIn.toString());

    await contracts.starToken.approve(
  ...
  static async performRedemptionWithMaxFeeAmount(
    redeemer,
    contracts,
    STARAmount,
    STARMaxFee,
    ...
    const finalSTARAmount = STARAmount;
    const finalSTARFeeAmount = STARMaxFee;
    // TODO Ensure max fee is not greater than the amount of STAR fee calculated
  ...
  static async estimateSTAREligible(contracts, STARAmount) {
    const totalSTARSupply = await contracts.troveManagerRedemptions.getEntireSystemDebt(); // S
    ...
    const sqrtTerm2 = this.toBN(this.dec(2, 0))
      .mul(this.toBN(STARAmount))
      .mul(this.toBN(this.dec(1, 36)))
      .div(totalSTARSupply);
    const finalSqrtTerm = this.sqrt(
      sqrtTerm.add(sqrtTerm2).mul(this.toBN(this.dec(1, 18)))
    ); //.div(this.toBN(this.dec(1,9)))

    const finalSTARAmount = totalSTARSupply
      .mul(finalSqrtTerm.sub(squareTerm.mul(this.toBN(this.dec(1, 9)))))
      .div(this.toBN(this.dec(1, 27)));
    // console.log("FINAL YUDS AMOUNT : " + finalSTARAmount)
    // console.log("FINAL STAR FEE : ", this.toBN(STARAmount).sub(finalSTARAmount).toString())
    return finalSTARAmount;
  }
  ...
  static sqrt(x) {
    ...
    while (z.lt(y)) {
      ...
      z = x
        .div(z)
        .add(z)
        .div(this.toBN(this.dec(2, 0)));
    }
    return y;
  }

  static async estimateRedemptionFee(contracts, STARAmount) {
    const estimateUpdatedBaseRate = await this.estimateUpdatedBaseRateFromRedemption(
      contracts,
      STARAmount
    );
    // console.log("ESTIMATED UDPATED BSE RATE " + estimateUpdatedBaseRate.toString())
    return this.toBN(estimateUpdatedBaseRate)
      .add(this.toBN(this.dec(5, 15)))
      .mul(STARAmount)
      .div(this.toBN(this.dec(1, 18)));
  }

  static async estimateUpdatedBaseRateFromRedemption(contracts, STARAmount) {
    const STARSupplyAtStart = await contracts.troveManagerRedemptions.getEntireSystemDebt();
    const decayedBaseRate = await contracts.troveManager.calcDecayedBaseRate();

    /* Convert the drawn ETH back to STAR at face value rate (1 STAR:1 USD), in order to get
     * the fraction of total supply that was redeemed at face value. */
    const redeemedSTARFraction = STARAmount.mul(this.toBN(this.dec(1, 18))).div(
      STARSupplyAtStart
    );
    const BETA = 2;
    const newBaseRate = decayedBaseRate.add(
      redeemedSTARFraction.div(this.toBN(this.dec(BETA, 0)))
    );
    // console.log("STARSUPPLY AT START ", STARSupplyAtStart.toString())
    // console.log("REDEEMED STAR FRACTION " + redeemedSTARFraction.toString())
  ...
  static async provideToSP_allAccounts_randomAmount(
    ...
    const gasCostList = [];
    for (const account of accounts) {
      const randomSTARAmount = this.randAmountInWei(min, max);
  ...
  static async withdrawFromSP_allAccounts_randomAmount(
    ...
    for (const account of accounts) {
      const randomSTARAmount = this.randAmountInWei(min, max);
      const tx = await stabilityPool.withdrawFromSP(randomSTARAmount, {
        from: account,
      });
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
