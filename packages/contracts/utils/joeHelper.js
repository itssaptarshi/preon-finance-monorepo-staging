...
const wavax_weth_Address = "0xFE15c2695F1F920da45C30AAE47d11dE51007AF9";
...
async function getWAVAX(signerAddress) {
...
async function getJOEContracts(signerAddress) {
  const joeRouter = new ethers.Contract(
    "0x60aE616a2155Ee3d9A68541Ba4544862310933d4",
    routerABI,
    await hre.ethers.getSigner(signerAddress)
  );
  const joeZap = new ethers.Contract(
    "0x2C7B8e971c704371772eDaf16e0dB381A8D02027",
    zapABI,
    await hre.ethers.getSigner(signerAddress)
  );
  const wavax_weth_jlp = new ethers.Contract(
    wavax_weth_Address,
    jlpABI,
    await hre.ethers.getSigner(signerAddress)
  );
  ...
  return {
    joeRouter: joeRouter,
    joeZap: joeZap,
    wavax_weth_jlp: wavax_weth_jlp,
  };
}

async function wrapAVAX(signerAddress, amount) {
  const wavax = await getWAVAX(signerAddress);
  await wavax.deposit({ value: amount.toString() });
}

// gets WAVAX-WETH JLP tokens for signer and returns the number that it got
// also approves all newly created jlp tokens to be spent
async function zapForWAVAX_WETH_JLP(signerAddress, AVAXIn) {
  const joe = await getJOEContracts(signerAddress);
  const initial = await joe.wavax_weth_jlp.balanceOf(signerAddress);
  await joe.joeZap.zapIn(wavax_weth_Address, { value: AVAXIn.toString() });

  const final = await joe.wavax_weth_jlp.balanceOf(signerAddress);
  const diff = toBN(final).sub(toBN(initial));
  return diff;
}

async function approveJLP(signerAddress, amount, to) {
  const joe = await getJOEContracts(signerAddress);
  await joe.wavax_weth_jlp.approve(to, amount.toString());
}
