...
```typescript
export declare type TroveCreationParams<T = unknown> = _CollateralDeposit<T> & _NoCollateralWithdrawal & _YUSDBorrowing<T> & _NoYUSDRepayment;
```
...
<tr> <td> borrowYUSD </td> <td> T </td> <td> The amount of YUSD that's borrowed. </td> </tr>
